...
const PasswordResetForSignupPage = () => {
  ...
  const signInHandler = async (e) => {
    ...
    try {
      setLoadig(true)
      const response = await axios.post(`http://192.168.0.141:8092/api/auth/loginnnn`, input, {
        headers: {
    ...
    <div className='flex items-center w-screen h-screen justify-center bg-purple-100'>
      <form onSubmit={signInHandler} className='shadow-lg flex flex-col gap-4 p-8 bg-white rounded-lg'>
        <div className='my-3 flex justify-center items-center flex-col'>
          ...
          <p className='text-sm text-center text-wrap'>Verify your account details</p>
        </div>
        <div>
...
export default PasswordResetForSignupPage