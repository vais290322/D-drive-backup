...
const InstituteInformationPage = () => {
  ...
  const [submittedData, setSubmittedData] = useState({
    ...
    RegistrationNumber: "GVHS-2024-001",
    Address: "123 Green Street, Valley Town, NY 12345",
    Email: "contact@greenvalleyhigh.edu",
    Phone: "+1 (555) 123-4567",
    Website: "https://greenvalleyhigh.edu",
    schoolLogo: "https://picsum.photos/500/300",
    schoolImage: "https://picsum.photos/300/200",

    Description: "A premier institution for holistic education.",
    PrincipalName: "Dr. Sarah Johnson",
    PrincipalEmail: "sarah.johnson@greenvalleyhigh.edu",
    PrincipalPhone: "+1 (555) 765-4321",
  });
  ...
  return (
    <div className={` ${theme === "light" ? "dark" : "light"} h-auto`}>
      <div
        className={` ${
          theme === "light"
      ...
      >
            ...
            <div className="grid grid-cols-2 gap-4">
              {Object.keys(submittedData).map((key) => (
                <div key={key} className="mb-4">
                  ...
                  {key.includes("Logo") || key.includes("Image") ? (
                    <img
                      src={submittedData[key]}
                      alt={key}
                      className="w-full max-w-[300px] h-auto  rounded-md"
                    />
                  ) : (
                    <span>{submittedData[key]}</span>
                  )}
                </div>
              ))}
            </div>
        ...
        {/* {instituteInformation && (
          <div
            className={`mb-20 p-6 bg-gray-100 rounded-md ${
              theme === "light"
                ? "border-[rgba(193,193,193,0.3)] bg-[#262626] text-white"
                : "bg-white border-slate-200"
            }`}
          >
            <div className="grid grid-cols-2 gap-4 ">
              {Object.keys(instituteInformation).map((key) => (
                <div key={key} className="mb-4">
                  <strong className="block capitalize">
                    {key.replace(/([A-Z])/g, " $1")}:
                  </strong>
                  {key.includes("Logo") || key.includes("Image") ? (
                    <img
                      src={instituteInformation[key]}
                      alt={key}
                      className="w-full max-w-[300px]  rounded-md"
                    />
                  ) : (
                    <span>{instituteInformation[key]}</span>
                  )}
                </div>
              ))}
            </div>
...
{/* {Object.keys(instituteInformation).map((key) => (
  <div key={key} className="mb-4">
    ...
    {typeof instituteInformation[key] === "object" && instituteInformation[key] !== null ? (
      key.includes("Logo") || key.includes("Image") ? (
        <img
          src={instituteInformation[key]}
          alt={key}
        ...
        <pre className="bg-gray-100 p-2 rounded">
          {JSON.stringify(instituteInformation[key], null, 2)}
        </pre>
      )
    ) : (
      <span>{instituteInformation[key]}</span>
...
<div className={`mb-20 p-6 ${theme === "light" ? "bg-[#262626] text-white" : "bg-white"} rounded-md`}>
