...
const Research = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`${baseUrl}/womenstudy`);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await fetch(`${baseUrl}/womenstudy/${confirmDelete._id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="min-h-screen bg-black text-[#E8B245] px-2 sm:px-4 py-6 sm:py-8">
      <h2 className="text-2xl sm:text-3xl font-extrabold mb-4 sm:mb-6 border-b-2 border-[#E8B245] pb-2">
            ...
            <table className="w-full border-collapse text-xs sm:text-sm md:text-base">
              ...
              <tbody>
                {paginatedData.map((entry) => (
                  <tr key={entry._id} className="hover:bg-[#2a2a2a] transition-colors duration-200">
