...
const apiUrl = `${baseUrl}/abstracts-research`;
...
function OngoingResearch() {
  ...
  const handleOpenModal = (entry = null) => {
    setEditEntry(entry);
    setForm(entry ? { title: entry.title, description: entry.description } : { title: "", description: "" });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let body = JSON.stringify({ title: form.title, description: form.description });
      let headers = { "Content-Type": "application/json" };
      const res = await fetch(url, { method, body, headers });
      if (!res.ok) throw new Error("Failed to save entry");
      // Refresh entries
      const refreshed = await fetch(apiUrl).then((r) => r.json());
      if (refreshed && Array.isArray(refreshed.abstracts)) {
        setEntries(refreshed.abstracts);
  ...
  return (
    <div className="min-h-screen bg-black text-[#E8B245] px-2 sm:px-4 py-6 sm:py-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-2">
        <h1 className="text-2xl sm:text-3xl font-bold">Ongoing Research</h1>
        ...
        <table className="w-full border-collapse text-xs sm:text-sm md:text-base">
          ...
          <tbody>
            {paginatedEntries.length === 0 ? (
              <tr>
                <td colSpan={3} className="text-center py-4 text-[#E8B245]">No entries found.</td>
              </tr>
            ) : (
              paginatedEntries.map((entry) => (
                <tr key={entry._id || entry.id} className="hover:bg-[#2a2a2a] transition-colors duration-200">
                  ...
                  <td className="py-2 px-2 sm:px-4 border border-[#E8B245] break-words max-w-[300px] md:max-w-none">{entry.description || "-"}</td>
