...
// import ViewResearch from "./ViewReseach";
import { Eye } from "lucide-react";
import ViewResearch from "./ViewResearch";
const Table = ({ advisors, onEdit, onDelete }) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(5);
  const [filteredData, setFilteredData] = useState(advisors);
              ...
              currentItems.map((adv) => (
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="text-right">{adv?.bio}</span>
                  </div>
            ...
            <tbody>
              {currentItems.length > 0 ? (
                currentItems.map((adv) => (
                  ...
                  >
                    <td className="p-2">{adv?.name}</td>
                    <td className="p-2">{adv?.position}</td>
                    <td className="p-2">{adv?.education}</td>
                    <td className="p-2">
                      {adv?.bio
                        ? adv.bio.length > 15
                          ? `${adv.bio.substring(0, 15)}...`
                          : adv.bio
                        : "No bio available"}
                    </td>
