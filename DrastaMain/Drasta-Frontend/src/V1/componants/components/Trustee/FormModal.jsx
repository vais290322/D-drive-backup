...
const FormModal = ({ open, onClose, onSubmit, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({ 
        ...initialData, 
        img: null // Reset to null when editing to allow new file selection
      });
      setFileName(initialData.img ? 'Image selected' : 'No file selected');
    } else {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm bg-opacity-30 flex justify-center items-center z-50">
      <div className="relative bg-white text-gray-900 p-6 rounded-lg shadow-lg w-full max-w-lg">
        ...
        <h3 className="text-xl font-semibold mb-6">
          {initialData ? "Edit Trustee" : "Add Trustee"}
        </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4" encType="multipart/form-data">
          ...
          <div>
            ...
            <textarea
              name="desc"
              placeholder="Trustee description"
              value={formData.desc}
              onChange={handleChange}
              className="w-full border border-gray-300 p-2 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              rows="3"
