...
const FormModal = ({ open, onClose, onSubmit, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({ name: "", qualification: "", image: null });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formPayload.append("qualification", formData.qualification.trim());
      if (formData.image) {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm bg-opacity-30 flex justify-center items-center z-50">
      <div className="relative bg-white text-gray-900 p-6 rounded-lg shadow-lg w-full max-w-lg">
        ...
        <h3 className="text-xl font-semibold mb-6">
          {initialData ? "Edit Advisor" : "Add Advisor"}
        </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4" encType="multipart/form-data">
          ...
          <div>
            <label className="block mb-2">Qualification<span className="text-red-600">*</span></label>
            <input
              type="text"
              name="qualification"
              placeholder="Advisor qualification"
