...
const Table = ({ data, loading, onDelete }) => {
  ...
  useEffect(() => {
    const filtered = data?.filter(
      (item) =>
        ...
        item.enquiry?.toLowerCase().includes(searchTerm.toLowerCase())
    );
        ...
        {isMobile ? (
          <div className="space-y-2">
            {currentItems.length > 0 ? (
              ...
              <div className="p-4 text-white text-center">No contacts found</div>
            )}
          </div>
        ) : (
              ...
              {currentItems.length > 0 ? (
                currentItems.map((item) => (
                  ...
                  >
                    ...
                    <td className="p-2">
                      {item.enquiry?.length > 15
                        ? `${item.enquiry.substring(0, 15)}...`
                        : item.enquiry}
                    </td>
                ...
                <tr>
                  <td colSpan="6" className="p-4 text-white">
                    No contacts found
                  </td>
                </tr>
              )}
