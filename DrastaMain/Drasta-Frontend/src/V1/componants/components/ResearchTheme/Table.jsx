...
import ViewResearchTheme from "./ViewResearchTheme";
...
const Table = ({ data, loading, onEdit, onDelete }) => {
  ...
  useEffect(() => {
    const filtered = data?.filter(
      (item) =>
        item.contentHeader?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.contentFooter?.toLowerCase().includes(searchTerm.toLowerCase())
    );
        ...
        {isMobile ? (
          <div className="space-y-2">
            {currentItems.length > 0 ? (
              currentItems.map((item) => (
                ...
                >
                  ...
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span>{item.contentHeader?.length > 15 ? `${item.contentHeader.substring(0, 15)}...` : item.contentHeader}</span>
                  </div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="font-bold">Footer:</span>
                    <span>{item.contentFooter?.length > 15 ? `${item.contentFooter.substring(0, 15)}...` : item.contentFooter}</span>
                  </div>
                  <div className="flex justify-end space-x-3 mt-3">
                    <button
                      className="text-blue-400 hover:underline"
                      onClick={() => setViewTheme(item)}
                    >
                      <Eye size={18} />
                    </button>
                    <button
              ...
              <div className="p-4 text-white text-center">No themes found</div>
            )}
          </div>
        ) : (
              ...
              {currentItems.length > 0 ? (
                currentItems.map((item) => (
                  ...
                  >
                    ...
                    <td className="p-2">{item.contentHeader?.length > 15 ? `${item.contentHeader.substring(0, 15)}...` : item.contentHeader}</td>
                    <td className="p-2">
                      {item.contentFooter?.length > 15
                        ? `${item.contentFooter.substring(0, 15)}...`
                        : item.contentFooter}
                    </td>
