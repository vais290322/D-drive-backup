  ...
  return (
    ...
    >
      ...
      <div className="text-center mt-8 text-xs text-white px-4 sm:px-0 font-bold">
        © Copyright 2015 — {new Date().getFullYear()} | Drasta | All Rights Reserved | Powered by{" "}
        <a href="https://www.vais.co.in/" className="text-white underline">
          VAIS Engineering Private Limited
        </a>
        <button
          onClick={() => navigate('/v1-login')}
          className="ml-4 mt-2 px-3 py-1 bg-yellow-400 cursor-pointer text-green-800   rounded-md hover:bg-yellow-500 transition-colors text-xs"
        >
          Admin Login
        </button>
        <button
          onClick={() => navigate('/')}
          className="ml-4 px-3 py-1 bg-yellow-400 cursor-pointer text-green-800   rounded-md hover:bg-yellow-500 transition-colors text-xs"
        >
         ← Back
        </button>
      </div>
