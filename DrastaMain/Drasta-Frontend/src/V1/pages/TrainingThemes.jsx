...
export const TrainingThemes = () => {
  ...
  React.useEffect(() => {
    const fetchThemes = async () => {
      try {
        const response = await fetch(`${API}/api/v1/traning-theme`);
        const data = await response.json();
        setThemes(data?.themes[0] || []); // Ensure data is in the expected format
        console.log("Fetched Themes:", data?.themes[0]);
      } catch (error) {
      ...
      <div className="min-h-[49.6vh] bg-white">
        ...
        <section className="max-w-6xl mx-auto px-4 py-12">
          ...
          <p className="text-sm md:text-[14px] text-gray-700 mt-4 mb-6 leading-relaxed">
            {themes.content || "In C-DRAṢṬᾹ, the experience gained through ongoing research projects feeds into training programs. C-DRAṢṬᾹ also emphasizes the importance of different types of research perspectives for addressing any development dilemma. Training at C-DRAṢṬᾹ, which is realized through Competency Enhancing Workshops (CEWs), is linked with the various stages of the research process. They enable participants to provide effective end-to-end solutions to various development problems. CEWs consist of modules that are stand-alone or collated based on a theme which may be a particular research stage or research approach. Alternatively, CEWs could also correspond to applications in a particular domain (such as education, women’s studies, health, etc.). Experts from both academics and industry conduct the training sessions. The CEWs benefit participants across the social and environmental sciences as also those from different vocations, such as corporate executives, administrators, bureaucrats, young professionals, NGO workers, students and researchers. However, a major thrust area of the CEWs is on imparting training for the design and implementation of evaluation studies for socio-economic development projects under CSR. In order to achieve its objectives, the program design will be customized based on feedback from industry, potential trainees and advisors from academia and practitioners. An illustrative classification of CEWs on Research Methods and its Applications, based on participant need, is provided below."}
