...
import { FiTrash2, FiUpload, FiEdit, FiX, FiSearch } from "react-icons/fi";
...
const Gallery = () => {
  ...
  const fetchImages = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${baseUrl}/about-gallery`);
      const data = await res.json();
      if (data && data.gallery && Array.isArray(data.gallery.images)) {
        setImages(data.gallery.images);
        setFilteredImages(data.gallery.images);
      } else {
        setImages([]);
        setFilteredImages([]);
      }
  ...
  useEffect(() => {
    const results = images.filter(img => {
      const searchStr = (img._id + (img.url || "")).toLowerCase();
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const res = await fetch(`${baseUrl}/about-gallery/${editId}`, {
        method: "PUT",
        body: formData,
      });
      ...
      setEditId(null);
      setFile(null);
      if (document.getElementById('gallery-edit-file-input')) {
        document.getElementById('gallery-edit-file-input').value = "";
      }
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    setError(null);
    try {
      const res = await fetch(`${baseUrl}/about-gallery/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="min-h-screen w-full bg-black text-[#E8B245] px-2 sm:px-4 py-6 sm:py-8 flex flex-col items-center">
      <h2 className="text-2xl sm:text-3xl font-extrabold mb-4 sm:mb-6 border-b-2 border-[#E8B245] pb-2 w-full text-center">Gallery Images</h2>
      {/* Search and Add */}
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center w-full max-w-4xl mx-auto mb-4 sm:mb-6 gap-3 sm:gap-4">
      ...
      {confirmDeleteId && (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex justify-center items-center z-50 transition-all duration-300 px-2">
          <div className="bg-[#181818] text-[#E8B245] w-full max-w-xs sm:max-w-sm p-4 sm:p-6 rounded-lg border border-[#E8B245] relative flex flex-col items-center justify-center mx-auto">
            <button
            ...
            <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-4 text-center">Confirm Deletion</h3>
            <p className="mb-4 sm:mb-6 text-center text-sm sm:text-base">Are you sure you want to delete this image?</p>
            <div className="flex gap-2 sm:gap-4 w-full justify-center">
              <button
                className="px-3 sm:px-4 py-2 bg-red-500 text-white rounded font-bold cursor-pointer hover:bg-red-700 text-xs sm:text-base"
                onClick={async () => { await handleDelete(confirmDeleteId); setConfirmDeleteId(null); }}
              >
                Delete
              </button>
              <button
                className="px-3 sm:px-4 py-2 bg-gray-700 text-white rounded font-bold cursor-pointer hover:bg-gray-900 text-xs sm:text-base"
                onClick={() => setConfirmDeleteId(null)}
              >
                Cancel
              </button>
      ...
      {addMode && (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex justify-center items-center z-50 transition-all duration-300 px-2">
          <div className="bg-[#181818] text-[#E8B245] w-full max-w-xs sm:max-w-lg p-4 sm:p-6 rounded-lg border border-[#E8B245] relative flex flex-col items-center justify-center mx-auto">
            <button
              className="absolute top-2 right-2 sm:top-3 sm:right-3 text-[#E8B245] cursor-pointer hover:text-white"
              onClick={() => { setAddMode(false); setFile(null); }}
            >
              <FiX size={24} />
            </button>
            <h3 className="text-lg sm:text-2xl font-bold mb-2 sm:mb-4">Add Image</h3>
            <label className="mb-3 sm:mb-4 w-full flex items-center border border-[#E8B245] rounded px-2 sm:px-3 py-2 bg-black cursor-pointer">
              ...
              <input
                ...
                className="text-white w-full bg-black focus:outline-none cursor-pointer text-xs sm:text-base"
                style={{ display: 'inline-block' }}
              />
            </label>
            <button
              className="w-full px-3 sm:px-4 py-2 bg-[#E8B245] text-black rounded hover:bg-[#cfa23c] font-bold cursor-pointer text-xs sm:text-base"
      ...
      {editMode && (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex justify-center items-center z-50 transition-all duration-300 px-2">
          <div className="bg-[#181818] text-[#E8B245] w-full max-w-xs sm:max-w-lg p-4 sm:p-6 rounded-lg border border-[#E8B245] relative flex flex-col items-center justify-center mx-auto">
            <button
              className="absolute top-2 right-2 sm:top-3 sm:right-3 text-[#E8B245] cursor-pointer hover:text-white"
              onClick={() => { setEditMode(false); setEditId(null); setFile(null); }}
            >
              <FiX size={24} />
            </button>
            <h3 className="text-lg sm:text-2xl font-bold mb-2 sm:mb-4">Edit Image</h3>
            <label className="mb-3 sm:mb-4 w-full flex items-center border border-[#E8B245] rounded px-2 sm:px-3 py-2 bg-black cursor-pointer">
              <FiUpload className="text-[#E8B245] mr-2" />
              <input
