...
export const Advisors = () => {
  ...
  React.useEffect(() => {
    const fetchPerson = async () => {
      try {
        const response = await fetch(`${API}/api/v1/superadvisors`);
        const data = await response.json();
        setPerson(data.superAdvisors);
        // console.log(data.superAdvisors);
      } catch (error) {
      ...
      <section className="bg-white py-12 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {person.map((advisor, index) => (
              <div key={index} className="flex items-start gap-4">
                <img
                  src={advisor.image ||expert8} 
                  alt={advisor.name}
                ...
                <div className="mt-4">
                  ...
                  <p className="text-sm text-gray-700 pt-4">{advisor.qualification}</p>
                </div>
              </div>
            ))}
          </div>
