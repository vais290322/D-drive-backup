...
const AddNotePage = () => {
  ...
  const fetchNotes = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get(`${backendDomainA}/api/v1/notes/all`);
  ...
  const handleAddSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${backendDomainA}/api/v1/notes/create`, formData);
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`${backendDomainA}/api/v1/notes/update/${currentNote._id}`, formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this note?')) {
      try {
        await axios.delete(`${backendDomainA}/api/v1/notes/delete/${id}`);
        fetchNotes();
        setAlert({
          open: true,
