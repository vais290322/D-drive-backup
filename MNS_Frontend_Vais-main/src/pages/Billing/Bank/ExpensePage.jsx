...
const ExpensePage = () => {
  ...
  useEffect(() => {
   ...
   const fetchExpenses = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${backendDomainA}/api/v1/expense/all`);
  ...
  const resetFilters = () => {
    ...
    const handleAddSubmit = async (e) => {
        ...
        try {
          const response = await axios.post(`${backendDomainA}/api/v1/expense/create`, formData);
          ...
          toast.error(error.response?.data?.message || 'Failed to add expense', 'error');
        }
      };
      ...
      const handleEditSubmit = async (e) => {
        ...
        try {
          const response = await axios.put(`${backendDomainA}/api/v1/expense/update/${currentExpense._id}`, formData);
          if(response.data && response.data.success) {
          ...
          toast.error(error.response?.data?.message || 'Failed to update expense', 'error');
        }
      };
      ...
      const handleDelete = async (id) => {
        ...
        try {
          const response = await axios.delete(`${backendDomainA}/api/v1/expense/delete/${id}`);
          if(response.data && response.data.success) {
          ...
          toast.error(error.response?.data?.message || 'Failed to delete expense', 'error');
        }
      };
...
//   for pdf 
  const generatePDF = () => {
    ...
    const tableRows = filteredExpenses.map(expense => [
      ...
      expense.description.substring(0, 100) + (expense.description.length > 100 ? '...' : '')
    ]);
