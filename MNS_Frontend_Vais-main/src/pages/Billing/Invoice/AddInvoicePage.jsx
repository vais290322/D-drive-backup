...
const InvoiceForm = () => {
  ...
  const [transportationPercentage, setTransportationPercentage] = useState(18);
  ...
  // console.log("gasdxgasdsag", items);
  ...
  const calculateItemFields = (item, sourceField) => {
    if (sourceField === "amount" && item.amount) {
      ...
      const taxAmount =
        formData.taxGroup === "No Tax" ? 0 : (grossAmount * totalTaxRate) / 100;
  ...
  const calculateTotals = (currentItems = items) => {
    ...
    // Calculate grand total without transportation charges
    const grandTotalWithoutTransport = totalTaxableAmount + totalTax;
    // Calculate transportation charges based on default 18%
    const transportationCharges =
      (grandTotalWithoutTransport * transportationPercentage) / 100;
    // Update formData with calculated transportation charges
    setFormData((prev) => ({
      ...prev,
