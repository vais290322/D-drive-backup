  ...
  const handleSaveEdit = async () => {
    try {
      ...
      const completeItems = items.map((item) => {
        // Find the original item data from itemsData if available
        const originalItemData = itemsData.find(
          (originalItem) => originalItem.item_name === item.itemName
        );

        // Calculate additional fields needed for the invoice
        const grossAmount = item.quantity * item.sellingPrice;
        const discountAmount = Number(item.discountAmount || 0);
        const netAmount = grossAmount - discountAmount;
        const taxAmount = (netAmount * item.taxRate) / 100;

        // Combine the edited values with all original item data
        return {
          // Include all properties from the original item data
          ...(originalItemData || {}),

          // Override with the edited values
          itemName: item.itemName,
          item_name: item.itemName, // Ensure both formats are included
          quantity: Number(item.quantity),
          sellingPrice: Number(item.sellingPrice),
          discountAmount: discountAmount,
          taxRate: Number(item.taxRate || 0),
          
          // Add additional fields needed for invoice display
      ...
      if (!response.ok) {
        throw new Error(jsonData.error || jsonData.message || "Failed to update invoice");
      }
