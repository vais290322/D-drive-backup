...
const SnigdhaGstReportPage = () => {
  ...
  const handleExcelExport = () => {
    ...
    XLSX.writeFile(workbook, `GST_Report_Snigdha_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const handlePdfExport = () => {
    ...
    const tableColumn = [
      ...
      "Invoice Value", "Rate", "Taxable Value", "Integrated Tax Amount", "Central Tax Amount", "State/UT Tax Amount"
    ];
    ...
    doc.autoTable({
      ...
      columnStyles: {
        ...
        4: { cellWidth: 26 },   // GSTIN
        5: { cellWidth: 16 },   // HSN
        6: { cellWidth: 26 },   // Description
        7: { cellWidth: 12 },   // UQC
        8: { cellWidth: 14 },   // Total Quantity
        9: { cellWidth: 18 },   // Invoice Value
        10: { cellWidth: 14 },  // Rate
        11: { cellWidth: 20 },  // Taxable Value
        12: { cellWidth: 20 },  // Integrated Tax Amount
        13: { cellWidth: 20 },  // Central Tax Amount
        14: { cellWidth: 20 },  // State/UT Tax Amount
      }
    });

    doc.save(`GST_Report_Snigdha_${new Date().toISOString().split('T')[0]}.pdf`);
  };
  ...
  const getAllInvoices = async() => {
    try {
      ...
      // console.log("Invoice Data from snigdha gst report page", jsonData) 
            ...
            <tbody>
              {paginatedRows.map((row, idx) => (
                ...
                >
                  ...
                  <td className="px-4 py-2 border border-blue-200 align-top text-right">{row.invoiceValue}</td>
