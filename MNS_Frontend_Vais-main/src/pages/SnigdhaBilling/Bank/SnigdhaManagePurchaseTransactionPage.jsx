...
const SnigdhaManagePurchaseTransactionPage = () => {
  ...
  const fetchPurchases = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${backendDomainS}/api/v1/purchase-window/all`);
  ...
  const fetchInvoices = async () => {
    try {
      const response = await axios.get(`${backendDomainS}/api/v1/purchase-order/all`);
      if (response.data?.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modalMode === 'create') {
        // Create new purchase
        response = await axios.post(`${backendDomainS}/api/v1/purchase-window/create`, formData);
        if (response.data?.success) {
