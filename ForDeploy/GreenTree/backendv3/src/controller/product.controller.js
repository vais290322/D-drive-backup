...
exports.getArriveProducts = async (req, res) => {
  try {
    // Fetch latest 10 products (customize limit as needed)
    const products = await Product.find()
      .sort({ createdAt: -1 })
      .limit(10);

    res.status(200).json({
      message: "Arrive products fetched successfully",
      data: products,
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
...
exports.getTopDealProducts = async (req, res) => {
  try {
    // Fetch all products with valid pricing
    const products = await Product.find({
      costprice: { $gt: 0 },
      sellingprice: { $gt: 0 },
    });
    ...
    const deals = products
      ...
      .filter((p) => p.discount > 0); // Only include discounted items

    // Sort by discount percentage descending
    deals.sort((a, b) => b.discount - a.discount);

    // You can limit results if needed (e.g., top 10)
    // const topDeals = deals.slice(0, 10);

    res.status(200).json({
      message: "Top deals fetched successfully",
      data: deals, // or topDeals if you want to limit
    });
  } catch (error) {
    res.status(500).json({
