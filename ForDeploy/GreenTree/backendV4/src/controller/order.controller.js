...
exports.createOrder = async (req, res) => {
  try {
    ...
    if (
      !populatedUser?.city ||
      !populatedUser?.state ||
      !populatedUser?.user?.phone ||
      !populatedUser?.user?.email ||
      products?.length === 0
    ) {
      return res.status(400).json({
        message: "Required address or product details missing for Shiprocket.",
      });
    }
    ...
    const shiprocketOrderPayload = {
      ...
      pickup_location: "Raipur,Chakla,Deganga,West Bengal-743424",
      ...
      // If shipping = billing → Shiprocket requires "yes" or 1
      ...
      shipping_charges: populatedUser?.shippingPrice || 0,
      giftwrap_charges: 0,
      transaction_charges: 0,
      total_discount: 0,
    
      sub_total: Number(populatedUser?.paidAmount) || 0,
    
      // Package Dimensions
      length: Number(products?.[0]?.product?.length) || 10,
      breadth: Number(products?.[0]?.product?.width) || 10,
      height: Number(products?.[0]?.product?.height) || 10,
      weight: Number(products?.[0]?.product?.weight) || 1,
    ...
    const valuedata = {
      ...
      message:"202096" ,
      sender_id:"GTNSRY",
      variables_values: `${populatedUser._id}|${shiprocketResponse.data.awb_code}|${next7}`,
      flash: 0,
      numbers:populatedUser?.user?.phone.toString(), // Ensure this is a string
    };
