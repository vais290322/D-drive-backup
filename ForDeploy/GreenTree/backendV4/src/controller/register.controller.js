...
exports.registerUser = async (req, res) => {
  try {
    ...
    // if (  !name || !email  || !address || !city || !state || !postalCode || !street || !district || !landmark || !gender )
...
exports.getCustomerWithAddress = async (req, res) => {
  try {
    let { page = 1, limit = 5, search = "" } = req.query;
    page = parseInt(page, 10);
    limit = parseInt(limit, 10);
    ...
    // Total users count
    const total = await Register.countDocuments(matchQuery);

    // Aggregation to fetch user + address
    const customers = await Register.aggregate([
      { $match: matchQuery },
      ...
      // Lookup address collection
      {
        $lookup: {
          from: "addresses",              // collection name (lowercase plural)
          localField: "_id",
          foreignField: "user",
          as: "addresses",
        }
    ...
    res.status(200).json({
      message: "Customer with address fetched successfully",
      pagination: {
        totalItems: total,
        totalPages: Math.ceil(total / limit),
        currentPage: page,
...
exports.getUser = async (req, res) => {
  try {
    ...
    console.log(id);
    const customer = await Register.findOne({ _id: id })
      .sort({ createdAt: -1 })
      .select("-otp -role");
    res
      .status(200)
...
exports.updateUser = async (req, res) => {
  try {
    const id = req.user.id;
    const { name,email,  dateOfBirth, anniversaryDate, gender  } =
      req.body;
      // if (  !name || !email  || !address || !city || !state || !postalCode || !street || !district || !landmark || !gender )
      //   return res.status(400).json({ message: "All fields are required" });
    ...
    const user = await Register.findOneAndUpdate(
      {_id: id },
      { name, email, dateOfBirth, anniversaryDate, gender  },
...
exports.otp = async (req, res) => {
  ...
  try {
    // Find or create OTP record for this phone
    let otpRecord = await Register.findOne({ phone });
    console.log(otpRecord);
    if (!otpRecord) {
      otpRecord = new Register({
        phone,
        otp: otpValue,
      });
    } else {
...
exports.otpverify = async (req, res) => {
  ...
  try {
    // ✅ Find OTP inside array using $in
    const otpRecord = await Register.findOne({ otp: otp });
    ...
    // ✅ Create login token
    const token = jwt.sign(
      { id: otpRecord._id, role: otpRecord.role },
      process.env.JWT_SECRET,
      { expiresIn: "7d" }
    );
    ...
    res.status(200).json({
      message: "Login successful",
      token,
      data: otpRecord.email ? true:false,
    });
