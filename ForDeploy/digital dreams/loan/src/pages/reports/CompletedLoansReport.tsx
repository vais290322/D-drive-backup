...
export default function CompletedLoansReport() {
  ...
  const loadData = async () => {
    try {
      ...
      setLoans(loansData.filter(l => l.status === "completed"));
  ...
  const handleDownload = () => {
    const csvContent = [
      ["Loan ID", "Customer Name", "Loan Amount", "Total Payable", "Tenure", "Start Date", "Completion Date"].join(","),
      ...loans.map(l => [
        l.id,
              ...
              <TableBody>
                {loans.map((loan) => (
                  <TableRow key={loan.id}>
                    ...
                    <TableCell>{getCustomerName(loan.customer_id || "")}</TableCell>
                    <TableCell>{formatCurrency(loan.principal_amount || 0)}</TableCell>
                    <TableCell className="text-success">{formatCurrency(loan.total_payable || 0)}</TableCell>
