...
export default function LoanForm() {
  ...
  const calculateEMI = (values: Partial<LoanFormData>) => {
    ...
    // Use the utility functions for accurate calculation with fees included
    const totalInterest = calculateTotalInterest(principal, rate, tenure, interestType, loanType);
    const installmentAmount = calcEMI(principal, rate, tenure, interestType, loanType, processingFee, insuranceFee);
  ...
  const onSubmit = async (data: LoanFormData) => {
    try {
      ...
      await api.loans.create(loanData, data.loan_id_type === "manual" ? data.loan_id_manual : undefined);
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Disburse a new loan to a customer</p>
        </div>
      </div>
