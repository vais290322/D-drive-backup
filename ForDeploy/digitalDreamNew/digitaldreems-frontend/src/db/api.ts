...
const AUTH_BASE = import.meta.env.VITE_API_BASE_URL?.replace('/api', '') || 'http://localhost:4000';
...
export async function getAvailableProducts(): Promise<Product[]> {
  return apiCall<Product[]>('/products?status=available');
}
...
export async function getAllPenalties(): Promise<Penalty[]> {
  return apiCall<Penalty[]>('/penalties');
}
...
export const api = {
  ...
  // Alias for EMI Schedule (backward compatibility)
  emiSchedule: {
  ...
  crm: {
    ...
    contacts: {
      ...
      create: (data: any) => apiCall('/crm/contacts', { method: 'POST', body: JSON.stringify(data) }),
      update: (id: string, data: any) => apiCall(`/crm/contacts/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
      delete: (id: string) => apiCall(`/crm/contacts/${id}`, { method: 'DELETE' }),
    },
    deals: {
      getAll: getAllDeals,
    },
