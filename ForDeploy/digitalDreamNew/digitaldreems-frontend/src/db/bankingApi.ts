...
export async function processDeposit(
    ...
    try {
        const resp = await apiCall<any>('/banking/transactions/deposit', {
            method: 'POST',
            body: JSON.stringify({
        ...
        // Normalize to TransactionResult that frontend expects
        return {
            transaction_id: resp?.transaction?.id || resp?.transaction?._id || '',
            new_balance: resp?.new_balance ?? (resp?.transaction?.account?.balance ?? 0),
...
export async function processWithdrawal(
    ...
    try {
        const resp = await apiCall<any>('/banking/transactions/withdrawal', {
            method: 'POST',
            body: JSON.stringify({
                account_id: accountId,
                amount,
