...
export default function LoanDetail() {
  ...
  const loadLedger = async () => {
    ...
    try {
      ...
      // Customer and product are already populated by the backend
      // No need to fetch them separately
      const customer = loan.customer;
      const product = loan.product || null;
  ...
  const handlePayment = async (data: PaymentFormData) => {
    ...
    try {
      ...
      // Set payment for receipt and trigger auto-print
      setSelectedPaymentForReceipt(payment);

      // Refresh ledger after a short delay to allow receipt to render
      setTimeout(() => {
        loadLedger();

        // Auto-print receipt after DOM updates
        setTimeout(() => {
          try {
            ...
            toast.info("Payment recorded! Click 'Print Receipt' to print.");
          }
        }, 500);
      }, 100);
    } catch (error) {
