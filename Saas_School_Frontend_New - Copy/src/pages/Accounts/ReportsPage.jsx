...
const ReportsPage = () => {
  ...
  const addBankjData = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${accountApi}/account/create/${schoolId}`, input, {
        headers: {
  ...
  const fetchCashInHand = async () => {
    const response = await axios.get(`${accountApi}/income/cash-in-hand/${schoolId}`);
    if (response) {
      // Make sure we're setting a number or string, not an object
      setCashInHand(response?.data?.data || response?.data || 0);
    }
  };

  const fetchOnlineAmount = async () => {
    const response = await axios.get(`${accountApi}/income/online-payments/${schoolId}`);
    if (response) {
      // Make sure we're setting a number or string, not an object
  ...
  const fetchBankData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${accountApi}/bank-transactions/all/${schoolId}`);
      // Make sure we're setting an array, not an object
              ...
              >
                <div className="flex flex-col items-center p-4 text-wrap ">
                  ...
                  <div className="mt-4 flex items-center gap-2 text-sm text-wrap ">
                    ...
                    <span>{Number(cashInHand) > 3000 ? "Growing" : "Decreasing"}</span>
                  </div>
                </div>
              </MagicCard>
            </div>
              ...
              >
                <div className="flex flex-col items-center p-4 text-wrap ">
                  ...
                  <div className="mt-4 flex items-center gap-2 text-sm text-wrap ">
                    ...
                    <span>
                      {Number(onlineAmount) > 5000 ? "Increasing" : "Decreasing"}
                    </span>
                  </div>
                </div>
              </MagicCard>
