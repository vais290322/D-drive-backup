...
const AdmissionPage = () => {
  ...
  useEffect(() => {
    const fetchImages = async () => {
      const schoolLogo = await convertImageToBase64(instituteId[0]?.schoolLogo);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${mainUrlApi.admissioin.url}/${instituteId[0].id}`,
        formData1,
        {
          headers: {
  ...
  const fetchClsasFeeStructure = async () => {
    try {
      const response = await axios.get(
        `${accountApi}/api/fees-structure/${pdfData?.className}`
      );
