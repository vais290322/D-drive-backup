...
const MarkRegisterPage = () => {
  ...
  const searchMarkRegister = async (e) => {
    ...
    try {
      const response = await axios.get(
        `${resultUrlApi.searchStudentForMark.url}/students?className=${searchData.className}&section=${searchData.section}`
      );
      if (response) {
  ...
  const fetchExamTypeData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        `${resultUrlApi.fetchExamType.url}`
      );
