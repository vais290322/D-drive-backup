...
import { setEdpInfo, setLibrarianInfo } from "@/utils/teacher/teacherInformationSlice";
...
const ManageLibrarianPage = () => {
  ...
  const allLibrarianInfo = useSelector((state) => state.teacherInfo.librarianInfo) || [];
  ...
  const fetchEdpData = async () => {
    try {
      const response = await axios.get(`${teacherEdpLibraryanUrlApi.addLibraryan.url}`);
  ...
  //     const response = await axios.post("http://192.168.0.141:8089/api/edps", input, {
  //       headers: {
            ...
            <TableBody>
              {paginatedData.map((item, index) => (
                ...
                >
                  ...
                  >
                    {item.villPost}, {item.policeStation}, {item.district},{" "}
                    {item.city}, {item.state}, {item.pincode}, {item.pinCode} ,{item.country}
                  </TableCell>
                  <TableCell
                    className={`px-4 py-2 border text-left ${
                      theme === "light"
                  ...
                  >
                    <div className="flex justify-end items-center gap-2">
                      ...
                      <DeleteComponent
                        name={item.librarianName} // Name of the item
                        deletePath={`${teacherEdpLibraryanUrlApi.addLibraryan.url}/${item.id}`} // API endpoint for deletion
                        onDelete={() => {
                          const updatedData = allLibrarianInfo.filter(
                            (data) => data.id !== item.id
                          );
                          dispatch(setLibrarianInfo(updatedData)); // Update the Redux state after deletion
                        }}
                      />
