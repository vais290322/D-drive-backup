...
import { FiSave, FiCalendar, FiDollarSign, FiSearch, FiUsers, FiCheckCircle, FiXCircle, FiPlus, FiTrash2 } from "react-icons/fi";
...
const CreateSalaryPage = () => {
  ...
  const addNewField = (index, fieldType) => {
    ...
    if (fieldType === 'additional') {
      ...
      updatedEmployees[actualIndex].additionalFields.push({ description: '', amount: '' });
    } else if (fieldType === 'deduction') {
      if (!updatedEmployees[actualIndex].deductionFields) {
        updatedEmployees[actualIndex].deductionFields = [];
      }
      updatedEmployees[actualIndex].deductionFields.push({ description: '', amount: '' });
    }
  ...
  const removeField = (employeeIndex, fieldIndex, fieldType) => {
    ...
    if (fieldType === 'additional' && updatedEmployees[actualIndex].additionalFields) {
      updatedEmployees[actualIndex].additionalFields.splice(fieldIndex, 1);
    } else if (fieldType === 'deduction' && updatedEmployees[actualIndex].deductionFields) {
                  ...
                  <table className="w-full border-collapse text-sm">
                    <thead>
                      <tr className="bg-gradient-to-r from-gray-50 to-gray-100 text-gray-700">
                        <th className="px-4 py-3 text-left font-semibold border-b">Add Field</th>
                        <th className="px-4 py-3 text-left font-semibold border-b">Deduction Field</th>
                        ...
                        <tr key={employee.employeeCode} className="hover:bg-blue-50 transition-colors duration-150">
                          <td className="px-4 py-3">
                            <div className="flex flex-col space-y-2">
                              ...
                              {employee.additionalFields && employee.additionalFields.map((field, fieldIndex) => (
                                <div key={fieldIndex} className="bg-blue-50 p-2 rounded space-y-2">
                                  <div className="flex justify-between items-center">
                                    <span className="text-xs font-medium text-blue-700">Additional Field {fieldIndex + 1}</span>
                                    <button
                                      onClick={() => removeField(index, fieldIndex, 'additional')}
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => handleFieldChange(index, fieldIndex, 'additional', 'description', e.target.value)}
                                  ...
                                  <input
                                    ...
                                    placeholder="Amount"
                                    value={field.amount}
                                    onChange={(e) => handleFieldChange(index, fieldIndex, 'additional', 'amount', e.target.value)}
                                    className="border border-gray-300 rounded-lg p-1 w-full text-xs focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                  />
                                </div>
                              ))}
                          ...
                          <td className="px-4 py-3">
                            <div className="flex flex-col space-y-2">
                              ...
                              {employee.deductionFields && employee.deductionFields.map((field, fieldIndex) => (
                                <div key={fieldIndex} className="bg-red-50 p-2 rounded space-y-2">
                                  <div className="flex justify-between items-center">
                                    <span className="text-xs font-medium text-red-700">Deduction Field {fieldIndex + 1}</span>
                                    <button
                                      onClick={() => removeField(index, fieldIndex, 'deduction')}
                                      className="text-red-500 cursor-pointer hover:text-red-700"
                                    >
                                      <FiTrash2 size={14} />
                                    </button>
                                  ...
                                  <input
                                    ...
                                    placeholder="Description"
                                    value={field.description}
                                    onChange={(e) => handleFieldChange(index, fieldIndex, 'deduction', 'description', e.target.value)}
                                    className="border border-gray-300 rounded-lg p-1 w-full text-xs focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                  />
                                  <input
                                    type="number"
                                    placeholder="Amount"
                                    value={field.amount}
                                    onChange={(e) => handleFieldChange(index, fieldIndex, 'deduction', 'amount', e.target.value)}
                                    className="border border-gray-300 rounded-lg p-1 w-full text-xs focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                  />
                                </div>
                              ))}
