...
import { PDFDownloadButton } from "./MNSPurchasePdf";
...
export default function MNSViewPurchaseOrderPage() {
    ...
    const fetchInventoryItems = async () => {
      try {
        const response = await axios.get(
          `${backendDomainA}/api/v1/inventory/all`
        );
  ...
  const calculateGrossAmount = (item) => {
    ...
    // Store the calculated tax in both tax and taxAmount fields
    item.tax = tax.toFixed(2);
    item.taxAmount = tax.toFixed(2); // Ensure taxAmount is also set
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      const cleanedItems = items.map((item) => ({
        ...
        amount: item.grossAmount, // Assuming grossAmount is the amount field in your backend,
        grossAmount: item.grossAmount,
        taxAmount: item.taxAmount || item.tax || 0, // Use taxAmount if available, fall back to tax, or default to 0
        group: item.group,
        unit: item.unit,
      ...
      const response = await fetch(
        `${backendDomainA}/api/v1/po/update/${selectedInvoice._id}`,
        {
          method: "PUT",
          headers: {
  ...
  const confirmDelete = async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `${updateInvoice}/api/v1/po/delete/${invoiceToDelete._id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
                  ...
                  <table className="w-full mb-4">
                    <thead>
                      <tr className="bg-gray-100 text-sm">
                        <th className="p-2 text-left text-gray-600">Item id</th>
                        ...
                        <th className="p-2 text-right text-gray-600">Gross</th>
