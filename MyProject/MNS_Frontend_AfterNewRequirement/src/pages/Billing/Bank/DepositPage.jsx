...
const DepositPage = () => {
  ...
  const fetchDeposits = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${backendDomainA}/api/v1/deposit-credit/all`);
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${backendDomainA}/api/v1/deposit-credit/create`, formData);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`${backendDomainA}/api/v1/deposit-credit/update/${currentDeposit._id}`, formData);
      if (response.data && response.data.success) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`${backendDomainA}/api/v1/deposit-credit/delete/${id}`);
      if (response.data && response.data.success) {
