...
const [totalProfit, setTotalProfit]= useState("")
  ...
  // console.log("salse data : ",salsesData);

  const fetchSalesAnalysis = async ()=>{
    const response = await axios.get(`${backendDomainA}/api/v1/analytics/sales`)
  ...
  // console.log("hkjjklj : ",totalSellingAmount)
...
const fetchItemDetails = async () => {
  ...
  try {
    const response = await axios.get(
      `${backendDomainA}/api/v1/inventory/details-and-orders/${itemId.trim()}`
    );
...
const calculateItemSales = () => {
  ...
  return itemPurchaseOrders?.reduce(
    (totals, order) => {
      const itemInOrder = order.items.find((item) => item.item_id === itemId);
      if (itemInOrder) {
