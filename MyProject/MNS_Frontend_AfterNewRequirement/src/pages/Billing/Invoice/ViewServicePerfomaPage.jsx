...
import { Dialog, DialogContent, DialogTitle, IconButton, TextField, InputAdornment, Pagination, Select, MenuItem, FormControl, InputLabel } from "@mui/material";
import {
  Close as CloseIcon,
  Edit as EditIcon,
...
import MnsServicePdf from '../Reports/MnsServicePdf'

// const fetchInvoice = import.meta.env.VITE_REACT_FETCH_ALL_PRO_INVOICE_MNS;

const fetchInvoice =import.meta.env.VITE_REACT_FETCH_SERVICE_PERFOMA_INVOICE_MNS;
const updateInvoice = import.meta.env.VITE_REACT_UPDATE_SERVICE_PERFOMA_INVOICE_MNS;
const deleteInvoice = import.meta.env.VITE_REACT_DELETE_SERVICE_PERFOMA_INVOICE_MNS;



const ViewServicePerfomaPage = () => {
      ...
      if (searchTerm.trim()) {
        ...
        result = result?.filter(invoice => {
          if (searchField === "all") {
            ...
            return invoice.receiverDetails?.phoneNumber && invoice.receiverDetails.phoneNumber.toLowerCase().includes(term);
          } else if (searchField === "paymentType") {
            return invoice.paymentType && invoice.paymentType.toLowerCase().includes(term);
          } else if (searchField === "amount") {
            return invoice?.total?.grandTotal && invoice?.total?.grandTotal.toString().includes(term);
          }
          return false;
        });
      }
        ...
        try {
          ...
          const updateData = {
            ...
            total: {
              ...
              grossAmount: selectedInvoice.total?.grossAmount || selectedInvoice.taxableAmount || "0.00",
              grandTotal: selectedInvoice.total?.grandTotal || selectedInvoice.grandTotal || "0.00",
            },
            // Include any other fields that might be required by your API
          };
    
          const response = await fetch(`${updateInvoice}/${selectedInvoice._id}`, {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
            },
          ...
          toast.success(jsonData.message || "Invoice updated successfully");
          setIsEditMode(false);
          handleCloseDialog();
          getAllInvoices(); // Refresh the list after update
        } catch (error) {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-6">Service Proforma Invoice Management</h2>
      ...
      Sl.No.
        ...
        {(currentPage - 1) * itemsPerPage + index + 1}
                        ...
                        <td className="px-4 py-3 text-sm text-gray-900">
                          <div className="font-medium">{invoice.receiverDetails.name}</div>
                          <div className="text-xs text-gray-500">{invoice.receiverDetails.phoneNumber}</div>
                        </td>
                        <td className="px-4 py-3 text-sm font-medium text-gray-900">
                  ...
                  <tfoot>
                    <tr className="bg-gray-200 font-semibold">
                      ...
                      <td className="px-4 py-3 text-right text-gray-900 font-bold">
                        ₹{currentItems?.reduce((total, invoice) => total + (invoice?.total?.grandTotal || 0), 0)?.toFixed(2)}
                      </td>
                      <td colSpan={2}></td>
                    </tr>
                  </tfoot>
      ...
      >
        <DialogTitle className="flex items-center justify-between bg-gray-50 border-b">
          <span className="text-xl font-semibold">{isEditMode ? "Edit Invoice" : "Invoice Details"}</span>
          <div>
            {isEditMode && (
              <button
            ...
            <div className="p-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="bg-gray-50 p-4 rounded-lg">
                  <h3 className="font-bold mb-3 text-gray-700 border-b pb-2">Invoice Information</h3>
                  {isEditMode ? (
                    ...
                    <div className="space-y-2">
                      <p><span className="font-medium text-gray-600">Invoice Number:</span> {selectedInvoice?.invoiceNumber}</p>
                      <p><span className="font-medium text-gray-600">Date:</span> {formatDate(selectedInvoice?.date)}</p>
                      <p><span className="font-medium text-gray-600">Payment Type:</span> {selectedInvoice?.paymentType}</p>
                    </div>
                  )}
                </div>
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  {isEditMode ? (
                    ...
                    <div className="space-y-2">
                      <p><span className="font-medium text-gray-600">Name:</span> {selectedInvoice.receiverDetails?.name}</p>
                      <p><span className="font-medium text-gray-600">Phone:</span> {selectedInvoice.receiverDetails?.phoneNumber}</p>
                      <p><span className="font-medium text-gray-600">Address:</span> {selectedInvoice.receiverDetails?.address}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg mb-6">
                <h3 className="font-bold mb-3 text-gray-700 border-b pb-2">Invoice Details</h3>
                <div className="overflow-x-auto">
                  <table className="w-full mb-4">
                    <thead>
                      <tr className="bg-gray-100">
                        <th className="p-2 text-left text-gray-600">Description</th>
                        <th className="p-2 text-right text-gray-600">Number of Person</th>
                        <th className="p-2 text-right text-gray-600">Number of Duty</th>
                        <th className="p-2 text-right text-gray-600">Gross Amount</th>
                      </tr>
                    </thead>
                    <tbody>
                      {selectedInvoice.items?.map((item, index) => (
                        console.log("item from table : ", selectedInvoice),
                        <tr key={index} className="border-b hover:bg-gray-100">
                          <td className="p-2 font-medium">{item.description}</td>
                          <td className="p-2 text-right">{item.noOfPerson}</td>
                          <td className="p-2 text-right">{item.noOfDuites}</td>
                          <td className="p-2 text-right font-medium">₹{selectedInvoice?.total?.grossAmount}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="flex justify-end">
                <div className="w-64 bg-gray-50 p-4 rounded-lg">
                  <div className="flex justify-between mb-2">
                    ...
                    <span>₹{selectedInvoice?.total?.grossAmount?.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between mb-2">
                    <span className="text-gray-600">Tax : </span>
                    <span>₹{selectedInvoice?.taxAmount?.toFixed(2) || 0} </span>
                  </div>
                  <div className="flex justify-between font-bold text-lg border-t pt-2 mt-2">
                    <span>Total:</span>
                    <span>₹{selectedInvoice?.total?.grandTotal?.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      {showPdfPreview && selectedInvoice && (
        ...
        >
          ...
          <DialogContent>
            <MnsServicePdf invoiceData={selectedInvoice} header = "PROFORMA" />
          </DialogContent>
        </Dialog>
      )}
    </div>
...
export default ViewServicePerfomaPage;
