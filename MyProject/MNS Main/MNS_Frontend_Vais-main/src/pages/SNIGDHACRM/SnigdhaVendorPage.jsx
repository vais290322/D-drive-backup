...
import { FaEdit, FaTrash, FaPlus, FaSearch, FaFilter, FaFileExcel, FaUpload, FaEye, FaBuilding, FaMapMarkerAlt, FaPhone, FaEnvelope, FaIdCard , 
    FaDownload, 
...
const [excelUploading, setExcelUploading] = useState(false);
  ...
  const fetchVendors = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${backendDomainR1}/api/v1/mns/vendor/s`);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(`${backendDomainR1}/api/v1/mns/vendor/delete/${vendorToDelete.id}/s`);
      fetchVendors();
      setAlert({
        open: true,
  ...
  const handleExcelUpload = async () => {
    ...
    try {
      const response = await axios.post(`${backendDomainR1}/vendors/upload-excel/s`, formData, {
        headers: {
  ...
  const handleVendorSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        ...
        await axios.put(`${backendDomainR1}/api/v1/mns/vendor/update/${vendorForm.id}/s`, vendorForm);
        setAlert({
          open: true,
        ...
        // Create new vendor
        await axios.post(`${backendDomainR1}/api/v1/mns/vendor/create/s`, vendorForm);
        setAlert({
          open: true,
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this vendor?')) {
      try {
        await axios.delete(`${backendDomainR1}/api/v1/mns/vendor/delete/${id}/s`);
        fetchVendors();
        setAlert({
          open: true,
          message: 'Vendor deleted successfully',
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      await axios.post(`${backendDomainR1}/api/v1/mns/vendor/upload/s`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        },
  ...
  workbook.xlsx.writeBuffer().then((buffer) => {
    ...
    a.href = url;
    a.download = 'Snigdha_All_Vendors_List.xlsx';
    a.click();
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Vendor Management in Snigdha</h1>
      ...
      {toast.visible && (
        <div
          className={`fixed bottom-4 right-4 z-50 rounded-lg shadow-lg px-6 py-4 flex items-center space-x-3 ${
...
export default SnigdhaVendorPage;
