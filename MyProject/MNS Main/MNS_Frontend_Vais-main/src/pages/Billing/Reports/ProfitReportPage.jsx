...
const ProfitReportPage = () => {
  ...
  const getAllInvoices = async () => {
    try {
      setLoading(true);
      const response = await axios.get(fetchInvoice);
  ...
  const generatePDF = () => {
    ...
    try {
      ...
      doc.text(`Total Sales: ${reportSummary.totalSales.toFixed(2)}`, 60, 62);
      doc.text(`Total Cost: ${reportSummary.totalCost.toFixed(2)}`, 130, 62);
      doc.text(`Total Profit: ${reportSummary.totalProfit}`, 190, 62);
      doc.text(`Profit Margin: ${reportSummary.profitMargin.toFixed(2)}%`, 250, 62);
      ...
      if (viewMode === 'invoices') {
        ...
        // Add summary row
        const totalSales = filteredInvoices.reduce((sum, inv) => sum + inv.totalSales, 0);
        ...
        const avgMargin = totalCost ? (totalProfit / totalCost) * 100 : 0;
        ...
        // Add summary row
        const totalSales = getItemProfitData().map(item => item.sales).reduce((sum, item) => sum + item, 0);
        const totalCost = getItemProfitData().map(item => item.cost).reduce((sum, item) => sum + item, 0);
        const totalProfit = getItemProfitData().map(item => item.profit).reduce((sum, item) => sum + item, 0);
        const avgMargin = totalCost ? (totalProfit / totalCost) * 100 : 0;
    ...
    try {
      ...
      if (viewMode === 'invoices') {
        ...
        data.push({
          ...
          'Phone': '',
          'Payment Type': '',
          'Sales Amount': totalSales.toFixed(2),
          'Cost': totalCost.toFixed(2),
          'Profit': totalProfit.toFixed(2),
          'Profit Margin (%)': avgMargin.toFixed(2),
          'Items Count': ''
        });
      } else {
        // Item profit data
