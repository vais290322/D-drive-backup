...
const MasterLedgerReportPage = () => {
  ...
  const fetchReportData = async () => {
    ...
    try {
      let endpoint = `${backendDomainA}/api/v1/master-ledger/${reportType}`;
    ...
    try {
      ...
      if (activeTab === "all") {
        ...
        const rows = getFilteredData().map((item, index) => [
          ...
          // item.paymentMethod || '',
          // item.description || '',
          item.type === "Sales" || item.type === "Service"
        ...
        const totalIncome = getFilteredData()
          .filter(item => item.type === "Sales" || item.type === "Service")
      ...
      const colWidths =
        activeTab === "all"
          ? [8, 12, 15, 15, 12, 15, 30, 12, 12, 20, 12]
          : [25, 25, 12, 15, 15, 15];
      ...
      XLSX.writeFile(
        wb,
        `Master_Ledger_${activeTab}_${dayjs().format("YYYY-MM-DD")}.xlsx`
      );
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(16);
      doc.text(
        `Pending Ledger Report - ${activeTab === "all" ? "All Transactions" : "Customer Wise"
        }`,
        14,
        15
      );
      ...
      const dateRangeText =
        startDate && endDate
          ...
          : "Period: Invalid Date to Invalid Date";
      ...
      if (activeTab === "all") {
        ...
        doc.autoTable({
          ...
          theme: "plain",
          tableWidth: "auto",
          margin: { left: 10, right: 10 }, // Set explicit margins instead of using lastAutoTable
          didDrawCell: (data) => {
            // Apply green color to Income column
      ...
      // Save PDF file
      doc.save(
        `Pending_Ledger_mns_${activeTab}_${dayjs().format("YYYY-MM-DD")}.pdf`
      );
  ...
  const getFilteredData = () => {
    ...
    if (reportData.serviceLedger) {
      Object.entries(reportData.serviceLedger.customers).forEach(
        ([customerName, data]) => {
  ...
  const getCustomerWiseData = () => {
    ...
    const groupedData = filteredData.reduce((acc, item) => {
      ...
      if (item.type === "Sales" || item.type === "Service") {
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          View and analyze all pending receivables and payables
        </p>
      </div>
      ...
      {!loading && !error && reportData && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          {reportData.serviceLedger && (
            <div className="bg-white rounded-lg shadow-md overflow-hidden border-l-4 border-green-500">
              <div className="p-5">
                ...
                <p className="text-sm text-gray-600">
                  {Object.keys(reportData.serviceLedger.customers).length}{" "}
                  Customers
                </p>
              </div>
            </div>
          ...
          {reportData.summary && (
            <div className="bg-white rounded-lg shadow-md overflow-hidden border-t-4 border-purple-500 md:col-span-3">
              <div className="p-5">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                ...
                {getPaginatedData().length > 0 && (
                  <tr className="bg-gray-50 font-bold border-t-2 border-gray-300">
                    ...
                    >
                      {getFilteredData().length} items in total
                    </td>
                  </tr>
                )}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {getPaginatedCustomerWiseData().map((item, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {item.minDate.toLocaleDateString()} to{" "}
                      {item.maxDate.toLocaleDateString()}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      {item.name}
                    </td>
                  ...
                  <tr className="bg-gray-50 font-bold border-t-2 border-gray-300">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {getCustomerWiseData().reduce(
                        (total, item) => total + item.invoices.length,
                        0
                      )}{" "}
                      invoices
                    </td>
      ...
      {notification.show && (
        <div
          className={`fixed bottom-4 right-4 px-6 py-3 rounded-md shadow-lg ${notification.type === "success"
...
export default MasterLedgerReportPage;
