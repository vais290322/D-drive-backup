  ...
  return invoices.map((inv, idx) => {
    ...
    const expandedItems = items.map((item) => {
      ...
      return {
        ...
        invoiceValue: (inv.totalPayableAmount || inv.grandTotal || 0).toFixed(2),
        rate: (item.taxRate || item.taxRatePercent || 0) + "%",
        taxableValue: (item.netAmount || item.grossAmount || 0).toFixed(2),
        integratedTaxAmount: integrated.toFixed(2),
        centralTaxAmount: central.toFixed(2),
        stateTaxAmount: state.toFixed(2),
      };
    });
    ...
    return {
      ...
      gstin: inv.receiverDetails?.gstin,
      items: expandedItems,
      invoiceType: inv.paymentType,
      totalInvoiceValue: (inv?.totalPayableAmount || inv?.grandTotal || 0).toFixed(2), // Changed from inv.grandTotal to inv.totalPayableAmount
    };
  });
};
...
const GstReportPage = () => {
  ...
  const [allinvoice, setAllInvoice] = useState([]); // Using mock data for demo
  ...
  const modifiedServiceInvoice = allServiceInvoice.map((inv, idx) => {
    ...
    const hsn = "998525, 9985";
    const description = `${inv.receiverDetails?.monthYear} - ${inv.receiverDetails?.year}`;
    const uqc = null;
    const quantity = null;
    ...
    const invoiceType = inv.paymentType;
  ...
  // console.log("sorted invoice : ",sortedInvoices)
  ...
  const filteredInvoices = sortedInvoices.filter((inv) => {
    ...
    const matchesSearch =
      ...
      inv.invoiceType?.toLowerCase().includes(query) ||
      inv.items?.some(item =>
    ...
    return matchesSearch && matchesGstType && matchesDateRange && matchesInvoiceType;
  });
  ...
  // console.log("all invoice ; ",groupedRows)
  ...
  const handleExcelExport = () => {
    ...
    finalInvoices.forEach((invoice, idxInv) => {
    ...
    const invoiceValueTotal = finalInvoices.reduce((total, invoice) => {
    ...
    XLSX.writeFile(
      workbook,
      `GST_Report_for_MNS_Secure_Solutions_Private_Limited_${new Date().toISOString().split("T")[0]}.xlsx`
    );
  };
  ...
  const handlePdfExport = () => {
    ...
    doc.setFontSize(18);
    doc.text("GST Report for MNS Secure Solutions Private Limited ", 14, 18);
      ...
      >
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 print:hidden">
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
            ...
            <button className="cursor-pointer text-amber-500 hover:text-amber-700 border p-1 rounded bg-amber-200" onClick={handleResetFilter}>
              Reset Filter
            </button>
          </div>
