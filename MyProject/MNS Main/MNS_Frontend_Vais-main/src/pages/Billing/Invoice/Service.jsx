...
const InvoiceForm = ({ customer }) => {
  ...
  const handleTaxChange = (field, value) => {
    ...
    if (customer?.taxGroup === "State Tax") {
      updatedServiceData.total.cgst =
        field === "cgst" ? value : updatedServiceData.total.cgst;
      updatedServiceData.total.sgst =
        field === "sgst" ? value : updatedServiceData.total.sgst;
      updatedServiceData.total.igst = "0";
    } else if (customer?.taxGroup === "Other Tax") {
      updatedServiceData.total.igst =
        field === "igst" ? value : updatedServiceData.total.igst;
      updatedServiceData.total.cgst = "0";
      updatedServiceData.total.sgst = "0";
    } else {
      updatedServiceData.total.cgst = "0";
      updatedServiceData.total.sgst = "0";
      updatedServiceData.total.igst = "0";
  ...
  const calculateTotals = (data) => {
    ...
    // Calculate total payable amount with current round off
    calculateTotalPayableAmount(
      grandTotal.toFixed(2),
      updatedData.total.roundOff
    );
  ...
  const handleRoundOffChange = (e) => {
    // const value = parseFloat(parseFloat(e.target.value).toFixed(4)) || 0;
    const value = e.target.value || 0;
    const updatedServiceData = { ...serviceData };
    updatedServiceData.total.roundOff = value.toString();
    setServiceData(updatedServiceData);
    calculateTotalPayableAmount(updatedServiceData.total.grandTotal, value);
  };

  const calculateTotalPayableAmount = (grandTotal, roundOff) => {
    const grandTotalNum = parseFloat(grandTotal) || 0;
    const roundOffNum = parseFloat(roundOff) || 0;
    let totalPayableAmount = grandTotalNum;

    if (roundOffNum !== 0) {
      const decimalPart = (grandTotalNum % 1).toFixed(4);

      // If decimal part is greater than 0.50, add the roundOff
      // If decimal part is less than or equal to 0.50, subtract the roundOff
      if (decimalPart >= 0.5) {
        totalPayableAmount = grandTotalNum + roundOffNum;
      } else {
        totalPayableAmount = grandTotalNum - roundOffNum;
      }
    }
    ...
    const updatedServiceData = { ...serviceData };
    updatedServiceData.total.totalPayableAmount = totalPayableAmount.toFixed(2);
    setServiceData(updatedServiceData);
  };
    ...
    try {
      ...
      const payload = {
        ...
        receiverDetails: {
          ...
          pf: customer?.pf || "",
          esi: customer?.esi || "",
          formDate: customer?.formDate || "",
          toDate: customer?.toDate || "",
          id: customer?.id || "",
        ...
        <div className="flex flex-col items-end justify-end gap-2 mt-3">
          ...
          <div className="flex justify-between gap-2">
            ...
            <input
              ...
              min="0"
              className="p-1 w-[150px] border outline-none bg-gray-50"
              value={serviceData?.total?.roundOff || "0.0000"}
          ...
          <div className="flex justify-between gap-2">
            ...
            <input
              type="number"
              className="p-1 w-[150px] border outline-none bg-gray-50"
              value={serviceData?.total?.totalPayableAmount || "0.00"}
              readOnly
            />
          </div>
        </div>
