...
const ProformBill = ({ customerDetails }) => {
  ...
  // console.log("items : ", items, customerDetails.taxGroup)
  ...
  const handleSelect = (index, suggestion) => {
    ...
    if (customerDetails.taxGroup === "State Tax") {
    ...
    } else if (customerDetails.taxGroup === "No Tax" || !customerDetails.taxGroup) {
  ...
  }, [query, itemid, itemMainData, customerDetails.taxGroup]);
  ...
  const calculateItemFields = (item, sourceField) => {
    if (customerDetails.taxGroup === "No Tax" || !customerDetails.taxGroup) {
      // No tax calculation
  ...
  const handleItemChange = (index, e) => {
    ...
    if (name === "cgst" && customerDetails.taxGroup === "State Tax") {
    ...
    if (name === "sgst" && customerDetails.taxGroup === "State Tax") {
    ...
    if (name === "igst" && customerDetails.taxGroup !== "State Tax") {
    ...
    try {
      ...
      const payload = {
        ...
        items: items.map((item) => ({
          ...
          cgst: Number(parseFloat(item.cgst)),
          sgst: Number(parseFloat(item.sgst)),
          igst: Number(parseFloat(item.igst)),
          gst: Number(parseFloat(item.gst)),
          uom: item.uom || "",
        ...
        totalPayableAmount: parseFloat(totals.totalPayableAmount),
