...
const New_Url = import.meta.env.VITE_REACT_SERVICE_PERFOMA_INVOICE_MNS;



const ServiceProforma = ({ customer }) => {
  ...
  const handleItemChange = (index, field, value) => {
    ...
    if (["noOfPerson", "noOfDuites", "rate", "month"].includes(field)) {
      ...
      let amount = 0;
      if (monthDays > 0) {
        amount = ((noOfPersons * noOfDuties * ratePerMonth) / monthDays) / 2;
      } else {
  ...
  const calculateTotals = (data) => {
    ...
    calculateTotalPayableAmount(grandTotal.toFixed(2), updatedData.total.roundOff);
  ...
  const calculateTotalPayableAmount = (grandTotal, roundOff) => {
    ...
    if (roundOffNum !== 0) {
      const decimalPart = grandTotalNum % 1;
      ...
      const payload = {
        ...
        receiverDetails: { 
          ...
          address: typeof customer.address === 'object' ?
            customer.address.map(part => part.includes('/') ? `"${part}"` : part).join(', ') :
            customer.address,
        ...
        roundOff: serviceData.total.roundOff,
        totalPayableAmount: serviceData.total.totalPayableAmount,
    ...
    <div>
      <div className="bg-white">
        ...
        <div className="flex flex-col items-end justify-end gap-2 mt-3">
          ...
          {/* Add these after the Grand Total input */}
          <div className="flex justify-between gap-2">
            <label>Round Off: </label>
            <input
              type="number"
              className="p-1 w-[150px] border outline-none"
              value={serviceData.total.roundOff || "0.00"}
...
export default ServiceProforma;