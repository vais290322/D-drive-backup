...
const AddBankPage = () => {
  ...
  const handleSubmit = async () => {
    try {
      if (isEdit) {
        await axios.put(
          `${backendDomainA}/api/v1/bank/update/${currentId}`,
          formData
        );
  ...
  // Handle setting a bank as default
  const handleSetDefault = async (id) => {
    try {
      await axios.put(`${backendDomainA}/api/v1/bank/${id}/default`);
      showAlert("Default bank updated successfully", "success");
      fetchBanks(); // refresh the list
    } catch (error) {
      showAlert(
        error.response?.data?.message || "Failed to set default bank",
        "error"
      );
          ...
          <div className="flex items-center mb-4 md:mb-0">
            ...
            <div>
              ...
              <p className="text-blue-100 mt-1">
                Manage all your banking information in one place
              </p>
            </div>
          </div>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-3">
                        ...
                        <button
                          className={`flex items-center cursor-pointer justify-center h-8 w-8 rounded-full ${
                            bank.isDefault
                          ...
                          title={
                            bank.isDefault ? "Default Bank" : "Set as Default"
                          }
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            className="h-4 w-4"
                            fill={bank.isDefault ? "currentColor" : "none"}
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                          >
                            <path
