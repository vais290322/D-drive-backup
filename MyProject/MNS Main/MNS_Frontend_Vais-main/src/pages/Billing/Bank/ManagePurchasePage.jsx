...
import { FaPlus, FaEdit, FaTrash, FaSearch, FaFilter, FaFileInvoice, FaMoneyBillWave, FaStore } from 'react-icons/fa';
...
const ManagePurchasePage = () => {
  ...
  const fetchPurchases = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${backendDomainA}/api/v1/purchase-window/all`);
      console.log("all purchase : ",response)
  ...
  const fetchInvoices = async () => {
    try {
      const response = await axios.get(`${backendDomainA}/api/v1/purchase-order/all`);
      if (response.data?.success) {
  ...
  const applyFiltersAndSearch = () => {
    ...
    if (filters.minAmount) {
      filtered = filtered.filter(purchase => 
        purchase.amount >= parseFloat(filters.minAmount)
      );
    }
    
    if (filters.maxAmount) {
      filtered = filtered.filter(purchase => 
        purchase.amount <= parseFloat(filters.maxAmount)
      );
    }
    ...
    if (filters.bankId) {
      filtered = filtered.filter(purchase => 
        purchase.bankId === filters.bankId
      );
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate form
    if (!formData.invoiceNumber || !formData.invoiceId || !formData.date || 
        !formData.amount || !formData.vendorName || !formData.paymentMethod || 
    ...
    try {
      ...
      if (modalMode === 'create') {
        // Create new purchase
        response = await axios.post(`${backendDomainA}/api/v1/purchase-window/create`, formData);
        if (response.data?.success) {
        ...
        response = await axios.put(
          `${backendDomainA}/api/v1/purchase-window/update/${selectedPurchase._id}`, 
          formData
        );
        if (response.data?.success) {
      ...
      console.error(`Error ${modalMode === 'create' ? 'creating' : 'updating'} purchase:`, error);
      toast.error(`Failed to ${modalMode === 'create' ? 'create' : 'update'} purchase`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeletePurchase = async () => {
    ...
    try {
      const response = await axios.delete(
        `${backendDomainA}/api/v1/purchase-window/delete/${purchaseToDelete._id}`
      );
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Create, view, and manage purchase payments</p>
        </div>
        {/* <button
                    ...
                    <>
                      ...
                      <tr className="bg-gray-100 font-medium">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">
                          ₹{getPaginatedPurchases().reduce((sum, purchase) => sum + purchase.amount, 0).toLocaleString()}
                        </td>
                        <td colSpan="3"></td>
                      </tr>
                      ...
                      <tr className="bg-green-50 font-medium">
                        <td colSpan="3" className="px-6 py-4 whitespace-nowrap text-sm text-green-900 text-right">
                      ...
                      <tr className="bg-gray-200 font-medium">
                        <td colSpan="3" className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                          <strong>Grand Total ({filteredPurchases.length} purchases):</strong>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">
                          ₹{filteredPurchases.reduce((sum, purchase) => sum + purchase.amount, 0).toLocaleString()}
                        </td>
                        <td colSpan="3"></td>
                      </tr>
                    </>
        ...
        {!loading && purchases.length === 0 && (
          <div className="bg-white p-8 rounded-xl shadow-md text-center">
            ...
            <p className="text-gray-500 mb-6">
              Start by creating a new purchase payment record using the "New Purchase" button above.
            </p>
            <div className="bg-blue-50 p-4 rounded-lg text-sm text-blue-700 max-w-md mx-auto">
              <p className="font-medium mb-2">💡 Tip:</p>
              <p>
                Make sure you have invoices and bank accounts set up before creating purchase records.
              </p>
            </div>
          </div>
        )}
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  >
                    {modalMode === 'create' ? 'Create New Purchase' : 'Edit Purchase'}
                  </Dialog.Title>
                        ...
                        >
                          ...
                          {invoices.map(invoice => (
                            <option key={invoice._id} value={invoice._id}>
                              {invoice.invoiceNumber} - {invoice.vendorName || 'Unknown Vendor'}
                            </option>
                          ))}
                        </select>
                      </div>
