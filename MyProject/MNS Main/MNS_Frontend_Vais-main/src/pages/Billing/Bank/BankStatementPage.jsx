...
const BankStatementPage = () => {
  ...
  const fetchBankDetails = async (bankId) => {
    try {
      const response = await axios.get(`${backendDomainA}/api/v1/bank/${bankId}`);
      if (response.data?.success) {
  ...
  const transactionsWithBalance = useMemo(() => {
    ...
    return filteredTransactions.map(tx => {
      const isCredit = tx.transactionType === 'deposit' || tx.transactionType === 'transferCredit';
  ...
  const exportToPDF = () => {
    ...
    doc.text(`Bank Statement - ${selectedBank?.bankName || 'Unknown Bank'}`, 14, 22);
    ...
    const totalDebit = filteredTransactions
      .filter(t => !(t.transactionType === 'deposit' || t.transactionType === 'transferCredit'))
    ...
    // Save
    doc.save(
      `bank_statement_mns_${selectedBank?.bankName || 'unknown'}_${format(new Date(), 'yyyyMMdd')}.pdf`
    );
  };
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(
      wb,
      `bank_statement_mns_${selectedBank?.bankName || 'unknown'}_${format(new Date(), 'yyyyMMdd')}.xlsx`
    );
  };

  const calculateNetChange = (transactions) => {
    return transactions.reduce((net, transaction) => {
      if (transaction.transactionType === "deposit" || transaction.transactionType === "transferCredit") {
          ...
          <div className="px-8 py-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
              ...
              <div className="bg-white/10 rounded-lg p-4">
                ...
                <p className="text-white text-xl"> ₹ {summary?.totalExpenses || 0}</p>
              </div>
              <div className="bg-white/10 rounded-lg p-4">
                      ...
                      getPaginatedTransactions().map((transaction, index) => (
                        <tr key={index} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span
                              className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${transaction.transactionType === "deposit" ||
                                  transaction.transactionType === "transferCredit"
