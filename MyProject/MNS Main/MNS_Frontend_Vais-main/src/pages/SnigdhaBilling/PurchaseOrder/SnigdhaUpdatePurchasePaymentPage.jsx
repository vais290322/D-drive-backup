...
import PaymentAndViewVendorBlanceComponent from "../../../component/PaymentForCreditDebitNote/PaymentAndViewVendorBlanceComponent";
import { Calendar, FileText, Hash, IndianRupee } from "lucide-react";


const SnigdhaUpdatePurchasePaymentPage = () => {
  ...
  const getAllPurchaseInvoices = async () => {
    try {
      ...
      const response = await fetch(
        `${backendDomainS}/api/v1/purchase-account/all`,
        {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    ...
    if (amountRange.min) {
      result = result.filter(
        (invoice) =>
          parseFloat(invoice?.grandTotal || 0) >= parseFloat(amountRange.min)
      );
    }
    ...
    if (amountRange.max) {
      result = result.filter(
        (invoice) =>
          parseFloat(invoice?.grandTotal || 0) <= parseFloat(amountRange.max)
      );
    }
  ...
  const handleSaveEdit = async () => {
    // console.log(data)
    // Validate payment amount is not greater than due amount
    const paymentAmount = parseFloat(data.paymentAmount) || 0;
    const dueAmount = parseFloat(selectedInvoice.dueAmount.toFixed(2)) || 0;
    ...
    try {
      // Create the payload directly from the data state
      ...
      if (data.paymentMode === "Voucher" && selectedVoucher) {
        voucherDetails = [{
          ...
          vendorId: selectedInvoice?.invoiceId?.receiverDetails?.id
        }];
      }
      ...
      // console.log("Payment payload:", paymentDetails, voucherDetails);
      // return;

      const response = await fetch(
        `${backendDomainS}/api/v1/purchase-account/update/${selectedInvoice._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        throw new Error(jsonData.message || "Failed to create ledger");
      }
    ...
    setData((prev) => ({
      ...
      transactionId:
        note && note?.noteType === "credit"
          ...
          : "", // credit voucher → no auto transaction ID
    }));
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <PaymentAndViewVendorBlanceComponent
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-6">
          Update Payment For Purchase In Snigdha
        </h2>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {currentItems?.map((invoice, index) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-sm font-medium text-gray-900">
                          {invoice?.paymentDetails[0]?.paymentDate}
                        </td>
                        ...
                        <td className="px-4 py-3 text-sm text-center">
                          <div className="flex justify-end gap-2">
                            ...
                            <IconButton
                              onClick={() => handleViewVendorBalance(invoice?.invoiceId?.receiverDetails?.id)}
                              size="small"
      ...
      >
        <DialogTitle className="flex items-center justify-between bg-gray-50 border-b">
          <span className="text-xl font-semibold">
            {isEditMode ? "Update Payment" : "Payment Details"}
          </span>
          <div>
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  {isEditMode ? (
                    <div className="space-y-3">
                      ...
                      {selectedInvoice && data?.paymentMode !== "Voucher" &&
                  ...
                  {isEditMode ? (
                    <div className="space-y-2">
                      ...
                      <p>
                        ...
                        {selectedInvoice?.invoiceId?.receiverDetails?.phoneNumber}
                      </p>
                      <p>
                        <span className="font-medium text-gray-600">
                          Address:
                    ...
                    <div className="space-y-2">
                      ...
                      <p>
                        ...
                        {selectedInvoice?.invoiceId?.location}
                      </p>
                      <p>
                        <span className="font-medium text-gray-600">
                          State:
                      ...
                      <p>
                        ...
                        {selectedInvoice?.invoiceId?.receiverDetails?.gstin}
                      </p>
                    </div>
                  )}
                </div>
                          ...
                          selectedInvoice?.invoiceId?.items?.map((item, index) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="p-2 text-right">{hsnCode}</td>
...
export default SnigdhaUpdatePurchasePaymentPage;
