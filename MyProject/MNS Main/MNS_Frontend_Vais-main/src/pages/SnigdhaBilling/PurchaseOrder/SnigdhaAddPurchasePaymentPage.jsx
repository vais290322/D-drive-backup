...
const SnigdhaAddPurchasePaymentPage = () => {
  ...
  const handleEditInvoice = (invoice) => {
    ...
    setProductDetails({
      ...
      totalAmount: Number(invoice.totalPayableAmount).toFixed(2) || Number(invoice.grandTotal).toFixed(2) || "",
  ...
  const handleSaveEdit = async () => {
    ...
    const grandTotal = parseFloat(Number(selectedInvoice.totalPayableAmount).toFixed(2) || Number(selectedInvoice?.grandTotal).toFixed(2) || 0);
  ...
  const handleVoucherSelection = (note) => {
    ...
    if (note?.noteType === 'debit') {
      ...
      const maxAmount = Math.min(
        ...
        parseFloat(Number(selectedInvoice.totalPayableAmount).toFixed(2) || Number(selectedInvoice.grandTotal).toFixed(2) || 0) - parseFloat(productDetails.paymentDetails[0].paymentAmount || 0)
      );
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {currentItems?.map((invoice, index) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-sm font-medium text-gray-900">
                          ₹{Number(invoice?.totalPayableAmount || invoice?.grandTotal).toFixed(2)}
                        </td>
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        {selectedInvoice && productDetails?.paymentDetails?.[0]?.paymentMode !== "Voucher" && 
                          ...
                          ) > parseFloat(Number(selectedInvoice.totalPayableAmount)?.toFixed(2) || Number(selectedInvoice.grandTotal)?.toFixed(2) || 0) && (
              ...
              <div className="flex justify-end">
                <div className="w-64 bg-gray-50 p-4 rounded-lg">
                  <div className="flex justify-between mb-2">
                    ...
                    <span>₹{Number(selectedInvoice.taxableAmount)?.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between mb-2">
                  ...
                  <div className="flex justify-between font-bold text-lg border-t pt-2 mt-2">
                    <span>Total:</span>
                    <span>₹{Number(selectedInvoice.totalPayableAmount || selectedInvoice.grandTotal)?.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
