...
import { DownloadIcon, InfoIcon, ShieldCloseIcon } from "lucide-react";
...
const SnigdhaInventoryManagementPage = () => {
  ...
  const downloadAllItemsAsExcel = () => {
    ...
    worksheet.columns = [
      ...
      { header: "Unit Price", key: "unit_prize", width: 15 },
      { header: "Selling Price", key: "sellingPrice", width: 15 },
      { header: "Closing Quantity", key: "quantity", width: 20 },
      { header: "Total Value", key: "total_prize", width: 18 },
      ...
      { header: 'Total Damage', key: 'totalDamage', width: 20 },
      { header: 'Total Damage Amount', key: 'totalDamageAmount', width: 20 },
      // { header: 'Seller Email', key: 'seller_email', width: 20 },
      // { header: 'Seller Address', key: 'seller_address', width: 25 },
    ];
    ...
    currentInventory.forEach((item, idx) => {
      worksheet.addRow({
        ...
        unit_prize: Number(item.unit_prize).toFixed(2) || "",
        sellingPrice: Number(item.sellingPrice).toFixed(2) || "",
        quantity: Number(item.quantity).toFixed(2) || "",
        total_prize: Number(item.total_prize).toFixed(2) || "",
        ...
        totalDamage: item.totalDamageQty || '',
        totalDamageAmount: Number(item.totalDamageAmount).toFixed(2) || '',
        // seller_email: item.seller_details?.email || '',
        // seller_address: item.seller_details?.address || '',
      });
    });
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      a.href = url;
      a.download = "Snigdha_All_Inventory_Items.xlsx";
      a.click();
  ...
  return (
    <Box sx={{ p: { xs: 1, sm: 2, md: 3 } }}>
        ...
        >
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <Button
              ...
              onClick={() => downloadAllItemsAsExcel()}
            >
              {/* <DownloadIcon /> */}
              Download All Items
            </Button>
                  ...
                  >
                    ...
                    >
                      <Typography variant="body2" color="info.dark">
                        Enter the Item ID of an existing item in the holding
                        inventory to add it to the current inventory.
                      </Typography>
                    </Box>
                  </Paper>
                </Grid>
                    ...
                    >
                      ...
                      >
                        <Typography variant="body2" color="warning.dark">
                          This action will add all items from the holding
                          inventory to the current inventory. Use with caution.
                        </Typography>
                      </Box>
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                      ...
                      <Box>
                        ...
                        <Typography variant="body2" color="success.dark">
                          • Use the "Add Single Item" option to add specific
                          items from holding inventory
                          <br />
                          • The "Add All Items" option will transfer all items
                          from holding to current inventory
                          <br />
                          • You can view all current inventory items in the
                          "Current Inventory" tab
                          <br />• To sell an item, go to the "Current Inventory"
                          tab and use the sell form
                        </Typography>
                      </Box>
                    </Box>
                  </Paper>
              ...
              <Grid item xs={12} md={6}>
                ...
                >
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={6}>
                      <Tooltip title="View damage breakdown">
                        <Box onMouseEnter={() => setDamageOpen(true)} sx={{ cursor: 'pointer' }}>
                          <Typography variant="body2" color="text.secondary">
                            Total Damage
                          </Typography>
                          <Typography variant="h6" fontWeight="bold">
                            {selectedItem.totalDamageQty || "0"}
                          </Typography>
                        </Box>
                      </Tooltip>
                    </Grid>
                    <Grid item xs={6}>
                      <Box>
                        ...
                        <Typography variant="h6" fontWeight="bold">
                          {selectedItem.quantity - selectedItem.totalDamageQty || "0"}
                        </Typography>
                      </Box>
                    </Grid>
                    ...
                    <Grid item xs={6}>
                      <Tooltip title="View damage breakdown">
                        <Box onMouseEnter={() => setDamageOpen(true)} sx={{ cursor: 'pointer' }}>
                          ...
                          <Typography variant="h6" fontWeight="bold">
                            {selectedItem.totalDamageAmount || "0.00"}
                          </Typography>
                        </Box>
                      </Tooltip>
                    </Grid>
                    ...
                    <Grid container spacing={2}>
                      <Grid item xs={12} sm={6}>
                        ...
                        >
                          ...
                          >
                            {selectedItem.seller_details.seller_name?.charAt(
                              0
                            ) || "S"}
                          </Avatar>
                          <Box>
      ...
      <Dialog open={damageOpen} onClose={() => setDamageOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Damage Breakdown</DialogTitle>
        <DialogContent dividers>
          <Table size="small">
            <TableBody>
              {(selectedItem?.damage || []).map((d, i) => (
                <TableRow key={i}>
                  <TableCell>{d.grnNumber}</TableCell>
                  <TableCell align="right">{d.damageQty}</TableCell>
                  <TableCell align="right">₹ {d.damageAmount}</TableCell>
                </TableRow>
              ))}
              <TableRow>
                <TableCell>Total</TableCell>
                <TableCell align="right">{selectedItem?.totalDamageQty || 0}</TableCell>
                <TableCell align="right">₹ {selectedItem?.totalDamageAmount || 0}</TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDamageOpen(false)}>Close</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
