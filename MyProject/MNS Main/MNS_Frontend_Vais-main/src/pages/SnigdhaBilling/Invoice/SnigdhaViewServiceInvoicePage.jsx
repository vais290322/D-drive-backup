...
import ServicePdfFormate from "./ServicePdfPage";



import { backendDomainR1, backendDomainS } from "../../../Common/index";

const SnigdhaViewServiceInvoicePage = () => {
  ...
  const customerOptions = allCustomerData.map((customer) => ({
    label: customer.companyName || customer.name,
    value: customer.companyName || customer.name || "",
    id: customer.id || "",
    location: customer.location || "",
    state: customer.state || "",
    phoneNumber: customer.companyNumber || customer.alternateNumber || "",
  }));
  ...
  const calculateGrossAmount = (item) => {
    ...
    const netAmount = Number(parseFloat(quantity * sellingPrice).toFixed(2));
  ...
  const calculateTotals = (items) => {
    const taxableAmount =
      Number(
        items.reduce((total, item) => total + Number(item.netAmount), 0)
      ) || 0;
    const taxAmount =
      Number(
    ...
    const totalPayableAmount =
      ...
      0 + Number(selectedInvoice.transportationCharges) ||
      0 + Number(selectedInvoice.roundOff) ||
      0;
  ...
  const handleDelete = async (invoice) => {
    if (window.confirm("Are you sure you want to delete this invoice?")) {
      try {
        const response = await fetch(
          `${backendDomainS}/api/v1/service-invoice/${invoice._id}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const response = await fetch(
        `${backendDomainS}/api/v1/service-invoice/${selectedInvoice._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-6">
          View Service Invoice Management
        </h2>
                  ...
                  {selectedInvoice && isEditMode ? (
                    <div className="space-y-3">
                      <div>
                        ...
                        {/* <ReactSelect
                          name="name"
                          value={customerOptions.find(option => option.value === selectedInvoice?.receiverDetails?.name) || null}

                          onChange={(selectedOption) => {
                            setSelectedInvoice({
                              ...selectedInvoice,
                              receiverDetails: selectedOption ? { ...selectedInvoice.receiverDetails, name: selectedOption.value, id: selectedOption.id  } : "",
                            });
                          }}
                            ...
                            >
                              <td className="p-2 min-w-58">
                                <input
                                  ...
                                  className="w-full border rounded min-w-58 px-2 py-1"
                                />
                              </td>
                              <td className="p-2">
                                <input
                      ...
                      <div className="flex justify-between mb-2">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            const roudOff = selectedInvoice.roundOff || 0;
                      ...
                      <div className="flex justify-between mb-2">
                        ...
                        <span>
                          ...
                          {(
                            ...
                            (Number(selectedInvoice.transportationCharges) || 0)
                          ).toFixed(2)}
                        </span>
                      </div>
                      <div className="flex justify-between mb-2">
                            ...
                            >
                              <td className="p-2">
                                <input
                                  type="text"
                                  value={item.description}
                                  className="w-full focus:outline-none rounded min-w-58"
                                  readOnly
                                />
                              </td>
                              <td className="p-2">
                              ...
                              <td className="p-2">
                                <input
                                  type="number"
                                  value={item.quantity}
                                  className="w-full focus:outline-none rounded text-right"
                                  readOnly
                                />
                              </td>
                              <td className="">
                  ...
                  <div className="flex justify-end">
                    <div className="w-64 bg-gray-50 p-4 rounded-lg">
                      ...
                      <div className="flex justify-between mb-2">
                        ...
                        <span>₹{selectedInvoice.transportationCharges}</span>
                      </div>
                      <div className="flex justify-between mb-2">
                        <span className="text-gray-600">Total:</span>
                        <span>
                      ...
                      <div className="flex justify-between mb-2">
                        <span className="text-gray-600">RoundOff:</span>
                        <span>₹{selectedInvoice.roundOff}</span>
                      </div>
                      <div className="flex justify-between font-bold text-lg border-t pt-2 mt-2">
                        <span>Total Payable :</span>
                        <span>
      ...
      {showPdfPreview && selectedInvoice && (
        ...
        >
          ...
          <DialogContent>
            <ServicePdfFormate serviceinvoiceData={selectedInvoice} />
          </DialogContent>
        </Dialog>
      )}
    </div>
...
export default SnigdhaViewServiceInvoicePage;
