...
const SnigdhaGstReportPage = () => {
  ...
  const filteredData = allinvoice?.filter((item) => {
    // Check if search query is for HSN code (exact match)
    const hasExactMatchingHsnCode = item.items?.some(itemData => 
      (itemData.hsnCode || "") === searchQuery
    );

    // Check if search query is for invoice number, customer name, or GSTIN (partial match)
    ...
    const matchesInvoiceType =
      filterInvoiceType === "all" || item?.paymentType === filterInvoiceType;
    ...
    return (basicSearchMatch || hasExactMatchingHsnCode) && matchesGstType && matchesInvoiceType && matchesDateRange;
  });
...
const handlePdfExport = () => {
  ...
  doc.setFontSize(18);
  doc.text("HSN and GST Wise Report", doc.internal.pageSize.width / 2, 15, { align: "center" });
