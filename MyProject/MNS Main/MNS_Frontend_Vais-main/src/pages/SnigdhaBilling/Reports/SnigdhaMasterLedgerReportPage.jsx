...
const SnigdhaMasterLedgerReportPage = () => {
    ...
    try {
      ...
      if (activeTab === "all") {
        ...
        const rows = getFilteredData().map((item, index) => [
          ...
          item.type === "Service" ? formatCurrency(item.amount || 0) : "",
          item.type === "Purchase" ? formatCurrency(item.amount || 0) : "",
          item.personName || item.customerName || item.vendorName || "",
        ]);

        // Calculate totals
        const totalIncome = getFilteredData()
          .filter(item => item.type === "Sales")
          .reduce((sum, item) => sum + (item.amount || 0), 0);
        const totalServiceIncome = getFilteredData()
          .filter(item => item.type === "Service")
          .reduce((sum, item) => sum + (item.amount || 0), 0);
        const totalExpense = getFilteredData()
          .filter(item => item.type === "Purchase")
          .reduce((sum, item) => sum + (item.amount || 0), 0);

        // Add total row at the end of rows
        rows.push([
          "", "", "", "TOTAL",
        ...
        const rows = getCustomerWiseData().map((item) => [
          ...
          item.name || "",
          item.finalType === "sales" ? "Sales" : item.finalType === "service" ? "Service" : "Purchase",
          item.invoices.length,
          item.finalType === "sales"
            ? `+${formatCurrency(item.income || 0).replace("₹", "")}`
            : "",
          item.finalType === "service" ? `+${formatCurrency(item.serviceIncome || 0).replace("₹", "")}` : "",
        ...
        rows.push([
          ...
          "Summary (Total)",
          "",
          `${totals.totalInvoices} invoices`,
          `+${formatCurrency(totals.totalIncome).replace("₹", "")}`,
          `+${formatCurrency(totals.totalServiceIncome).replace("₹", "")}`,
          `-${formatCurrency(totals.totalExpense).replace("₹", "")}`,
        ]);
      ...
      const colWidths =
        activeTab === "all"
          ? [8, 12, 15, 15, 12, 12, 15, 30]
          : [25, 25, 12, 15, 15, 15, 15,];
      ...
      XLSX.writeFile(
        wb,
        `Snigdha_Pending_Ledger_${activeTab}_${dayjs().format(
          "YYYY-MM-DD"
        )}.xlsx`
      );
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(16);
      doc.text(
        `Snigdha Pending Ledger Report - ${activeTab === "all" ? "All Transactions" : "Customer Wise"
        }`,
        14,
        15
      );
      ...
      if (activeTab === "all") {
        ...
        const rows = getFilteredData().map((item, index) => [
          ...
          item.invoiceNumber || "",
          item.type || "",
          item.type === "Sales" ? item.amount.toFixed(2) || 0 : "",
          item.type === "Service" ? item.amount.toFixed(2) || 0 : "",
          item.type === "Purchase" ? item.amount.toFixed(2) || 0 : "",
          item.personName || item.customerName || item.vendorName || "",
        ]);
        ...
        const rows = customerWiseData.map((item) => {
          ...
          const income =
            item.finalType === "sales"
              ? item.income.toFixed(2) || 0
              : "";
          const expense =
            item.finalType === "purchase"
              ? item.expense.toFixed(2) || 0
              : "";
          ...
          return [
            ...
            item.invoices.length,
            income,
            item.finalType === "service" ? item.serviceIncome.toFixed(2) || 0 : "-",
            expense,
          ];
        });
        ...
        doc.autoTable({
          body: [
            [
              ...
              `${totalIncome.toFixed(2)}`,
              `${totals.totalServiceIncome.toFixed(2)}`,
              `${totalExpense.toFixed(2)}`,
            ],
          ],
      ...
      // Save PDF file
      doc.save(
        `Snigdha_Pending_Ledger_${activeTab}_${dayjs().format("YYYY-MM-DD")}.pdf`
      );
  ...
  const getCustomerWiseData = () => {
    ...
    const groupedData = filteredData.reduce((acc, item) => {
      ...
      if (item.type === "Sales") {
        ...
        acc[name].serviceIncome += item.amount || 0;
        acc[name].finalType = "service";
      }
  ...
  const calculateTotals = () => {
    const data = getCustomerWiseData();
    return {
      totalIncome: data.reduce((sum, item) => sum + item.income, 0),
      totalServiceIncome: data.reduce((sum, item) => sum + item.serviceIncome, 0),
      totalExpense: data.reduce((sum, item) => sum + item.expense, 0),
      totalInvoices: data.reduce((sum, item) => sum + item.invoices.length, 0)
    };
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            >
              ...
              <option value="services">Services Ledger</option>
        ...
        <div className="flex flex-col md:flex-row justify-between space-y-4 md:space-y-0">
          ...
          <div className="flex space-x-3">
            <button
              ...
              className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed flex items-center cursor-pointer"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
            ...
            <button
              onClick={exportToPDF}
              disabled={loading || !reportData}
              className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed flex items-center cursor-pointer"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-5 w-5 mr-2"
          ...
          {reportData.serviceLedger && (
            <div className="bg-white rounded-lg shadow-md overflow-hidden border-l-4 border-green-500">
              <div className="p-5">
                ...
                <p className="text-2xl font-bold mb-1">
                  {formatCurrency(reportData.serviceLedger.totalPending)}
                </p>
                <p className="text-sm text-gray-600">
                  {Object.keys(reportData.serviceLedger.customers).length} Customers
                </p>
              </div>
            </div>
          )}
              ...
              <div className="p-5">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <p
                      className={`text-xl font-bold ${reportData.summary.netPosition >= 0
                        ? "text-green-600"
                        : "text-red-600"
                        }`}
                    >
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                  ...
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                         ${item.finalType === "sales"
                            ? "bg-blue-100 text-blue-800"
                            : item.finalType === "service"  // Add service condition
                              ? "bg-green-100 text-green-800"
                              : "bg-orange-100 text-orange-800"
                          }`}
                      >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-green-600 font-medium">
                      {item.finalType === "sales"
                        ...
                        : item.finalType === "service"
                          ? formatCurrency(item.serviceIncome || 0)
                          : "-"}
                    </td>
                ...
                <tr className="bg-gray-50 font-medium">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-green-600 font-medium">
                    ...
                    {formatCurrency(
                      getCustomerWiseData().reduce((sum, item) => {
                        if (item.finalType === "sales") return sum + (item.income || 0);
                        if (item.finalType === "service") return sum + (item.serviceIncome || 0);
                        return sum;
                      }, 0)
                    )}
                  </td>
