...
const SnigdhaPurchaseReportPage = () => {
  ...
  const generatePDF1 = () => {
    ...
    try {
      ...
      // Add title
      doc.setFontSize(18);
      doc.text('Purchase Report For Snigdha Enterprice', 14, 22);
        ...
        doc.autoTable({
          ...
          didDrawPage: function (data) {
            ...
            if (data.pageNumber > 1) {
              doc.setFontSize(16);
              doc.text("Purchase Report For Snigdha Enterprise", 14, 20);
              doc.setFontSize(10);
    ...
    try {
      if (viewMode === 'orders') {
        ...
        if (viewMode === 'orders') {
          ...
          XLSX.writeFile(
            workbook,
            `Snigdha_Purchase_Report_${new Date().toISOString().split("T")[0]}.xlsx`
          );
                    ...
                    {selectedOrder.items && Array.isArray(selectedOrder.items) ? (
                      selectedOrder.items.map((item, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell>{item.uom || 0}</TableCell>
                          <TableCell>₹{parseFloat(item.unitPrice || 0).toFixed(2)}</TableCell>
                          <TableCell>{selectedOrder.taxGroup ===
                            "State Tax" ? item?.cgst || (item?.taxRate>0 && (item.taxRate/2)) || 0 :0 || 0} %</TableCell>
                          <TableCell>{selectedOrder.taxGroup ===
                            "State Tax" ? item?.sgst || (item?.taxRate>0 && (item.taxRate/2)) || 0 :0 || 0} %</TableCell>
                          <TableCell>{selectedOrder.taxGroup ===
                            "Other Tax" ? item?.igst || (item?.taxRate>0 && (item.taxRate)) || 0 :0 || 0} %</TableCell>
