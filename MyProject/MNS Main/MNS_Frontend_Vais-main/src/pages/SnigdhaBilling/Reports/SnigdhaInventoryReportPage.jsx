...
import SnigdhaItemsReportComponent from "../../../component/Reports/SnigdhaItemsReportComponent";
...
const SnigdhaInventoryReportPage = () => {
  ...
  const [isItemReportOpen, setIsItemRportOpen]=useState(false)
  ...
  const fetchCurrentInventory = async () => {
    try {
      ...
      // console.log("all inventory items : ",response?.data?.data)
  ...
  const filteredData = currentInventory?.filter((item) => {
    ...
    const matchesItemId = selectedItemId.length === 0 || selectedItemId.some(option => option.value === item?.item_id);
    
    // Add group filtering
    const matchesGroup = selectedGroup.length === 0 || selectedGroup.some(option => option.value === item?.group);
  ...
  const handleExcelExport = () => {
    ...
    const exportData = filteredData.map((item) => {
      const totalValue = Number(item?.quantity || 0) * Number(item?.unit_prize || 0);
      const status =
        (item?.quantity || 0) > 10
          ? "In Stock"
          : (item?.quantity || 0) > 0
      ...
      return {
        ...
        "Issued": item?.totalSales || 0,
        "Closing Quantity": item?.quantity || 0,
        "Unit Price": Number(item?.unit_prize).toFixed(2) || 0,
        "Total Value": totalValue.toFixed(2),
        "Status": status,
        "Damage Qty": item?.totalDamageQty || 0,
        "Damage Amount": item?.totalDamageAmount || 0,
        "GST": item?.gst || 0,
      };
    });
  ...
  const handlePdfExport = () => {
    ...
    filteredData.forEach((item) => {
      ...
      const rowData = [
        ...
        item?.totalSales || 0,
        item?.quantity || 0,
        Number(item?.unit_prize).toFixed(2) || 0,
    ...
    <div className="container mx-auto p-4 bg-gray-50 min-h-screen">
      ...
      >
        <h1 className="text-2xl font-bold text-gray-800 mb-6">
          Inventory Report for snigdha
        </h1>

        <div className="flex justify-center gap-4">
          <button onClick={()=>setIsItemRportOpen(false)} className="bg-green-400 font-sans cursor-pointer p-2 rounded hover:bg-green-600 ">Inventory Report</button>
          <button onClick={()=>setIsItemRportOpen(true)} className="bg-blue-400 font-serif cursor-pointer p-2 rounded hover:bg-blue-600 ">Item Report</button>
        </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-6 border border-gray-200 print:hidden">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <Select
                  isMulti
                  options={uniqueItemIds.filter(id => id).map(id => ({ value: id, label: id }))}
                  value={selectedItemId}
                  onChange={(selectedOptions) => setSelectedItemId(selectedOptions || [])}
              ...
              <div>
                ...
                <Select
                  isMulti
                  options={uniqueGroups.filter(group => group).map(group => ({ value: group, label: group }))}
                  value={selectedGroup}
        ...
        {(selectedItemId.length > 0 || selectedGroup.length > 0 || stockFilter !== "all" || searchQuery) && (
          <div className="flex flex-wrap gap-2 mb-4 print:hidden">
            <span className="text-sm text-gray-600">Active Filters:</span>
            {selectedItemId.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {selectedItemId.map((option, index) => (
                  <span key={index} className="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded flex items-center">
                    ...
                    <button 
                      onClick={() => setSelectedItemId(prev => prev.filter(item => item.value !== option.value))} 
            ...
            {selectedGroup.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {selectedGroup.map((option, index) => (
                  <span key={index} className="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded flex items-center">
                    Group: {option.label}
                    <button 
                      onClick={() => setSelectedGroup(prev => prev.filter(item => item.value !== option.value))} 
                      className="ml-1 text-blue-800 hover:text-blue-900"
                    >
                      ×
                    </button>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mt-6 print:hidden">
          <div className="flex items-center mb-4 sm:mb-0">
            ...
            <select
              ...
              className="border rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              {[5, 10, 25, 50, 100,200,300,500,1000].map((size) => (
