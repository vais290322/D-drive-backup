...
import SnigdhaAddCustomer from "./SnigdhaAddCustomer";
...
const SnigdhaTableComponent = () => {
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      const response = await fetch(
        `${backendDomainR1}/api/v1/mns/crm/upload/s`,
        {
          method: "POST",
          body: formData,
        }
  ...
  const getAllCustomerData = async () => {
    try {
      const customerData = await fetch(`${getAllCustomerUrl}/s`, {
        method: "GET",
  ...
  const handelSave = async (customer) => {
    ...
    try {
      const addCustomer = await fetch(`${createCustomerUrl}/s`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handelUpdate = async (updatedCustomer) => {
    try {
      const updateData = await fetch(
        `${updateCustomerUrl}/${updatedCustomer.id}/s`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const deleteCustomer = await fetch(`${deleteCustomerUrl}/${id}/s`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      });
  ...
  workbook.xlsx.writeBuffer().then((buffer) => {
    ...
    a.href = url;
    a.download = 'Snigdha_All_Customers_List.xlsx';
    a.click();
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-xl shadow-md p-6 mb-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-6 pb-3">
          Customer Management In Snigdha
        </h1>
...
export default SnigdhaTableComponent;
