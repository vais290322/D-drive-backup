...
const MnsPaymentAndViewVendorBlanceComponent = ({ vendorId, open, onClose }) => {

    const [customerBalance, setCustomerBalance] = useState(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    // console.log('Customer Balance Component:', { vendorId, open, onClose });
    ...
    const fetchVendorBalance = async () => {
        try {
          setLoading(true);
          setError(null);
          const response = await axios.get(`${backendDomainA}/api/v1/credit-debit-notes/mns-vendor-blance/${vendorId}`);
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
          <DialogTitle className="flex justify-between items-center bg-indigo-50">
            <span className="text-xl font-semibold">Vendor Balance Details</span>
          ...
          <DialogContent dividers>
            {loading ? (
              <div className="flex justify-center items-center py-10">
                <CircularProgress />
                <span className="ml-3">Loading Vendor balance...</span>
              </div>
            ) : error ? (
              ...
              <div className="p-4">
                ...
                <div className="bg-blue-50 p-4 rounded-lg mb-6">
                  <h3 className="text-lg font-semibold text-blue-800 mb-2">Vendor Information</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <p><span className="font-medium">Vendor Name:</span> {customerBalance.vendorName || 'N/A'}</p>
                    <p><span className="font-medium">Vendor ID:</span> {customerBalance.vendorId || 'N/A'}</p>
              ...
              <div className="text-center py-10">
                <p className="text-gray-500">No Vendor balance data available</p>
              </div>
            )}
          </DialogContent>
        </Dialog>
...
export default MnsPaymentAndViewVendorBlanceComponent
