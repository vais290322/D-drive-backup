...
const SnigdhaItemsReportComponent = () => {
  // State variables
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${backendDomainS}/api/v1/item/itemsreport`, { params });
  ...
  const downloadExcel = () => {
    ...
    itemsToExport.forEach((item, itemIndex) => {
      ...
      itemCell.value = `${item.itemName} (${item.item_id})`;
      ...
      const totalPurchaseQty1 = item.report.reduce((sum, t) => sum + t.purchaseQuantity, 0);
  ...
  const downloadPDF = () => {
    try {
      ...
      itemsToExport.forEach((item, itemIndex) => {
        ...
        doc.autoTable({
          body: [
            [{ content: `${item.itemName} (${item.item_id})`, styles: { fontStyle: 'bold' } }]
          ],
          startY: yPos,
          theme: 'plain',
          styles: { cellPadding: 1 },
        ...
        doc.autoTable({
          body: [
            [
              { content: `Avg Rate: ${item?.unitPrice}`, styles: { fontSize: 8 } },
          ...
          {currentItems.map((item, index) => (
            <div key={item.item_id} className="mb-8 border rounded-lg overflow-hidden">
              <div className="bg-gray-200 p-3 flex justify-between items-center">
                <h2 className="text-lg font-semibold">{item.itemName} <span className="text-sm text-gray-600">({item.item_id})</span></h2>
...
export default SnigdhaItemsReportComponent;