...
export const getSalesReport = async (req, res) => {
  try {
    ...
    const totalPaid = invoices.reduce(
      (sum, invoice) => sum + (invoice.isPaid ? invoice.grandTotal : 0),
      0
    );
...
export const getPurchaseReport = async (req, res) => {
  try {
    ...
    // Get purchase orders
    const purchaseOrders = await SnigdhaPurchaseOrderNew.find(query).sort({
      date: -1,
    });
    ...
    const totalPurchases = purchaseOrders.reduce((sum, po) => {
      return (
        sum +
        (po.totalAmount ||
          po.items.reduce((itemSum, item) => itemSum + item.amount, 0))
      );
    }, 0);
...
export const getFinancialReport = async (req, res) => {
  try {
      ...
      financialData.details.expenses = {
        ...
        general:
          generalExpenses.length > 0
            ...
            : {total: 0, count: 0},
        withdrawals:
          withdrawals.length > 0 ? withdrawals[0] : {total: 0, count: 0},
      };
    ...
    if (!type || type === "all") {
      financialData.summary.profitLoss =
        financialData.summary.totalIncome - financialData.summary.totalExpenses;
    }
    ...
    // Get bank balances
    const bankBalances = await SnigdhaBank.find().select(
      "bankName accountNumber currentAmount"
    );
    financialData.details.bankBalances = bankBalances;

    // Get imprest fund balance
    const imprestFund = await SnigdhaImprestFund.findOne().select(
      "currentAmount"
    );
  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        stats: {
          totalReceivable:
            totalReceivable.length > 0 ? totalReceivable[0].total : 0,
...
export const getBankTransactionReport = async (req, res) => {
  try {
    ...
    if (!transactionType || transactionType === "deposit") {
      ...
      const deposits = await SnigdhaDepositCredit.find(depositQuery).lean();

  // Process each deposit to get invoice details
  const depositsWithDetails = await Promise.all(deposits.map(async deposit => {
    let invoiceDetails = null;

    if (deposit.invoiceId) {
      try {
        if (deposit.invoiceType === "product") {
          // Search in Invoice model for product invoices
          invoiceDetails = await Invoice.findById(deposit.invoiceId).lean();
        } else {
          // Search in SnigdhaServiceInvoice model for service invoices
          invoiceDetails = await SnigdhaServiceInvoice.findById(deposit.invoiceId).lean();
        }
      } catch (error) {
        console.error(`Error fetching invoice details for deposit ${deposit._id}:`, error);
      }
    }
...
return {
      ...
      description: `Deposit for ${deposit.invoiceType} invoice ${deposit.invoiceNumber}`,
      reference: deposit.reference,
      id: deposit._id,
      transactionId: deposit.transactionId,
      paymentMethod: deposit.paymentMethod,
      invoiceNumber: deposit.invoiceNumber,
      invoiceType: deposit.invoiceType,
      invoiceDetails: invoiceDetails, // Add full invoice details
      bankId: deposit.bankId,
      personName: deposit.invoiceType === "product" 
        ? invoiceDetails?.receiverDetails?.name 
        : invoiceDetails?.receiverDetails?.name || 'N/A'
    };
   }));
    transactions.push(...depositsWithDetails);
}
    ...
    if (!transactionType || transactionType === "withdraw") {
      ...
      const withdrawals = await SnigdhaWithdraw.find(withdrawQuery).sort({
        date: -1,
      });
    ...
    if (!transactionType || transactionType === "expense") {
      ...
      const expenses = await SnigdhaExpense.find(expenseQuery).sort({date: -1});
    ...
    if (!transactionType || transactionType === "purchase") {
      ...
      const purchaseWindows = await SnigdhaPurchaseWindow.find(purchaseQuery)
        .populate("invoiceId", "receiverDetails")
        .sort({date: -1});
      ...
      purchaseWindows.forEach((purchase) => {
        transactions.push({
          ...
          reference: `Invoice: ${
            ...
            (purchase.invoiceId ? purchase.invoiceId.invoiceNumber : "N/A")
          }`,
...
export const getDashboardReport = async (req, res) => {
  try {
    ...
    // 11. Get notes
    const notes = await SnigdhaNotes.find().sort({createdAt: -1}).limit(5);

    // 12. Calculate month-over-month changes
    const thisMonthSalesTotal =
      thisMonthSales.length > 0 ? thisMonthSales[0].total : 0;
    ...
    const thisMonthPurchasesTotal =
      thisMonthPurchases.length > 0 ? thisMonthPurchases[0].total : 0;
    const lastMonthPurchasesTotal =
      lastMonthPurchases.length > 0 ? lastMonthPurchases[0].total : 0;
    const purchasesChange =
      lastMonthPurchasesTotal === 0
        ? 100
        : ((thisMonthPurchasesTotal - lastMonthPurchasesTotal) /
            lastMonthPurchasesTotal) *
          100;

    const thisMonthExpensesTotal =
      thisMonthExpenses.length > 0 ? thisMonthExpenses[0].total : 0;
    const lastMonthExpensesTotal =
      lastMonthExpenses.length > 0 ? lastMonthExpenses[0].total : 0;
    const expensesChange =
      lastMonthExpensesTotal === 0
        ? 100
        : ((thisMonthExpensesTotal - lastMonthExpensesTotal) /
            lastMonthExpensesTotal) *
          100;

    // 13. Calculate profit for current month
    const thisMonthProfit =
      thisMonthSalesTotal - (thisMonthPurchasesTotal + thisMonthExpensesTotal);
    const lastMonthProfit =
      lastMonthSalesTotal - (lastMonthPurchasesTotal + lastMonthExpensesTotal);
    const profitChange =
      lastMonthProfit === 0
        ? 100
        : ((thisMonthProfit - lastMonthProfit) / Math.abs(lastMonthProfit)) *
          100;
    ...
    dashboardData.summary = {
      ...
      accountsReceivable:
        accountsReceivable.length > 0 ? accountsReceivable[0].total : 0,
      accountsPayable:
        accountsPayable.length > 0 ? accountsPayable[0].total : 0,
      inventory:
        inventory.length > 0
          ? {
...
// 9. all transactions report for all banks
...
export const getAllBankTransactionsReport = async (req, res) => {
  try {
    ...
    const depositsWithType = deposits.map((deposit) => ({
      ...
      personName: deposit.invoiceId
        ? deposit.invoiceId.receiverDetails?.name
        : "N/A",
    }));
    ...
    const bankSummaries = banks.map((bank) => {
      const bankTransactions = allTransactions.filter(
        (transaction) =>
          ...
          transaction.transferFromBankId?._id.toString() ===
            bank._id.toString() ||
    ...
    const overallSummary = {
      ...
      totalDeposits: depositsWithType.reduce(
        (sum, t) => sum + Number(t.amount),
        0
      ),
      totalWithdrawals: withdrawalsWithType.reduce(
        (sum, t) => sum + Number(t.amount),
        0
      ),
      totalExpenses: expensesWithType.reduce(
        (sum, t) => sum + Number(t.amount),
        0
      ),
      totalTransferDebits: transferDebitsWithType.reduce(
        (sum, t) => sum + Number(t.transferAmount),
        0
      ),
      totalTransferCredits: transferCreditsWithType.reduce(
        (sum, t) => sum + Number(t.transferAmount),
        0
      ),
      totalPurchases: purchaseWindowsWithType.reduce(
        (sum, t) => sum + Number(t.amount),
        0
      ),
    };

    overallSummary.netChange =
      overallSummary.totalDeposits +
      overallSummary.totalTransferCredits -
      (overallSummary.totalWithdrawals +
        overallSummary.totalExpenses +
        overallSummary.totalTransferDebits +
        overallSummary.totalPurchases);
