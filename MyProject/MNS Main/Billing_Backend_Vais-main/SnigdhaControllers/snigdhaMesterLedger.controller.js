...
const generateSnigdhaMasterLedgerReport = async (filters = {}) => {
  try {
    ...
    // Create date filter object for queries
    const dateFilter = {};
    // console.log("date fileter from master ledger : ", dateFilter, startDate, endDate)
    if (startDate) {
      dateFilter.createdAt = {$gte: new Date(startDate)};
    }
    if (endDate) {
    ...
    // Only fetch data for requested report types
    const fetchSales =
      !reportType || reportType === "sales" || reportType === "all";
    const fetchPurchases =
      !reportType || reportType === "purchases" || reportType === "all";
    const fetchServices =
      !reportType || reportType === "services" || reportType === "all";
    ...
    if (fetchSales) {
      ...
      pendingSalesInvoices.forEach((invoice) => {
        const customerName =
          invoice.receiverDetails?.name || "Unknown Customer";
        if (!salesByCustomer[customerName]) {
          salesByCustomer[customerName] = {
            totalPending: 0,
            invoices: [],
      ...
      pendingLedgers.forEach((ledger) => {
        if (ledger.invoiceId) {
          const customerName =
            ledger.invoiceId.receiverDetails?.name || "Unknown Customer";
          if (!salesByCustomer[customerName]) {
            salesByCustomer[customerName] = {
              totalPending: 0,
              invoices: [],
          ...
          const existingInvoice = salesByCustomer[customerName].invoices.find(
            (inv) =>
              inv.invoiceId.toString() === ledger.invoiceId._id.toString()
          );
    ...
    // 2. Get service data if needed
    if (fetchServices) {
      // Get all pending service invoices
      const pendingServiceInvoices = await SnigdhaServiceInvoice.find({
        paidOne: false,
        ...(startDate || endDate ? {date: dateFilter.createdAt} : {}),
      });

      console.log("pendingServiceInvoices : ", pendingServiceInvoices);

      // Get all service ledger accounts with pending payments
      const pendingServiceLedgers = await SngidhaServiceLedger.find({
        isPaid: false,
        ...dateFilter,
      }).populate("invoiceId");

      // Process pending service invoices
      pendingServiceInvoices.forEach((invoice) => {
        const customerName =
          invoice.receiverDetails?.name || "Unknown Customer";
        ...
        servicesByCustomer[customerName].totalPending +=
          invoice.grandTotal || 0;
        servicesByCustomer[customerName].invoices.push({
          invoiceId: invoice._id,
          invoiceNumber: invoice.invoiceNumber,
          date: invoice.date,
          amount: invoice.grandTotal,
      ...
      // Process pending service ledgers
      pendingServiceLedgers.forEach((ledger) => {
        if (ledger.invoiceId) {
          const customerName =
            ledger.invoiceId.receiverDetails?.name || "Unknown Customer";
          if (!servicesByCustomer[customerName]) {
          ...
          const existingInvoice = servicesByCustomer[
            customerName
          ].invoices.find(
            (inv) =>
              inv.invoiceId.toString() === ledger.invoiceId._id.toString()
    ...
    if (fetchPurchases) {
      ...
      pendingPurchaseAccounts.forEach((account) => {
        ...
        if (account.invoiceId) {
          existingPurchase = purchasesByVendor[vendorName].purchases.find(
            (pur) =>
              pur.purchaseId.toString() === account.invoiceId._id.toString()
          );
        }
    ...
    const totalPurchasesPending = Object.values(purchasesByVendor).reduce(
      (sum, vendor) => sum + vendor.totalPending,
      0
    );
    ...
    // Add summary if we have multiple sections
    if ((fetchSales || fetchServices) && fetchPurchases) {
      reportData.summary = {
        totalReceivables: totalSalesPending + totalServicesPending,
        totalPayables: totalPurchasesPending,
        netPosition:
          totalSalesPending + totalServicesPending - totalPurchasesPending,
      };
    }
...
const exportToExcel = async (reportData, filePath) => {
  try {
    ...
    dateCell.value = `Generated on: ${new Date(
    ...
    if (reportData.salesLedger) {
      summaryRows.push([
        ...
        reportData.salesLedger.totalPending,
      ]);
    }
    ...
    if (reportData.purchaseLedger) {
      summaryRows.push([
        ...
        reportData.purchaseLedger.totalPending,
      ]);
    }
    ...
    if (reportData.summary) {
      summaryRows.push([
        "Total Receivables",
        reportData.summary.totalReceivables,
      ]);
...
const addPurchasesSheet = (workbook, purchaseLedger) => {
  ...
  const headerRow = purchasesSheet.addRow([
    "Vendor",
    "Invoice Number",
    "Date",
    "Amount",
  ...
  try {
    ...
    if (reportData.salesLedger) {
      summaryTable.rows.push([
        "Total Sales Receivables",
        reportData.salesLedger.totalPending.toFixed(2),
      ]);
    }
    ...
    if (reportData.purchaseLedger) {
      summaryTable.rows.push([
        "Total Purchases Payables",
        reportData.purchaseLedger.totalPending.toFixed(2),
      ]);
    }
    ...
    if (reportData.summary) {
      summaryTable.rows.push([
        "Total Receivables",
        reportData.summary.totalReceivables.toFixed(2),
      ]);
      summaryTable.rows.push([
        "Total Payables",
        reportData.summary.totalPayables.toFixed(2),
      ]);
      summaryTable.rows.push([
        "Net Position",
        reportData.summary.netPosition.toFixed(2),
      ]);
    }
    ...
    if (
      reportData.salesLedger &&
      Object.keys(reportData.salesLedger.customers).length > 0
    ) {
      doc.addPage();
      doc
        .fontSize(16)
        .text("Sales Ledger - Pending Receivables", {align: "center"});
      doc.moveDown();
      ...
      Object.entries(reportData.salesLedger.customers).forEach(
        ([customerName, data]) => {
          customerSummaryTable.rows.push([
            customerName,
      ...
      await doc.table(customerSummaryTable, {
        ...
        prepareRow: (row, i) => {
          doc
            .font(
              i === customerSummaryTable.rows.length - 1
                ? "Helvetica-Bold"
                : "Helvetica"
            )
            .fontSize(10);
    ...
    if (
      reportData.purchaseLedger &&
      Object.keys(reportData.purchaseLedger.vendors).length > 0
    ) {
      doc.addPage();
      doc
        .fontSize(16)
        .text("Purchases Ledger - Pending Payables", {align: "center"});
      doc.moveDown();
      ...
      await doc.table(vendorSummaryTable, {
        ...
        prepareRow: (row, i) => {
          doc
            .font(
              i === vendorSummaryTable.rows.length - 1
                ? "Helvetica-Bold"
                : "Helvetica"
            )
            .fontSize(10);
    ...
    if (format) {
      ...
      if (format === "excel") {
        const filePath = path.join(
          exportDir,
          `snigdha_master_ledger_${timestamp}.xlsx`
        );
        ...
        return res.status(200).json({
          success: true,
          message:
            "Snigdha master ledger report exported to Excel successfully",
          filePath: filePath,
        });
      } else if (format === "pdf") {
        const filePath = path.join(
          exportDir,
          `snigdha_master_ledger_${timestamp}.pdf`
        );
  ...
  try {
    ...
    if (format) {
      ...
      if (format === "excel") {
        const filePath = path.join(
          exportDir,
          `snigdha_sales_ledger_${timestamp}.xlsx`
        );
        await exportToExcel(reportData, filePath);
        ...
        const filePath = path.join(
          exportDir,
          `snigdha_sales_ledger_${timestamp}.pdf`
        );
        await exportToPDF(reportData, filePath);
  ...
  try {
    ...
    if (format) {
      ...
      if (format === "excel") {
        const filePath = path.join(
          exportDir,
          `snigdha_purchases_ledger_${timestamp}.xlsx`
        );
        await exportToExcel(reportData, filePath);
        ...
        return res.status(200).json({
          success: true,
          message:
            "Snigdha purchases ledger report exported to Excel successfully",
          filePath: filePath,
        });
      } else if (format === "pdf") {
        const filePath = path.join(
          exportDir,
          `snigdha_purchases_ledger_${timestamp}.pdf`
        );
        await exportToPDF(reportData, filePath);
        ...
        return res.status(200).json({
          success: true,
          message:
            "Snigdha purchases ledger report exported to PDF successfully",
          filePath: filePath,
        });
      }
    }
...
// Get only service ledger data
const getSnigdhaServiceLedgerReport = async (req, res) => {
  try {
    const {startDate, endDate, format} = req.query;

    // Generate the report with only service data
    const reportData = await generateSnigdhaMasterLedgerReport({
      startDate,
      endDate,
      reportType: "services",
    ...
    if (format) {
      ...
      if (format === "excel") {
        const filePath = path.join(
          exportDir,
          `snigdha_service_ledger_${timestamp}.xlsx`
        );
        await exportToExcel(reportData, filePath);
        ...
        return res.status(200).json({
          success: true,
          message:
            "Snigdha service ledger report exported to Excel successfully",
          filePath: filePath,
        });
      } else if (format === "pdf") {
        const filePath = path.join(
          exportDir,
          `snigdha_service_ledger_${timestamp}.pdf`
        );
        await exportToPDF(reportData, filePath);

        return res.status(200).json({
          success: true,
          message: "Snigdha service ledger report exported to PDF successfully",
          filePath: filePath,
        });
      }
    }
    ...
    return res.status(200).json({
      success: true,
      message: "Snigdha service ledger report generated successfully",
      data: reportData,
    });
  } catch (error) {
    console.error("Error in getSnigdhaServiceLedgerReport:", error);
    return res.status(500).json({
      success: false,
      message: "Failed to generate Snigdha service ledger report",
      error: error.message,
    });
  }
};
