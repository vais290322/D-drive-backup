...
// Create a new purchase account entry modify on 09-01-2026
export const createPurchaseAccount = async (req, res) => {
  try {
   ...
   const payment = paymentDetails?.[0];
    ...
    const invoice = await MnsPurchaseOrderNew.findById(invoiceId);
    if (!invoice) {
      return res.status(404).json({
        ...
        if (payment.paymentMode === "Voucher") {
          ...
          const vendorBalance = await VendorBalance.findOne({ vendorId });
    ...
    if (payment.paymentMode === "Voucher" && voucherDetails) {
      ...
      if (voucher?.noteType === "credit") {
        ...
        ledger = await PurchaseAccount.create({
          invoiceId,
          invoiceNumber,
          vendorName,
          totalAmount,
     ...
     // Validate payment details if provided
        ...
        await MnsPurchaseOrderNew.findByIdAndUpdate(
          invoiceId,
          { paidOne: true },
          { new: true }
        );
    ...
    const populatedPurchaseAccount = await PurchaseAccount.findById(ledger._id)
      .populate("invoiceId");
...
export const getPurchaseAccountById = async (req, res) => {
  try {
    ...
    const purchaseAccount = await PurchaseAccount.findById(id)
      .populate("invoiceId")
      .populate("paymentDetails.bankId");
...
export const updatePurchaseAccountOld = async (req, res) => {
  try {
    ...
    if (paymentDetails && paymentDetails.length > 0) {
      ...
      for (const payment of paymentDetails) {
        ...
        if (bank) {
          ...
          bank.deductionAmount += Number(payment.paymentAmount);
          await bank.save();
        }
      }
    ...
    const populatedPurchaseAccount = await PurchaseAccount.findById(updatedPurchaseAccount._id)
      .populate("invoiceId")
      .populate("paymentDetails.bankId");
...
export const updatePurchaseAccount = async (req, res) => {
  try {
    ...
    if (isVoucher && voucherDetails && voucherDetails.length > 0) {
      ...
      const note = await CreditDebitNote.findById(v.noteId);
      if (note) {
        note.appliedAmount += Number(v.amount);
        await note.save();
      }

      // --- Update vendor blance ---
      const vendorBalance = await VendorBalance.findOne({ vendorId: v.vendorId });
  ...
  try {
    ...
    if (purchaseAccount.paymentDetails && purchaseAccount.paymentDetails.length > 0) {
      for (const payment of purchaseAccount.paymentDetails) {
        ...
        const bank = await Bank.findById(payment.bankId);
        if (bank) {
          bank.currentAmount += payment.paymentAmount;
          bank.deductionAmount -= payment.paymentAmount;
          await bank.save();
        }

        // Delete related purchase window entries
        await PurchaseWindow.deleteMany({
          invoiceId: purchaseAccount.invoiceId,
          bankId: payment.bankId
        });
      }
    ...
    await PurchaseAccount.findByIdAndDelete(id);
...
export const getPurchaseAccountsByInvoiceId = async (req, res) => {
  try {
    ...
    const purchaseAccounts = await PurchaseAccount.find({ invoiceId })
      .populate("invoiceId")
      .populate("paymentDetails.bankId");
...
export const getPurchaseAccountsByPaymentStatus = async (req, res) => {
  try {
    ...
    const purchaseAccounts = await PurchaseAccount.find({ isPaid: isPaidBoolean })
      .populate("invoiceId")
      .populate("paymentDetails.bankId");
