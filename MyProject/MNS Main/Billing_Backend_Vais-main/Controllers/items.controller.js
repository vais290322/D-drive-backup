...
export const itemsAllReport = async (req, res) => {
    try {
        ...
        for (const item of items) {
            // Get purchase transactions from MnsPurchaseOrderNew
            const purchaseFilter = { ...dateFilter, 'items.item_id': item.item_id };
            const purchaseOrders = await mongoose.model('MnsPurchaseOrderNew').find(purchaseFilter);
            
            // Get sales transactions from InvoiceMns
            const salesFilter = { ...dateFilter, 'items.item_id': item.item_id };
            const invoices = await mongoose.model('InvoiceMns').find(salesFilter);
            ...
            if (transactions.length > 0) {
                transactions.unshift({
                    ...
                    purchaseQuantity: item.openningStock,
                    salesQuantity: 0,
                    // purchaseAmount: item.openningStock * item.unit_prize,
                    purchaseAmount: item?.openningStock * transactions[0].rate,
                    salesAmount: 0
                });
