...
export const getBankTransactionReport = async (req, res) => {
  try {
    ...
    const deposits = await DepositCredit.find(depositQuery)
      ...
      const depositsWithType = await Promise.all(deposits.map(async deposit => {
  let receiverName = 'N/A';
  
  if (deposit.invoiceId) {
    try {
      if (deposit.invoiceType === 'product') {
        const invoice = await InvoiceMns.findById(deposit.invoiceId);
        receiverName = invoice?.receiverDetails?.name || 'N/A';
      } else {
        const serviceInvoice = await Service.findById(deposit.invoiceId);
        receiverName = serviceInvoice?.receiverDetails?.name || 'N/A';
      }
    } catch (error) {
