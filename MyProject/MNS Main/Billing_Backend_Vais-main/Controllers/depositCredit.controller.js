...
export const createDepositCredit = async (req, res) => {
  try {
    const { date, amount, transactionId, invoiceNumber, paymentMethod, invoiceId, bankId,invoiceType } = req.body;

    // Validate required fields
    const requiredFields = ['date', 'amount', 'transactionId', 'invoiceNumber', 'paymentMethod', 'bankId','invoiceType'];
    ...
    const newDepositCredit = await DepositCredit.create({
      date: new Date(date),
      amount,
      transactionId,
      invoiceNumber,
    ...
    const populatedDepositCredit = await DepositCredit.findById(newDepositCredit._id)
      .populate('bankId')
      .populate('invoiceId');
...
export const getAllDepositCredits = async (req, res) => {
  try {
    const depositCredits = await DepositCredit.find()
      .populate('bankId')
      .populate('invoiceId')
      .sort({ createdAt: -1 });
...
export const getDepositCreditById = async (req, res) => {
  try {
    ...
    const depositCredit = await DepositCredit.findById(id)
      .populate('bankId')
      .populate('invoiceId');
  ...
  try {
    ...
    if (updateData.amount && updateData.amount !== depositCredit.amount) {
      ...
      if (depositCredit.bankId) {
        const bank = await Bank.findById(depositCredit.bankId);
        if (bank) {
    ...
    if (updateData.bankId && updateData.bankId !== depositCredit.bankId.toString()) {
      ...
      if (depositCredit.bankId) {
        const oldBank = await Bank.findById(depositCredit.bankId);
        if (oldBank) {
      ...
      // Add to new bank
      const newBank = await Bank.findById(updateData.bankId);
      if (newBank) {
    ...
    const updatedDepositCredit = await DepositCredit.findByIdAndUpdate(
      id,
      updateData,
...
export const deleteDepositCredit = async (req, res) => {
  try {
    ...
    const deletedDepositCredit = await DepositCredit.findByIdAndDelete(id);
...
export const getDepositCreditsByInvoiceId = async (req, res) => {
  try {
    ...
    const depositCredits = await DepositCredit.find({ invoiceId })
      .populate('bankId')
      .populate('invoiceId')
      .sort({ createdAt: -1 });
...
export const getDepositCreditsByBankId = async (req, res) => {
  try {
    ...
    const depositCredits = await DepositCredit.find({ bankId })
      .populate('bankId')
      .populate('invoiceId')
      .sort({ createdAt: -1 });
