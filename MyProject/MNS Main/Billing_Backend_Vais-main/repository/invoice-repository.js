        ...
        try {
            ...
            if (updateData.items && Array.isArray(updateData.items)) {
                ...
                for (const originalItem of originalInvoice.items) {
                    ...
                    if (inventoryItem) {
                        ...
                        inventoryItem.quantity += Number(originalItem.quantity);
                         inventoryItem.totalSales = Math.max(
                        0, 
                        (inventoryItem.totalSales || 0) - Number(originalItem.quantity)
                    );
                ...
                for (const newItem of updateData.items) {
                    ...
                    const inventoryItem = await SnigdhaItem.findOne({ item_id: newItem.id }).session(session);
                    // console.log("inventory item : ",inventoryItem)
                    
                    if (!inventoryItem) {
                        throw new Error(`Item with ID ${newItem.id} not found in inventory.`);
                    }
