  ...
  return (
    <div className='container mx-auto px-4 my-10 relative'>
      <div className='flex justify-between items-center mb-6'>
        <h2 className='text-2xl font-bold text-green-800 py-2 border-b-2 border-green-500 inline-block'>{heading}</h2>
      ...
      {category === "Mango Veriety" && (
        <div className='flex gap-3 mb-4'>
          <button 
            className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${activeSubcategory === 'all' ? 'bg-green-600 text-white' : 'bg-green-100 text-green-800 hover:bg-green-200'}`}
            onClick={() => setActiveSubcategory('all')}
          >
            All Mangos
          </button>
          <button 
            className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${activeSubcategory === 'thai' ? 'bg-green-600 text-white' : 'bg-green-100 text-green-800 hover:bg-green-200'}`}
            onClick={() => setActiveSubcategory('thai')}
          >
            Thai Mangos
          </button>
          <button 
            className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${activeSubcategory === 'indian' ? 'bg-green-600 text-white' : 'bg-green-100 text-green-800 hover:bg-green-200'}`}
      ...
      <div className='flex items-center gap-4 md:gap-6 overflow-x-auto scrollbar-none transition-all pb-4' ref={scrollElement}>
        ...
        {loading ? (
          ...
          filteredData.map((product,index)=>{
            return(
              <div key={product?._id} className='group relative w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] h-40 bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-300 flex overflow-hidden border border-gray-100'>
                <Link to={"product/"+product?._id} className='flex w-full'>
                  <div className='bg-green-50 h-full p-4 min-w-[120px] md:min-w-[145px] flex items-center justify-center'>
                    <img 
                      src={product.productImage[0]} 
                      className='object-contain h-full group-hover:scale-110 transition-all duration-300'
                  ...
                  <div className='p-4 grid content-between w-full'>
                    ...
                    <div className='flex justify-end'>
                      <button 
                        className='text-sm bg-green-600 hover:bg-green-700 text-white px-4 py-1 rounded-full shadow-sm hover:shadow transition-all duration-300 flex items-center justify-center' 
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                ...
                <div className="absolute cursor-pointer top-2 right-2 bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  View Details
                </div>
              </div>
            )
