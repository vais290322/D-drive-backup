...
export const createApiKey = asyncHandler(async (req, res) => {
    ...
    const planDetails = userDetails.plan;
    let apiKeyLimit = 10;
    let apiKeyUsageLimit = 100;
    if (planDetails === "Free") {
        apiKeyLimit = 1;
        apiKeyUsageLimit = 100;
    } else if (planDetails === "Pro") {
        apiKeyLimit = 10;
        apiKeyUsageLimit = 5000;
    } else if (planDetails === "Enterprise") {
        apiKeyLimit = -1;
        apiKeyUsageLimit = -1;
    }
    ...
    if (activeKeysCount >= apiKeyLimit && apiKeyLimit !== -1) {
        throw new ApiError(
            400,
            `Maximum API key limit reached (${apiKeyLimit}). Please revoke some keys before creating new ones.`
        );
    }
    ...
    const apiKey = new ApiKey({
        ...
        permissions,
        prefix,
        usageLimit: usageLimit || apiKeyUsageLimit || 100,
    });
