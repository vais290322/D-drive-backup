...
const SnigdhaMoneyTransferPage = () => {
  ...
  const fetchTransfers = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get(`${backendDomainS}/api/money-transfers`);
  ...
  const handleTransferSubmit = async (e) => {
    ...
    try {
    const response =  await axios.post(`${backendDomainS}/api/money-transfers`, transferForm);
      if(response.data?.success){
  ...
  const exportAllToPDF = () => {
    ...
    const data = filteredTransfers.map(transfer => ({
      ...
      fromBank: transfer.transferFromBankId.bankName,
      toBank: transfer.transferToBankId.bankName,
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Contra</h1>
...
export default SnigdhaMoneyTransferPage;
