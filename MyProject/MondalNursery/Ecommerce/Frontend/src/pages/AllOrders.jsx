  ...
  return (
    <div className="bg-white p-4">
                ...
                >
                  ...
                  <td className="p-2 hidden md:table-cell">
                    <div className="text-sm ">
                      <strong>Name:</strong> {el?.shippingAddress.fullName}
                      <br />
                      <strong>Email:</strong> {el?.shippingAddress.email}
                      <br />
                      <strong>City:</strong> {el?.shippingAddress.city}
                      <br />
                      <strong>Phone:</strong> {el?.shippingAddress.phoneNumber}
                      <br />
                      <strong>State:</strong> {el?.shippingAddress.state}
                      <br />
                      <strong>Zip:</strong> {el?.shippingAddress.zipCode}
                      <br />
                      <strong>Street:</strong>{" "}
                      {el?.shippingAddress.streetAddress}
                    </div>
                  </td>
                  <td className="p-2 font-medium text-center hidden md:table-cell">
                    {el?.paymentMethod}
                  </td>
                  ...
                  <td
                    className={`p- font-medium  text-center ${
                      el.isPaid ? "text-green" : "text-red"
                    }`}
                  >
                    {el.isPaid ? "Yes" : "No"}
                  </td>
                  <td
                    className={`p-2 font-medium  text-center ${getStatusClass(
                      el.deliveryStatus
                    )}`}
                  >
                    {el?.deliveryStatus}
      ...
      <div className="flex justify-between items-center mt-4">
        <button
          className={`p-2 font-medium text-3xl  ${
            currentPage === 1 ? "text-gray-200" : "text-blue-500 "
          } rounded`}
        ...
        <button
          className={`p-2 font-medium text-3xl ${
            currentPage === totalPages ? "text-gray-200" : "text-blue-500 "
          } rounded`}
          onClick={() => handlePageChange(currentPage + 1)}
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg w-3/4 max-w-2xl relative">
            ...
            {selectedOrder.orderItems.map((item, idx) => (
              <div key={idx} className="border-b pb-4 mb-4">
                ...
                <p>
                  <strong>Selling Price:</strong> {item.productId.sellingPrice}
                </p>
                <p>
