...
const AdminProductCard = ({ data, fetchdata }) => {
  ...
  try {
    // 1. First delete all images from bucket (if fileId exists)
    if (Array.isArray(data.productImage)) {
      for (const img of data.productImage) {
        if (img?.fileId) {
          try {
            await deleteImageFromBucket(img.fileId);
          } catch (err) {
            console.error("Image delete failed for:", img.fileId, err);
            // We don't stop product deletion for single image failure
          }
        }
      }
    }

    // 2. Now delete product from backend
    const response = await fetch(`${SummaryApi.deleteProduct.url}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
