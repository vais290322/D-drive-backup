...
const AdminEditProduct = ({ onClose, productData, fetchdata }) => {
  ...
  const handleUploadProduct = async (e) => {
    ...
    try {
      ...
      setData(prev => ({
        ...prev,
        productImage: [...prev.productImage, newImage]
      }))
  ...
  const handleDeleteProductImageOld = (index) => {
    const updated = [...data.productImage]
  ...
  try {
    const image = data.productImage[index]

    // 🟡 OLD DATA (no fileId) → just remove locally
    if (!image?.fileId) {
      const updated = [...data.productImage]
      updated.splice(index, 1)
    ...
    // 🔴 NEW DATA (has fileId) → delete from bucket first
    const res = await deleteImageFromBucket(image.fileId)
        ...
        <form className='grid p-4 gap-2 overflow-y-scroll h-full pb-5' onSubmit={handleSubmit}>
          ...
          <div className='flex items-center gap-2 mt-2'>
            {data.productImage.map((el, index) => (
              <div key={el.fileId || index} className='relative group'>
                <img
                  src={el.url}
                  alt={el.fileId}
                  width={80}
