...
const loginUser = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (checkPassword) {
            ...
            console.log(token + ": in my controller")
            const tokenOption = {
                sameSite: 'None',
                httpOnly: true,
                secure: true
            ...
            res.cookie("token", token, tokenOption).status(200).json(
                new ApiResponse(200, { user, token }, "user login successfully done")
            )
        }
    } catch (error) {
        res.json({
...
const forgotPassword = asyncHandler(async (req, res) => {
    try {
        ...
        const resetLink = `https://jrmondalnursery.com/reset-password/${user._id}/${token}`
        ...
        const mailOptions = {
            ...
            <p>Thank you,<br>JR Mondal Nursery</p>
    `
        };
...
const createOrder = asyncHandler(async (req, res) => {
    try {
        ...
        console.log("order items", orderItems, shippingAddress, paymentMethod, itemsPrice, shippingPrice, totalPrice,
            isPaid, paidAt, razorpay);
        ...
        res.status(400).json(
            new ApiError(400, error?.message || error || "order not created")
...
const userOrders = asyncHandler(async (req, res) => {
    const userId = req.userId
    console.log(userId + "user id from user order for mobile ");
    try {
