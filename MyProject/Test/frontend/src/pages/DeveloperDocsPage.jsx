...
const DeveloperDocsPage = () => {
    ...
    const [activeBackendLang, setActiveBackendLang] = useState("node");
    ...
    const uploadResponse = {
        ...
        "data": {
            ...
            "fileName": "WhatsApp Image 2025-04-24 at 10.30.11 AM.jpeg",
            "fileUrl": "https://apibucket.vais.co.in/uploads/users/697b2c970829419d7080fbdc/2026/02/1770193219068-e9a18679-29fd-42e9-ad58-3eec28828b97-WhatsApp_Image_2025_04_24_at_10_30_11_AM__4_.jpeg",
            "fileType": "image",
            "mimeType": "image/png",
            "size": 265655,
    ...
    const codeExamples = {
        curl: {
            title: "cURL (Upload)",
            code: `curl -X POST https://apibucket.vais.co.in/api/v1/user/files/upload \\
  ...
  -F "folderId=695ce180d810b1e51cc9" # Optional folder ID`,
        ...
        deleteCurl: {
            title: "cURL (Delete)",
            code: `curl -X DELETE https://apibucket.vais.co.in/api/v1/user/files/695ce180d810b1e51cc91237 \\
  -H "X-API-Key: YOUR_API_KEY"`,
        },
        javascript: {
...
const response = await axios.post('https://apibucket.vais.co.in/api/v1/user/files/upload', formData, {
  headers: {
...
url = "https://apibucket.vais.co.in/api/v1/user/files/upload"
...
curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://apibucket.vais.co.in/api/v1/user/files/upload',
...
var request = new HttpRequestMessage(HttpMethod.Post, "https://apibucket.vais.co.in/api/v1/user/files/upload");
...
data = {
  "visibility": "public",
  "folderId": "695ce180d810b1e51cc9"   # optional
}
...
curl_setopt($ch, CURLOPT_URL, "https://apibucket.vais.co.in/api/v1/user/files/upload");
...
RequestBody body = new MultipartBody.Builder()
  ...
  .addFormDataPart("visibility","public")
  .addFormDataPart("folderId","695ce180d810b1e51cc9") // optional
  .build();
...
Console.WriteLine(body);`
        ...
        delete: {
            title: "Delete file (Server side)",
            code: `import axios from "axios";

await axios.delete(
  "https://apibucket.vais.co.in/api/v1/user/files/FILE_ID",
  {
    headers: {
      "X-API-Key": process.env.VAIS_BUCKET_API_KEY
    }
            ...
            <div className="max-w-5xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
                    ...
                    <nav className="hidden lg:block space-y-2 sticky top-24 h-fit">
                        ...
                        {["Introduction", "Authentication", "Base URL", "Endpoints", "Response Format", "Frontend Implementation", "Backend Implementation"].map((item) => (
                            <a
                                key={item}
                    ...
                    <div className="lg:col-span-3 space-y-16">
                        ...
                        <section id="base-url">
                            ...
                            <div className="relative inline-flex items-center bg-gray-900 text-blue-400 px-4 py-3 rounded-lg font-mono text-sm">

                                <span>https://apibucket.vais.co.in/api/v1/user</span>

                                <button
                                    onClick={() =>
                                        copyToClipboard(
                                            "https://apibucket.vais.co.in/api/v1/user",
                                            "base-url"
                                        )
                                    }
                                    className="ml-3 text-gray-400 hover:text-white transition-colors cursor-pointer"
                                    title="Copy Base URL"
                                >
                        ...
                        <section id="frontend-implementation">
                            <h2 className="text-3xl font-bold text-gray-900 mb-8 flex items-center gap-3">
                                <CommandLineIcon className="w-8 h-8 text-blue-600" />
                        ...
                        <section id="backend-implementation" className="mt-24">
                            <h2 className="text-3xl font-bold text-gray-900 mb-8 flex items-center gap-3">
                            ...
                            <p className="text-gray-700 mb-6 text-lg">
                                Use these examples when your server receives a file from your client and uploads it to
                                VaisBucket (Server → Server integration).
                                No cookies are required. Only the API key is used.
                            </p>
                            ...
                            <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
                                ...
                                <div className="flex bg-gray-50 border-b border-gray-100 overflow-x-auto">
                                    {Object.keys(backendExamples).map((lang) => (
                                        <button
                                            key={lang}
                                            onClick={() => setActiveBackendLang(lang)}
                                            className={`px-6 py-4 cursor-pointer text-sm font-bold whitespace-nowrap transition-all
            ${activeBackendLang === lang
                                                    ? "bg-white text-indigo-600 border-b-2 border-indigo-600"
                                                    : "text-gray-500 hover:text-gray-700 hover:bg-gray-100"}
          `}
                                        >
                                            {backendExamples[lang].title}
                                        </button>
                                    ))}
                                </div>
                                ...
                                <div className="p-6">
                                    <div className="flex justify-between items-center mb-4">
                                        <h3 className="text-lg font-bold text-gray-900">
                                            {backendExamples[activeBackendLang].title}
                                        </h3>

                                        <button
                                            onClick={() =>
                                                copyToClipboard(
                                                    backendExamples[activeBackendLang].code,
                                                    `backend-${activeBackendLang}`
                                                )
                                            }
                                            className="flex items-center gap-2 text-sm font-medium text-indigo-600 hover:text-indigo-700 bg-indigo-50 px-3 py-1.5 rounded-lg cursor-pointer"
                                        >
                                            {copiedSection === `backend-${activeBackendLang}` ? (
                                                <>
                                                    <CheckIcon className="w-4 h-4" />
                                                    <span>Copied!</span>
                                                </>
                                    ...
                                    <div className="bg-gray-900 rounded-xl overflow-hidden shadow-inner">
                                        <pre className="p-6 text-sm font-mono text-gray-100 overflow-x-auto leading-relaxed">
                                            <code>{backendExamples[activeBackendLang].code}</code>
                                        </pre>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mt-6 bg-indigo-50 border border-indigo-100 rounded-xl p-6">
                                <p className="text-indigo-900 text-sm">
                                    Typical flow:
                                    <br />
                                    <strong>Browser / Mobile App → Your Backend → VaisBucket</strong>
                                </p>
                            </div>
                        </section>
