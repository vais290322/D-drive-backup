...
export default function ItemsList() {
  ...
  const handleInputChange = (e) => {
    ...
    const newUpdatedData = { 
      ...updatedData, 
      [name]: name === "quantity"  || name === "unit_prize" || name === "sellingPrice"
        ? parseFloat(value) || 0 
        : value 
    };
    
    // Calculate total price whenever relevant fields change
    if (["quantity", "unit_prize", "cgst", "sgst", "igst"].includes(name)) {
  ...
  const handleUpdate = async () => {
    setLoading(true);
    try {
      const response = await axios.put(`${updateItemUrl}/${selectedItem._id}`, updatedData);
      if(response.data.success){
        toast.success(response.data.message)
      }
      fetchItems();
  ...
  const columns = [
    ...
    },
    {
      ...
      minWidth: 120,
      type: "number",
      valueFormatter: (params) => `₹ ${params.value?.toLocaleString() || '0.00'}`,
      renderCell: (params) => (
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <TextField 
                ...
                value={updatedData.item_description || ""} 
                onChange={handleInputChange} 
                fullWidth 
                margin="dense"
                multiline
      ...
      >
        ...
        <DialogContent sx={{ pt: 3, pb: 2, px: 3 }}>
          ...
          <Typography variant="body1">
            Are you sure you want to delete this item? This will permanently remove the item from your inventory.
          </Typography>
        </DialogContent>
      ...
      {items.length === 0 && !loading && (
        <Box sx={{ 
          ...
          <Typography variant="body2" color="text.secondary" align="center" sx={{ maxWidth: 400, mb: 3 }}>
            There are no items in your inventory yet. Add your first item to get started.
          </Typography>
          <Button 
            variant="contained" 
            color="primary" 
