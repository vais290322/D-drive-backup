      ...
      if (searchTerm.trim()) {
        ...
        result = result?.filter(invoice => {
          if (searchField === "all") {
            ...
            return invoice.grandTotal && invoice.grandTotal.toString().includes(term);
          }
          return false;
        });
      }
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-6">Service Invoice Management</h2>
            ...
            <div className="p-4">
              ...
              <div className="bg-gray-50 p-4 rounded-lg mb-6">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full mb-4">
                    <thead>
                      <tr className="bg-gray-100 flex items-center justify-between">
                        <th className="p-2 text-left text-gray-600">Description</th>
                        <th className="p-2 text-right text-gray-600">Number of Person</th>
                        <th className="p-2 text-right text-gray-600 pl-14">Amount(Rs)</th>
                      </tr>
                    </thead>
                    <tbody>
                      {selectedInvoice.items?.map((item, index) => (
                        
                        <tr key={index} className="border-b flex items-center justify-between  hover:bg-gray-100">
              ...
              <div className="flex justify-end">
                <div className="w-64 bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="flex justify-between mb-2">
                    <span className="text-gray-600">Tax ({(parseFloat(selectedInvoice.total?.cgst || 0) + 
                                       parseFloat(selectedInvoice.total?.sgst || 0) + 
                                       parseFloat(selectedInvoice.total?.igst || 0))}%) : </span>
                    <span>₹{((parseFloat(selectedInvoice.total?.grossAmount || 0) * 
                        (parseFloat(selectedInvoice.total?.cgst || 0) + 
                         parseFloat(selectedInvoice.total?.sgst || 0) + 
                         parseFloat(selectedInvoice.total?.igst || 0)) / 100)).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between font-bold text-lg border-t pt-2 mt-2">
                    <span>Total:</span>
                    <span>₹{(parseFloat(selectedInvoice.total?.grossAmount || 0) + 
                        (parseFloat(selectedInvoice.total?.grossAmount || 0) * 
                         (parseFloat(selectedInvoice.total?.cgst || 0) + 
                          parseFloat(selectedInvoice.total?.sgst || 0) + 
                          parseFloat(selectedInvoice.total?.igst || 0)) / 100)).toFixed(2)}</span>
                  </div>
