...
const fetchInventoryUrl=import.meta.env.VITE_REACT_FETCH_INVENTORY_SIN
...
const SnigdhaInventoryReportPage = () => {
  ...
  const categories = ['all', ...new Set(currentInventory?.map(item => item?.category))];
  ...
  const filteredData = currentInventory?.filter(item => {
    ...
    // For now, we'll skip category and stock filtering since the data structure is different
    // We can implement these once we have the proper fields
  ...
  const sortedData = [...(filteredData || [])].sort((a, b) => {
    ...
    switch(sortField) {
      case 'name':
        ...
        break;
      case 'sku':
        aValue = a?.inventory_id || '';
        bValue = b?.inventory_id || '';
        break;
      case 'quantity':
        aValue = a?.quantity || 0;
        bValue = b?.quantity || 0;
  ...
  const lowStockItems = filteredData?.filter(item => 
    (parseInt(item?.quantity) || 0) > 0 && (parseInt(item?.quantity) || 0) <= 10
  ).length;
  
  const outOfStockItems = filteredData?.filter(item => 
  ...
  const handleExcelExport = () => {
    ...
    const exportData = filteredData.map(item => {
      ...
      const status = (item?.quantity || 0) > 10 ? 'In Stock' : 
                     (item?.quantity || 0) > 0 ? 'Low Stock' : 'Out of Stock';
      
      return {
        'Product Name': item?.item_name || 'N/A',
        'Inventory ID': item?.inventory_id || 'N/A',
  ...
  const handlePdfExport = () => {
    ...
    // Create table
    const tableColumn = ["Name", "Inventory ID", "Quantity", "Unit Price", "Total Value", "Status"];
    const tableRows = [];
    ...
    filteredData.forEach(item => {
      ...
      const rowData = [
        item?.item_name || 'N/A',
        item?.inventory_id || 'N/A',
        item?.quantity || 0,
        formatCurrency(item?.unit_price || 0).replace('₹', 'Rs.'),
    ...
    const summaryData = [
      ['Total Inventory Value', formatCurrency(totalInventoryValue).replace('₹', 'Rs.')],
  ...
  return (
    <div className="container mx-auto p-4 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-lg shadow-lg p-6 mb-6" ref={componentRef}>
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Inventory Report for snigdha</h1>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 print:hidden">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 shadow-sm">
              <h3 className="text-lg font-semibold text-green-800 mb-2">Total Value</h3>
            ...
            <table className="min-w-full bg-white border border-gray-200 rounded-lg overflow-hidden">
              ...
              <tbody className="divide-y divide-gray-200">
                {currentItems?.map((item, index) => {
                  ...
                  return (
                    <tr key={item?.inventory_id || index} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-sm text-gray-900">{item?.item_name || "N/A"}</td>
                      <td className="px-4 py-3 text-sm text-gray-500">{item?.inventory_id || "N/A"}</td>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-900 text-right">{formatCurrency(item?.unit_prize || 0)}</td>
                      <td className="px-4 py-3 text-sm text-gray-900 text-right">{formatCurrency(item?.total_prize)}</td>
