...
const SnigdhaSetUnitPage = () => {
  ...
  const getAllUnitsInfo = async () => {
    try {
      ...
      const getUnitsData = await fetch(`${backendDomainS}/api/v1/units/all-units`, {
        method: "GET",
        headers: {
...
export default SnigdhaSetUnitPage;
...
const SetUnitsForm = ({getAllUnitsInfo,unitData,setUnitData}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const sendData = await fetch(`${backendDomainS}/api/v1/units/create-unit`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
const PaginationTable = ({data}) => {
  ...
  const handleDelete =async (_id) => {
    if (window.confirm("Are you sure you want to delete this unit?")) {
      const response = await axios.post(`${backendDomainS}/api/v1/units/delete-unit/${_id}`)
      if(response){
