...
const SnigdhaAddClientPage = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        `${backendDomainS}/api/v1/company/create`,
        newClient
      );
      // console.log("response : ", response);
      if (response) {
  ...
  const handleEditSubmit = async () => {
    try {
     ...
     const response = await axios.put(
        `${backendDomainS}/api/v1/company/update/${selectedClient._id}`,
        selectedClient
      );
      if (response) {
        fetchClients();
  ...
  const confirmDelete = async () => {
    try {
      ...
      const resonse = await axios.delete(
        `${backendDomainS}/api/v1/company/delete/${selectedClient._id}`
      );
      if (resonse) {
        fetchClients();
  ...
  return (
    ...
    <div className="flex justify-between items-center mb-4 bg-white p-4">
      <h1 className="text-2xl font-semibold">All Clients in Tamanna</h1>
      <input
        type="text"
...
export default SnigdhaAddClientPage 