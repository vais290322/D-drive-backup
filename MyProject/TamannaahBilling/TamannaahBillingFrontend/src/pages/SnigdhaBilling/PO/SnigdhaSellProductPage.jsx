...
const RaisePoPage = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const response = await axios.get(
          `${backendDomainS}/api/v1/inventory/all-inventory-current-items`
        );
    ...
    const fetchClients = async () => {
      try {
        const response = await axios.get(
          `${backendDomainS}/api/v1/company/all`
        );
  ...
  const addProductToSnigdha = async () => {
    ...
    if (parseInt(quantityToAdd) > parseInt(snigdhaItem.quantity)) {
      toast.error("Requested quantity exceeds available quantity in Tamanna inventory");
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(`Successfully added ${quantityToAdd} units to Tamanna inventory`);
      ...
      toast.error("Failed to add product to Tamanna inventory");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${backendDomainS}/api/v1/purchase-order/create`,
        submitData
      );
  ...
  return (
    <div className="p-4 bg-white w-[80%] md:w-[90%] md:mx-auto">
      <h2 className="text-xl font-semibold mb-4">Sell Product From Tamanna </h2>
          ...
          {!showAddForm ? (
            ...
            <div className="mt-4 border-t pt-4">
              ...
              <div className="flex space-x-2">
                ...
                >
                  {isLoadingSnigdhaItem ? "Adding..." : "Add to Tamanna Inventory"}
                </button>
                <button
                  onClick={() => {
                    setShowAddForm(false);
