...
const SnigdhaCreditDebitNotePage = () => {
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err.message || "An error occurred while adding note");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.error(err.message || "An error occurred while deleting note");
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportNoteToPDF = (note) => {
    ...
    pdf.text(`${companyName}`, margin + 25, 18);
    ...
    pdf.text(`${companyName}`, margin + 5, companyCardY + 3);
    pdf.text(
      `${companyAddress}`,
      margin + 5,
      companyCardY + 10
    ...
    pdf.text(
      `Phone: ${companyPhone} |  Email: ${companyEmail}  |  GSTIN: ${companyGST}`,
      margin + 5,
      companyCardY + 17
    );
    ...
    pdf.text(
      `Generated on: ${new Date().toLocaleString()} | Powered by ${companyName} PVT LTD`,
      pageWidth / 2,
      footerY + 8,
      { align: "center" }
    );
  ...
  const exportNoteToExcel = async (note) => {
    try {
      ...
      workbook.creator = "Tamanna System";
      workbook.lastModifiedBy = "Tamanna System";
      ...
      noteTypeHeader.value = `${note.noteType.toUpperCase()} NOTE - ${
        note.referenceNumber
      }`;
      noteTypeHeader.font = {
        name: "Arial",
      ...
      worksheet.getCell("A5").value = `${companyName}`;
      worksheet.getCell("A5").font = { bold: true };

      worksheet.mergeCells("A6:H6");
      worksheet.getCell("A6").value =  `${companyAddress}`;
      ...
      worksheet.getCell("B14").value = note.invoiceDetails?.receiverDetails?.name || "N/A";
      ...
      worksheet.getCell("A16").value = `Description: ${
        note.description || "No description provided"
      }`;

      worksheet.mergeCells("A17:H17");
      worksheet.getCell("A17").value = `Reason: ${
              ...
              <div className="hidden sm:flex sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm text-gray-700">
                    Showing{" "}
                    <span className="font-medium">{paginatedNotes?.length}</span>{" "}
                          ...
                          <div>
                            ...
                            >
                              ...
                              {invoices?.map((invoice) => (
                                ...
                                >
                                  {invoice.invoiceNumber} - Total  {invoice?.totalPayableAmount || invoice?.grandTotal || invoice?.total?.totalPayableAmount || invoice?.total?.grandTotal}
                                </option>
                              ))}
                            </select>
                          </div>
                ...
                <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <div className="sm:flex sm:items-start">
                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                      ...
                      {/* Edit form fields similar to Add Modal */}
                      {/* ... */}
                    </div>
                  </div>
                </div>
