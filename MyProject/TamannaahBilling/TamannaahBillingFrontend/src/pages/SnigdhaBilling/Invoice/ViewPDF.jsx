...
const ViewPDF = ({ invoiceData }) => {
  ...
  const discount = invoiceData?.discount.toFixed(2) || 0;
  const taxableAmount = invoiceData?.taxableAmount.toFixed(2) || 0;
  const taxAmount = invoiceData?.taxAmount.toFixed(2) || 0;
  const roundOff = invoiceData?.roundOff || 0;
  const totalPayableAmount = invoiceData?.totalPayableAmount || 0;

  // console.log("round of value : ",invoiceData)
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      pdf.text(`${companyName}`, 105, 18, { align: "center" }); // Y reduced from 20
      ...
      pdf.setTextColor(0, 0, 0);
      pdf.text(`${companyAddress}`, 105, 25, { // Y reduced from 28
        align: "center",
      });
      ...
      pdf.setTextColor(0, 0, 0);
      pdf.setFont("helvetica", "bold");
      pdf.text("GSTIN:", 20, 32); // Y reduced from 35
      pdf.text("STATE:", 20, 37); // Y reduced from 40

      pdf.text("Email:", 120, 32); // Y reduced from 35
      pdf.text("Phone:", 120, 37); // Y reduced from 40

      pdf.setFont("helvetica", "normal");
      pdf.setTextColor(50, 50, 50);
      pdf.text(gst, 35, 32); // Y reduced from 35
      pdf.text(`${companyState}`, 35, 37); // Y reduced from 40

      pdf.text(`${companyEmail}`, 135, 32); // Y reduced from 35
      pdf.text(`${companyPhone}`, 135, 37); // Y reduced from 40
      ...
      pdf.text(new Date(invoiceData?.poDate).toLocaleDateString('en-GB')|| " ", 140, 121); // Y reduced from 127
      ...
      if (hsnTableY > 245) { // Increased from 230
        pdf.addPage();
        pdf.setDrawColor(0, 0, 0);
        pdf.setLineWidth(0.5);
        pdf.rect(6, 10, 198, 277);
      ...
      if (invoiceData?.grossAmount > 0) {
        pdf.setFont("helvetica", "bold");
        pdf.setTextColor(0, 0, 0);
        pdf.text("Gross Amount:", 140, currentY);
        pdf.setFont("helvetica", "normal");
        pdf.setTextColor(50, 50, 50);
        pdf.text(`${invoiceData?.grossAmount}/-`, 190, currentY, {
          align: "right",
        });
      ...
      // Show Taxable Amount if greater than 0
      if (invoiceData?.totalServiceCharge > 0) {
        pdf.setFont("helvetica", "bold");
        pdf.setTextColor(0, 0, 0);
        pdf.text("Service Charge:", 140, currentY);
        pdf.setFont("helvetica", "normal");
        pdf.setTextColor(50, 50, 50);
        pdf.text(`${invoiceData?.totalServiceCharge}/-`, 190, currentY, { align: "right" });
        currentY += lineSpacing;
      }
      ...
      if (roundOff > 0) {
        ...
        pdf.setFont("helvetica", "normal");
        pdf.setTextColor(50, 50, 50);
        pdf.text(`${roundOff}/-`, 190, currentY+5, { align: "right" });
        currentY += lineSpacing;
      }
      ...
      pdf.setTextColor(0, 0, 0);
      pdf.setFontSize(8); // Reduced from 9
      pdf.text("Received the Material in Good Condition", 15, hsnTableY + 20); // Reduced from +55
      pdf.text(`For ${companyName} `, 185, hsnTableY + 20, { // Reduced from +55
        align: "right",
      });
        ...
        if (qrCodeElement) {
          html2canvas(qrCodeElement, {
            ...
            .then((canvas) => {
              ...
              pdf.addImage(qrDataURL, "PNG", 90, hsnTableY + 25, 25, 25); // Reduced from +50
      ...
      function finalizePdf() {
        ...
        pdf.setDrawColor(0, 0, 0);
        pdf.line(15, hsnTableY + 40, 80, hsnTableY + 40); // Reduced from +70
        pdf.line(130, hsnTableY + 40, 185, hsnTableY + 40); // Reduced from +70

        // Add signature labels
        pdf.setFontSize(7); // Reduced from 8
        pdf.text("Receiver's Signature & Seal", 15, hsnTableY + 45); // Reduced from +75
        pdf.text("Authorized Signature", 185, hsnTableY + 45, { // Reduced from +75
          align: "right",
        });

        // Add horizontal line
        pdf.setDrawColor(0, 0, 0);
        pdf.line(10, hsnTableY + 52, 200, hsnTableY + 52); // Reduced from +80
        ...
        pdf.text(
          ...
          105,
          hsnTableY + 57, // Reduced from +85
          { align: "center" }
        );
        pdf.text("Thank You!", 105, hsnTableY + 62, { align: "center" }); // Reduced from +90
        ...
        // Save the PDF
        pdf.save(`Tamanna-Product-Invoice-${invoiceData?.invoiceNumber || "download"}.pdf`);
      }
    } catch (error) {
...
const handleExcelExport = async () => {
  try {
    ...
    worksheet.mergeCells(`A${currentRow}:J${currentRow}`);
    worksheet.getCell(`A${currentRow}`).value = `${companyAddress}`;
    worksheet.getCell(`A${currentRow}`).alignment = { horizontal: 'center' };
    currentRow++;
    
    // Company details
    worksheet.getCell(`A${currentRow}`).value = `GSTIN: ${gst || "N/A"}`;
    worksheet.getCell(`E${currentRow}`).value = `Email: ${companyEmail || "N/A"}`;
    currentRow++;
    
    worksheet.getCell(`A${currentRow}`).value = `State: ${companyState} `;
    worksheet.getCell(`E${currentRow}`).value = `Phone: ${companyPhone || "N/A"}`;
    currentRow += 2;
    ...
    worksheet.getCell(`B${currentRow}`).value = new Date(date).toLocaleDateString('en-GB') || "N/A";
    currentRow++;
    ...
    const shippingRows = [
      ['Name:', name || "N/A", 'Name:', invoiceData?.receiverDetails?.name || "N/A"],
      ['Address:', address || "N/A", 'Address:', invoiceData?.receiverDetails?.deliveryAddress || "N/A"],
      ['State:', state || "N/A", 'State:', "N/A"],
      ['GSTIN:', gstNo || "N/A", 'GSTIN:', invoiceData?.receiverDetails?.gstin || "N/A"],
      ['Code:', code || "N/A", 'PO Number:', invoiceData?.poNumber || "N/A"],
      ['', '', 'PO Date:', new Date(invoiceData?.poDate).toLocaleDateString('en-GB') || "N/A"]
    ];
    ...
    const headers = ['Sl.No', 'Item Name', 'HSN/SAC', 'Qty', 'Unit', 'Rate', 'Amount', 'Tax %', 'Tax Amount', 'Service Charge','Total Amount'];
    ...
    invoiceData?.items?.forEach((item, index) => {
      ...
      const itemNameWithDesc = item.itemName + (description ? " (" + description + ")" : "");
    ...
    if (invoiceData?.items?.length > 0) {
      ...
      if (taxGroup === "State Tax") {
        hsnHeaders = ["HSN CODE", "TABLE AMT", "CGST%", "CGST AMT", "SGST%", "SGST AMT", "TOTAL TAX"];
      } else if (taxGroup === "Other Tax") {
        hsnHeaders = ["HSN CODE", "TABLE AMT", "IGST%", "IGST AMT"];
      } else {
        hsnHeaders = ["HSN CODE", "TABLE AMT", "CGST%", "CGST AMT", "SGST%", "SGST AMT", "TOTAL TAX", "IGST%", "IGST AMT"];
      }
      ...
      Object.values(hsnGroups).forEach((data) => {
        ...
        if (taxGroup === "State Tax") {
          const totalTax = parseFloat(data.cgstAmt) + parseFloat(data.sgstAmt);
          rowData = [
            data.hsnCode,
            data.tableAmt.toFixed(2),
            data.cgstPercent,
    ...
    // Amount in Words
    worksheet.mergeCells(`A${currentRow}:J${currentRow}`);
    worksheet.getCell(`A${currentRow}`).value = `${toWords.convert(totalPayableAmount || grandTotal)} RUPEES ONLY`;
    worksheet.getCell(`A${currentRow}`).font = { bold: true };
    currentRow += 2;
    ...
    if (parseFloat(transportationCharges) > 0) {
      summaryData.push(['Transportation Charges:', `${transportationCharges}/-`]);
    }
    if (parseFloat(taxAmount) > 0) {
    ...
    // Add QR Code
    try {
      const qrCodeElement = document.querySelector(".qr-code-container canvas, .qr-code canvas");
    ...
    if (bankdetails && Object.keys(bankdetails).length > 0) {
      ...
      worksheet.getCell(`B${currentRow}`).value = bankdetails?.bankName || "sample bank";
      currentRow++;
      
      worksheet.getCell(`A${currentRow}`).value = 'Acc No:';
      worksheet.getCell(`B${currentRow}`).value = bankdetails?.accountNumber || "sample Number";
      currentRow++;
      
      worksheet.getCell(`A${currentRow}`).value = 'IFSC Code:';
      worksheet.getCell(`B${currentRow}`).value = bankdetails?.ifscCode || "sample Code";
      currentRow++;
      
      worksheet.getCell(`A${currentRow}`).value = 'Branch:';
      worksheet.getCell(`B${currentRow}`).value = bankdetails?.branch || "sample Branch";
      currentRow += 2;
    }
    
    // Signature Section
    worksheet.getCell(`A${currentRow}`).value = 'Received the Material in Good Condition';
    worksheet.getCell(`H${currentRow}`).value = `For ${companyName}`;
    currentRow += 3;
    ...
    // Footer
    worksheet.mergeCells(`A${currentRow}:J${currentRow}`);
    worksheet.getCell(`A${currentRow}`).value = 'This is Computer Generated No Need To Signature';
    worksheet.getCell(`A${currentRow}`).alignment = { horizontal: 'center' };
    currentRow++;
    ...
    link.href = url;
    link.download = `Invoice_${invoiceNumber || invoiceData?.invoiceNumber || 'download'}.xlsx`;
    link.click();
    window.URL.revokeObjectURL(url);
    
    console.log('Excel file with QR code and HSN Summary exported successfully');
    
  } catch (error) {
    console.error('Excel Export Error:', error);
    alert('Error generating Excel file with QR code and HSN Summary. Please try again.');
  }
};
  ...
  `}
            ...
            <div className="px-2 flex flex-col gap-1">
              ...
              <div className="flex justify-between mb-4">
                <div>
                  ...
                  <p className="text-[#232B77] font-semibold text-sm">
                    Invoice Date: <span className="text-[#027bd1]">{new Date(date).toLocaleDateString('en-GB')}</span>
                ...
                <div>
                  ...
                  {parseFloat(invoiceData?.discount || 0) > 0 && (
                    <div className="flex gap-20 justify-between text-[#232B77] font-semibold text-sm">
                      <p>Discount Amount</p>
                      <p>{parseFloat(invoiceData?.discount).toFixed(2)}/-</p>
                    </div>
                  )}
                  ...
                  {invoiceData?.transportationCharges > 0 && (
                    <div className="flex gap-20 justify-between text-[#232B77] font-semibold text-sm">
                      <p>Transportation Charges</p>
                      <p>{invoiceData?.transportationCharges}/-</p>
                    </div>
                  )}
                  ...
                  <div className="flex gap-20 justify-between text-[#232B77] font-bold text-[16px] mt-2">
                    ...
                    <p>{invoiceData?.totalPayableAmount}/-</p>
                  </div>
                </div>
              </div>
      ...
      <div className="text-center mt-14 gap-2 flex items-center justify-center">
        <button
          ...
          className="bg-blue-500 cursor-pointer hover:bg-green-600 transition-colors text-white px-6 py-2 rounded-lg shadow-md font-medium"
        >
          Download PDF
        </button>
        <button
          onClick={handleExcelExport}
          className="bg-green-500 cursor-pointer hover:bg-blue-600 transition-colors text-white px-6 py-2 rounded-lg shadow-md font-medium"
        >
          Download excel
        </button>
      </div>
