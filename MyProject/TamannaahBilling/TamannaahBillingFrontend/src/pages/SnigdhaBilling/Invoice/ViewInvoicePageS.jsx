...
const ViewInvoicePageS = () => {
  ...
  const handleSelect = (index, suggestion) => {
    ...
    updatedItems[index] = {
      ...
      discountAmount: 0,
      serviceCharge: 0,
      taxRate: suggestion.gst || 0,
      query: suggestion.item_name,
      filteredSuggestions: [],
      id: suggestion.item_id, // Assuming each item has a unique ID
    };
    setItems(updatedItems);
  };
  ...
  const calculateGross = (item) => {
    ...
    if (item) {
      ...
      const tax = ((base - discount + serviceCharge) * item.taxRate) / 100;
      return (base - discount + serviceCharge + tax).toFixed(2);
    }
    ...
    items.forEach((item) => {
      ...
      taxAmount += ((itemSubtotal + serviceCharge) * item.taxRate) / 100;
    });

    const total = subtotal + totalServiceCharge + taxAmount;
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      const completeItems = items.map((item) => {
        ...
        const taxAmount = ((netAmount + serviceCharge) * item.taxRate) / 100;
        ...
        return {
          ...
          taxAmount: taxAmount,
          amount: netAmount + serviceCharge + taxAmount,
      ...
      const totalServiceCharge = completeItems.reduce(
        (sum, item) => sum + (parseFloat(item.serviceCharge) || 0),
        0
      );
        ...
        <div className="mb-6">
          <div className="flex flex-col md:flex-row gap-4 mb-4">
            ...
            <button
              ...
              className={`flex items-center cursor-pointer gap-1 px-4 py-2 ${
                showDeletedInvoices
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg mb-6">
                    ...
                    <div className="overflow-x-auto">
                              ...
                              <td className="p-2 relative">
                                ...
                                {item.filteredSuggestions &&
                                  item.filteredSuggestions.length > 0 && (
                                    <ul className=" w-[300px] bg-white border border-gray-200 rounded-md mt-1 shadow-md z-10 max-h-60 overflow-y-auto">
                      ...
                      <button
                        ...
                        className="bg-green-600 text-white px-4 py-2 rounded hover:bg-orange-500 cursor-pointer"
                      >
                        Add Item
                      </button>
                    </div>
                    ...
                    <div className="w-64 bg-gray-50 p-4 rounded-lg">
                      ...
                      <div className="flex justify-between mb-2">
                        ...
                        <span>
                          ₹
                          {isEditMode
                            ? calculateGross().serviceCharge
                            : selectedInvoice.serviceCharge?.toFixed(2) ||
                              "0.00"}
                        </span>
                      </div>
                      <div className="flex justify-between mb-2">
