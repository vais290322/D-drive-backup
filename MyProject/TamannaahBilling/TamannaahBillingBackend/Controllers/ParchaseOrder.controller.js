...
//     try {
//         // Get all invoices
//         const invoices = await InvoiceMns.find();

//     //    console.log("call form tst", invoices)
...
export const getSalesAnalytics = async (req, res) => {
    try {
        ...
        let totalGoodsValue = 0;
        let totalGST = 0;
        let totalSellingAmount = 0; // This will be the sum of all invoice grandTotals
        let itemsSold = 0;
        
        // First calculate the total selling amount from invoice grandTotals
        for (const invoice of invoices) {
            // Add each invoice's grandTotal to the total selling amount
            totalSellingAmount += Number(invoice.grandTotal) || 0;
        }
        
        // Process each invoice for other analytics
        for (const invoice of invoices) {
            if (invoice.items && Array.isArray(invoice.items)) {
                // Process each item in the invoice
                for (const item of invoice.items) {
                    ...
                    const gstRate = (cgst + sgst + igst) / 100;
                    const itemGST = itemSales * gstRate;
                    totalGST += itemGST;
        ...
        const analyticsData = {
            ...
            grandTotal: parseFloat((totalSales + totalGST).toFixed(2)),
            totalProfit: parseFloat(totalProfit.toFixed(2)),
            totalGoodsValue: parseFloat(totalGoodsValue.toFixed(2)),
            profitMargin: parseFloat(((totalProfit / totalSales) * 100).toFixed(2)),
            totalInvoices: invoices.length,
...
export const getItemSalesAnalytics = async (req, res) => {
    try {
        ...
        for (const invoice of invoices) {
            if (invoice.items && Array.isArray(invoice.items)) {
                ...
                for (const item of invoice.items) {
                    ...
                    const gstRate = (cgst + sgst + igst) / 100;
                    const itemGST = itemSales * gstRate;
                    const itemGrandTotal = itemSales + itemGST;
                    ...
                    if (itemAnalytics.has(itemId)) {
                        ...
                        existingData.totalGoodsValue += itemGoodsValue;
                        existingData.totalProfit += itemProfit;
                        existingData.totalGST += itemGST;
                        existingData.grandTotal += itemGrandTotal;
                        existingData.invoiceCount += 1;
                    } else {
                        itemAnalytics.set(itemId, {
                            itemId,
        ...
        const itemsAnalyticsArray = Array.from(itemAnalytics.values()).map(item => ({
            ...
            grandTotal: parseFloat(item.grandTotal.toFixed(2)),
            totalProfit: parseFloat(item.totalProfit.toFixed(2)),
            totalGoodsValue: parseFloat(item.totalGoodsValue.toFixed(2)),
            profitMargin: parseFloat(((item.totalProfit / item.totalSales) * 100).toFixed(2))
        }));
