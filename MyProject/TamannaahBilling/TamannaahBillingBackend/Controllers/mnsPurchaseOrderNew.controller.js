...
export const getAllPurchaseOrders = async (req, res) => {
  try {
    const purchaseOrders = await MnsPurchaseOrderNew.find().sort({createdAt: -1});
...
export const updatePurchaseOrder = async (req, res) => {
  ...
  try {
    ...
    const newTotalPurchase = Number(existingItem.totalPurchase || 0) - originalItem.quantity;
      ...
      for (const newItem of updateData.items) {
        ...
        if (existingItem) {
          ...
          const newTotalPrice =
            ...
            const newTotalPurchase = Number(existingItem.totalPurchase || 0) + newItem.quantity;
...
export const deletePurchaseOrder = async (req, res) => {
  ...
  try {
    ...
    for (const item of purchaseOrder.items) {
      ...
      const existingItem = await Item.findOne({item_id: item.item_id}).session(
        session
      );
      ...
      if (existingItem) {
        ...
        const newTotalPurchase = Number(existingItem.totalPurchase || 0) - item.quantity;
