    ...
    for (const item of holdingItems) {
      ...
      if (!success) {
        return res
          ...
          .json({
            success: false,
            message: `Failed to add item: ${item.item_id}`,
          });
      }
    }
...
// change model to items model form current model for new update 
...
export const getItemsByHsnCode = async (req, res) => {
  try {
    ...
    const items = await Item.find({ hsnCode });
...
export const transferItemsByHsnCode = async (req, res) => {
  try {
    ...
    const regularItem = await Item.findOne({ hsnCode });
    ...
    let snigdhaItem = await SnigdhaItem.findOne({ hsnCode });
