  ...
  const saveTutionFees = async (e) => {
    ...
    try {
      const response = await axios.put(
        `${accountApi}/student-admission-fee/add-tuition-payment/${currentStudent?.id}`,
        input,
        {
          headers: {
      ...
      if (response) {
        ...
        const schoolDetails = {
          ...
          name: `${schoolInfo[0]?.schoolName}`,
          // address: "123 Main Street, City, State - 123456",
          address: `${schoolInfo[0]?.schoolAddress}`,
          contact: `Phone: +91 ${schoolInfo[0]?.schoolPhone} | Email: ${schoolInfo[0]?.schoolEmail}`,
        };
  ...
  const fetchAdmissionFeeDone = async () => {
    try {
      const response = await axios.get(
        `${accountApi}/student-admission-fee/all`
      );
