...
const StudentFeeCollectionPage = () => {
  ...
  // console.log("schoolInfo : ", schoolInfo[4]);
  ...
  const fetchClassFeeStructure = async () => {
    try {
      const response = await axios.get(
        `${accountApi}/api/fees-structure/${currentStudent?.className}`
      );
  ...
  const handleSavePayment = async (
    ...
    try {
      ...
      const response = await axios.post(
        `${accountApi}/student-admission-fee/create`,
        payload,
        {
          headers: {
  ...
  const generatePaymentSlip = (student, feeStructure, transactionNumber, paymentMethod, schoolInfo) => {
    ...
    slipElement.innerHTML = `
      <div style="text-align: center; margin-bottom: 20px;">
        <h2>${schoolInfo[0]?.schoolName}</h2>
        <p>${schoolInfo[0]?.schoolAddress}</p>
      </div>
      ...
      <div style="margin-top: 20px; text-align: center;">
        <p></p>For any queries, please contact ${schoolInfo[0]?.schoolPhone} ${schoolInfo[0]?.schoolEmail}</p>
      </div>
