...
const AddBookCategoryPage = () => {
    ...
    const updateBookCategoryData = async (e, id) => {
        ...
        try {
          ...
          const response = await axios.post(
            `${libraryUrlApi.addBookCategory.url}/${id}`,
            editInput,
            {
              headers: {
  ...
  const fetchLibraryCategoryData = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.get(`${libraryUrlApi.addBookCategory.url}`);
  ...
  const addNewBookCategory = async (e) => {
    ...
    try {
      setPostApiLoading(true);
      const response = await axios.post(`${libraryUrlApi.addBookCategory.url}`, input, {
        headers: {
          "Content-Type": "application/json",
        },
                ...
                >
                  <div className="flex justify-end items-center gap-2">
                    ...
                    <DeleteComponent
                      ...
                      deletePath={`${libraryUrlApi.addBookCategory.url}/${item.id}`} // API endpoint for deletion
                      onDelete={() => {
