// import { getAllHoldingItems } from "../HelperFunctions/helperfunctions.js";
...
export const setHoldingItems = async (item) => {
    try {
      ...
      sgst,
        ...
        const newItem = new HoldingItems({
            item_name,
            inventory_id: _id,
            item_id,
            unit_prize,
...
export const deleteHoldingItem = async (itemId) => {
    try {
        ...
        const item = await HoldingItems.findOne({ inventory_id:itemId });
  
        if (!item) {
          return { success: false, message: "Inventory item not found" };
        }
       const result =  await HoldingItems.deleteOne({ inventory_id:itemId});
        if(!result){
...
export const getAllHoldingData = async (req, res) => {
    try {
        const allHoldingData = await HoldingItems.find(); // Fetch all holding items
...
export const updateHoldingItem = async (itemId, updatedDetails) => {
    try {
        ...
        const item = await HoldingItems.findOneAndUpdate({ inventory_id:itemId }, updatedDetails, {
...
export const deleteAllHoldingData = async () => {
    try {
        ...
        const result = await HoldingItems.deleteMany({});
