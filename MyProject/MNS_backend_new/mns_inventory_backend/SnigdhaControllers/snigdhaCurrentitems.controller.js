...
import {deleteAllHoldingData} from "./snigdhaHoldingItems.controller.js"
import Invoice from "../invoicemodel/invoice.model.js"

export const addAllFromHoldingItems = async (req, res) => {
  try {
    const holdingItems = await SnigdhaHoldingItems.find();
...
export const setCurrentData = async (item) => {
  try {
    ...
    const newData = new SnigdhaCurrentItems({
...
export const updateCurrentData = async (item) => {
  try {
    ...
    const updatedItem = await SnigdhaCurrentItems.findOneAndUpdate(
...
export const getAllCurrentItems = async (req, res) => {
  try {
    ...
    const items = await SnigdhaCurrentItems.find();
...
export const addOneCurrentItemFromHoldingItems = async (req, res) => {
  try {
    const { id } = req.params;
    const holdingItem = await SnigdhaHoldingItems.findOne({ item_id: id });
    if (!holdingItem) {
      return res
        .status(404)
    ...
    const currentItem = await SnigdhaCurrentItems.findOne({ item_id: id });
    ...
    if (!currentItem) {
      ...
      await SnigdhaHoldingItems.deleteOne({ item_id: id });
      return res
        .status(200)
        .json({
          success: true,
...
export const sellCurrentItem = async (req, res) => {
  try {
    ...
    const sellingItem = await SnigdhaCurrentItems.findOne({ item_id: sellingItemId });
    ...
    if (sellingItemQuantity === sellingItem.quantity) {
      const isDeleted = await SnigdhaCurrentItems.deleteOne({
...
export const getItemById = async (req, res) => {
  try {
    ...
    const item = await SnigdhaCurrentItems.findOne({ item_id });
...
export const getItemDetailsAndPurchaseOrders = async (req, res) => {
  try {
    ...
    const purchaseOrders = await Invoice.find({
      "items.id": item_id
    });
...
export const getItemsByHsnCode = async (req, res) => {
  try {
    ...
    const items = await SnigdhaCurrentItems.find({ hsnCode });
...
export const transferItemsByHsnCode = async (req, res) => {
  try {
    ...
    // Find items with the matching HSN code in Snigdha inventory
    const snigdhaItem = await SnigdhaCurrentItems.findOne({ hsnCode });
    ...
    if (!snigdhaItem) {
      return res
        ...
        .json({ 
          success: false, 
          message: `No items found with HSN Code: ${hsnCode} in Snigdha inventory`
        });
    }
    
    // Check if there's enough quantity in Snigdha inventory
    if (transferQuantity > snigdhaItem.quantity) {
      return res
        .status(400)
        .json({ 
          success: false, 
          message: `Not enough quantity in Snigdha inventory. Available: ${snigdhaItem.quantity}, Requested: ${transferQuantity}`
        });
    }
    
    // Check if the item exists in regular inventory
    let regularItem = await CurrentItems.findOne({ hsnCode });
    
    // If the item doesn't exist in regular inventory, create it
    if (!regularItem) {
      regularItem = new CurrentItems({
        item_name: snigdhaItem.item_name,
        inventory_id: snigdhaItem.inventory_id,
        item_id: snigdhaItem.item_id,
        unit_prize: snigdhaItem.unit_prize,
        sellingPrice: snigdhaItem.sellingPrice,
        total_prize: snigdhaItem.unit_prize * transferQuantity,
        quantity: transferQuantity,
        seller_details: snigdhaItem.seller_details,
        buyier_details: snigdhaItem.buyier_details,
        imported: snigdhaItem.imported,
        exported: snigdhaItem.exported,
        hsnCode: snigdhaItem.hsnCode,
        cgst: snigdhaItem.cgst,
        igst: snigdhaItem.igst,
        sgst: snigdhaItem.sgst,
      });
    ...
    // Update Snigdha inventory
    // If transferring all quantity, delete the item
    if (transferQuantity === snigdhaItem.quantity) {
      await SnigdhaCurrentItems.deleteOne({ _id: snigdhaItem._id });
    } else {
    ...
    // Create a purchase order record in Snigdha purchase model
    ...
    const totalAmount = snigdhaItem.unit_prize * transferQuantity;
    
    const purchaseOrder = new SnigdhaPurchaseOrder({
      date: formattedDate,
      vendorQuoteRef: `Transfer-${Date.now()}`,
      buyer: "MNS Inventory",
      requestedBy: "System Transfer",
      contactPerson: "MNS Admin",
      contactPersonNumber: "N/A",
      department: "Inventory Management",
      poDescription: `Transfer of ${transferQuantity} units of item with HSN Code: ${hsnCode} to MNS inventory`,
      billLocation: "MNS Warehouse",
      shipLocation: "MNS Warehouse",
      items: [itemForPurchase],
      companyDetails: {
        name: "MNS Inventory",
        address: "MNS Warehouse Address",
        contactNumber: "N/A"
      },
    ...
    return res
      ...
      .json({
        success: true,
        message: `Successfully transferred ${transferQuantity} units of item with HSN Code: ${hsnCode} from Snigdha inventory to regular inventory`,
        data: {
          snigdhaItem: snigdhaItem.quantity > 0 ? snigdhaItem : "Item removed from Snigdha inventory",
          regularItem,
          purchaseOrder
        }
      });
