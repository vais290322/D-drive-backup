        ...
        .on('end', async () => {
          ...
          for (const row of results) {
            try {
              ...
              const existingItem = await SnigdhaItem.findOne({ item_id: row.item_id });
              if (existingItem) {
              ...
              const newItem = new SnigdhaItem({
                ...
                quantity: row.quantity || row.openningStock || 1,
                hsnCode: row.hsnCode || '',
                gst: row.gst || 0,
                uom: row.uom,
  ...
  downloadItemTemplate(req, res) {
    try {
      const templateContent = 
        ...
        'Sample Item,ITEM001,Sample description,100,120,100,1,100,12345,9,pics\n';
...
export default new SnigdhaBulkController();
