...
export const createMoneyTransfer = async (req, res) => {
  ...
  try {
    ...
    // Find source bank
    const sourceBank = await SnigdhaBank.findById(transferFromBankId).session(session);
    if (!sourceBank) {
    ...
    const destinationBank = await SnigdhaBank.findById(transferToBankId).session(session);
    if (!destinationBank) {
      await session.abortTransaction();
      session.endSession();
      return res.status(404).json({
    ...
    const newMoneyTransfer = new SnigdhaMoneyTransfer({
      date: new Date(date),
      transferFromBankId,
      transferToBankId,
      transferAmount,
    ...
    const populatedTransfer = await SnigdhaMoneyTransfer.findById(newMoneyTransfer._id)
...
export const getAllMoneyTransfers = async (req, res) => {
  try {
    const moneyTransfers = await SnigdhaMoneyTransfer.find()
      .populate('transferFromBankId')
      .populate('transferToBankId')
      .sort({ date: -1 });
...
export const getMoneyTransferById = async (req, res) => {
  try {
    ...
    const moneyTransfer = await SnigdhaMoneyTransfer.findById(id)
      .populate('transferFromBankId')
      .populate('transferToBankId');
