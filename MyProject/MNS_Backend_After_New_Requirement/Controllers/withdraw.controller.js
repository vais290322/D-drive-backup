...
export const getWithdrawById = async (req, res) => {
  try {
    ...
    const withdraw = await Withdraw.findById(id).populate("bankId");
...
export const updateWithdraw = async (req, res) => {
  try {
    ...
    if (updateData.amount || updateData.bankId) {
      // Get original bank
      const originalBank = await Bank.findById(withdraw.bankId);
      console.log("original bank : ", originalBank);
      ...
      originalBank.currentAmount += Number(withdraw.amount);
      originalBank.deductionAmount -= Number(withdraw.amount);
      ...
      if (updateData.bankId && updateData.bankId !== withdraw.bankId.toString()) {
        ...
        targetBank = await Bank.findById(updateData.bankId);
        if (!targetBank) {
          return res.status(404).json({
            success: false,
            statusCode: 404,
      ...
      const newAmount = updateData.amount ? Number(updateData.amount) : Number(withdraw.amount);

      // Check if target bank has sufficient balance for the new amount
      if (targetBank.currentAmount < newAmount) {
      ...
      targetBank.currentAmount -= newAmount;
      targetBank.deductionAmount += newAmount;
      await targetBank.save();
    }

    // Update withdrawal
    const updatedWithdraw = await Withdraw.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    }).populate("bankId");
...
export const deleteWithdraw = async (req, res) => {
  try {
    ...
    const bank = await Bank.findById(withdraw.bankId);
    if (bank) {
      bank.currentAmount += Number(withdraw.amount);
      bank.deductionAmount -= Number(withdraw.amount);
      await bank.save();
    }
...
export const getWithdrawsByBankId = async (req, res) => {
  try {
    ...
    const withdraws = await Withdraw.find({ bankId })
      .populate("bankId")
      .sort({ createdAt: -1 });
...
export const getWithdrawsByType = async (req, res) => {
  try {
    ...
    const withdraws = await Withdraw.find({ transactionType: type })
      .populate("bankId")
      .sort({ createdAt: -1 });
