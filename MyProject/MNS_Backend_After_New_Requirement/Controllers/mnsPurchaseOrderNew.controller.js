...
export const createPurchaseOrder = async (req, res) => {
    ...
    try {
        ...
        for (const purchaseItem of purchaseOrderData.items) {
            ...
            const existingItem = await Item.findOne({ item_id: purchaseItem.item_id }).session(session);
        ...
        const newPurchaseOrder = await MnsPurchaseOrderNew.create([purchaseOrderData], { session });
...
export const updatePurchaseOrder = async (req, res) => {
    ...
    try {
        ...
        const originalPurchaseOrder = await MnsPurchaseOrderNew.findById(id).session(session);
        ...
        if (updateData.items && Array.isArray(updateData.items)) {
            ...
            for (const originalItem of originalPurchaseOrder.items) {
                const existingItem = await Item.findOne({ item_id: originalItem.item_id }).session(session);
            ...
            for (const newItem of updateData.items) {
                ...
                const existingItem = await Item.findOne({ item_id: newItem.item_id }).session(session);
...
export const deletePurchaseOrder = async (req, res) => {
    ...
    try {
        ...
        const purchaseOrder = await MnsPurchaseOrderNew.findById(id).session(session);
        ...
        for (const item of purchaseOrder.items) {
            ...
            const existingItem = await Item.findOne({ item_id: item.item_id }).session(session);
