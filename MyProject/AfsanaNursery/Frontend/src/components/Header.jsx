...
const Header = () => {
  ...
  return (
    <>
      <header className="h-16 shadow-md bg-gradient-to-r from-green-100 to-yellow-100 fixed w-full z-40">
        <div className="h-full container mx-auto flex items-center px-4 justify-between">
          {/* Logo */}
          <div className="flex items-center">
            <Link to={"/"} className="flex items-center">
              <img src={Afsana_Logo} alt="logo" className="h-16 w-16 mr-2 rounded-full " />
              <span className="font-bold text-emerald-800 text-lg hidden sm:block">Afsana Nursery</span>
            </Link>
          </div>
              ...
              {user?._id ? (
                ...
                >
                  {user?.profilePic ? (
                    ...
                    <div className="w-9 h-9 rounded-full bg-gradient-to-r from-green-100 to-yellow-100 flex items-center justify-center font-bold text-xl border-2 border-green-500 text-green-800">
                ...
                <Link
                  to={"/login"}
                 className="flex items-center gap-1 px-4 py-2 rounded-full text-white bg-gradient-to-r from-green-600 to-yellow-500 hover:from-green-700 hover:to-yellow-600 transition-colors shadow-sm"
                >
                  <FaUserCircle />
...
{menuDisplay && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-2 z-50 border border-green-100">
                  ...
                  <nav className="mt-2">
                    {user?.role === ROLE.ADMIN && (
                      <Link
                        ...
                        className="flex items-center px-4 py-2 text-gray-700 hover:bg-gradient-to-r hover:from-green-50 hover:to-yellow-50 hover:text-green-700"
                        onClick={() => setMenuDisplay(false)}
                      >
                        <FaLeaf className="mr-2 text-yellow-500" />
                        Admin Panel
                      </Link>
                    )}
                    ...
                    <button
                      ...
                      className="flex items-center w-full text-left px-4 py-2 text-gray-700 hover:bg-gradient-to-r hover:from-green-50 hover:to-yellow-50 hover:text-green-700"
                    >
                      <FaLeaf className="mr-2 text-yellow-500" />
                      Logout
                    </button>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white shadow-[0_-2px_10px_rgba(0,0,0,0.1)] z-40">
        <div className="flex justify-around items-center h-14">
          <Link to="/" className="flex flex-col items-center justify-center text-yellow-500 hover:text-emerald-600">
            <FaLeaf />
            <span className="text-xs mt-1">Home</span>
          </Link>
          <Link to="/product-category" className="flex flex-col items-center justify-center text-yellow-500 hover:text-emerald-600">
            <FaLeaf />
            <span className="text-xs mt-1">Products</span>
          </Link>
          <Link to="/about" className="flex flex-col items-center justify-center text-yellow-500 hover:text-emerald-600">
            <FaLeaf />
            <span className="text-xs mt-1">About</span>
          </Link>
          <Link to="/contact" className="flex flex-col items-center justify-center text-yellow-500 hover:text-emerald-600">
            <FaLeaf />
