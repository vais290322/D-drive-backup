...
const TableHeader = React.forwardRef(({ className, ...props }, ref) => (
  <thead 
    ref={ref} 
    className={cn("bg-gradient-to-r from-gray-50 to-emerald-50/30 sticky top-0 z-10", className)} 
    {...props} 
  />
));
...
const TableHead = React.forwardRef(({ className, ...props }, ref) => (
  <th
    ref={ref}
    className={cn(
      "h-12 px-6 text-left align-middle font-bold text-gray-700 [&:has([role=checkbox])]:pr-0 tracking-wide text-[8px] sm:text-xs uppercase",
      className
    )}
    {...props}
  />
