...
import { uploadImageForLoanCustomer } from "@/services/UploadPhotoForLoanCustomer";
...
type UploadedFile = { fileUrl: string; fileId: string } | null;
...
export default function CustomerForm() {
  ...
  const [voterIdCardFile, setVoterIdCardFile] = useState<File | null>(null);
  const [drivingLicenseCardFile, setDrivingLicenseCardFile] =
    useState<File | null>(null);
  const [otherDocImageFile, setOtherDocImageFile] = useState<File | null>(null);
  const [showCamera, setShowCamera] = useState(false);

const [uploadedFiles, setUploadedFiles] = useState<Record<string, any>>({});

  // console.log("customerPhoto : ", customerPhotoFile);
  ...
  const [drivingLicenseCard, setDrivingLicenseCard] = useState<string | null>(
    null,
  );
  const [otherDocImage, setOtherDocImage] = useState<string | null>(null);

  const [photoFileId, setPhotoFileId] = useState<string | null>(null);
const [aadhaarFrontFileId, setAadhaarFrontFileId] = useState<string | null>(null);
const [aadhaarBackFileId, setAadhaarBackFileId] = useState<string | null>(null);
const [panFileId, setPanFileId] = useState<string | null>(null);
const [voterFileId, setVoterFileId] = useState<string | null>(null);
const [licenseFileId, setLicenseFileId] = useState<string | null>(null);
const [otherDocFileId, setOtherDocFileId] = useState<string | null>(null);

  const uploadOnce = async (file: File | null, key: string) => {
  try {
    if (!file) return [];
    ...
    const res = await uploadImageForLoanCustomer(file);
    ...
    // store fileId for delete later
    if (key === "photo") setPhotoFileId(obj.fileId);
    if (key === "aadhaarFront") setAadhaarFrontFileId(obj.fileId);
    if (key === "aadhaarBack") setAadhaarBackFileId(obj.fileId);
    if (key === "panCard") setPanFileId(obj.fileId);
    if (key === "voter") setVoterFileId(obj.fileId);
    if (key === "license") setLicenseFileId(obj.fileId);
    if (key === "otherDoc") setOtherDocFileId(obj.fileId);
      ...
      if (customer) {
        form.reset({
          ...
          manual_loan_details: {
            ...
            agreement_number:
              customer.manual_loan_details?.agreement_number || "",
            item_name: customer.manual_loan_details?.item_name || "",
            item_serial_number:
              customer.manual_loan_details?.item_serial_number || "",
            item_description:
              customer.manual_loan_details?.item_description || "",
            ...
            emi_start_date: customer.manual_loan_details?.emi_start_date
              ? new Date(customer.manual_loan_details.emi_start_date)
                  .toISOString()
                  .split("T")[0]
              : "",
            emi_end_date: customer.manual_loan_details?.emi_end_date
              ? new Date(customer.manual_loan_details.emi_end_date)
                  .toISOString()
                  .split("T")[0]
              : "",
        ...
        setCustomerPhoto(customer.photo_url?.[0]?.fileUrl);
        setAadhaarFront(customer.aadhaar_front_url?.[0]?.fileUrl);
        setAadhaarBack(customer.aadhaar_back_url?.[0]?.fileUrl);
        setPanCard(customer.pan_card_url?.[0]?.fileUrl);
        setVoterIdCard(customer.voter_id_url?.[0]?.fileUrl);
        setDrivingLicenseCard(customer.driving_license_url?.[0]?.fileUrl);
        setOtherDocImage(customer.other_doc_url?.[0]?.fileUrl);

        setPhotoFileId(customer.photo_url?.[0]?.fileId || null);
setAadhaarFrontFileId(customer.aadhaar_front_url?.[0]?.fileId || null);
setAadhaarBackFileId(customer.aadhaar_back_url?.[0]?.fileId || null);
setPanFileId(customer.pan_card_url?.[0]?.fileId || null);
setVoterFileId(customer.voter_id_url?.[0]?.fileId || null);
setLicenseFileId(customer.driving_license_url?.[0]?.fileId || null);
setOtherDocFileId(customer.other_doc_url?.[0]?.fileId || null);
      }
    } catch (error) {
  ...
  try {
    ...
    setPhotoFileId(null); // old image replaced
      ...
      reader.onloadend = () => {
        ...
        switch (type) {
          ...
          case "pan":
            ...
            break;
          case "voter":
            setVoterIdCard(result);
            setVoterIdCardFile(file);
            break;
          case "license":
            setDrivingLicenseCard(result);
            setDrivingLicenseCardFile(file);
            break;
          case "other_doc":
            setOtherDocImage(result);
            setOtherDocImageFile(file);
            break;
        }
...
const removeImage = async (type: string) => {
  try {
    ...
    switch (type) {
      ...
      case "aadhaar_front":
        fileId = aadhaarFrontFileId;
        setAadhaarFront(null);
        setAadhaarFrontFile(null);
        setAadhaarFrontFileId(null);
        break;

      case "aadhaar_back":
        fileId = aadhaarBackFileId;
        setAadhaarBack(null);
        setAadhaarBackFile(null);
        setAadhaarBackFileId(null);
        break;
      ...
      case "voter":
        fileId = voterFileId;
        setVoterIdCard(null);
        setVoterIdCardFile(null);
        setVoterFileId(null);
        break;

      case "license":
        fileId = licenseFileId;
        setDrivingLicenseCard(null);
        setDrivingLicenseCardFile(null);
        setLicenseFileId(null);
        break;

      case "other_doc":
        fileId = otherDocFileId;
        setOtherDocImage(null);
        setOtherDocImageFile(null);
        setOtherDocFileId(null);
        break;
    }
 ...
 const onSubmit = async (data: CustomerFormData) => {
  ...
  if (customer) {
    ...
    const [
      ...
      panCardObj,
      voterIdCardObj,
      drivingLicenseCardObj,
      otherDocImageObj,
    ] = await Promise.all([
      customerPhotoFile
        ? uploadOnce(customerPhotoFile, "photo")
        : customer.photo_url || [],

      aadhaarFrontFile
        ? uploadOnce(aadhaarFrontFile, "aadhaarFront")
        : customer.aadhaar_front_url || [],

      aadhaarBackFile
        ? uploadOnce(aadhaarBackFile, "aadhaarBack")
        : customer.aadhaar_back_url || [],

      panCardFile
        ? uploadOnce(panCardFile, "panCard")
        : customer.pan_card_url || [],

      voterIdCardFile
        ? uploadOnce(voterIdCardFile, "voter")
        : customer.voter_id_url || [],

      drivingLicenseCardFile
        ? uploadOnce(drivingLicenseCardFile, "license")
        : customer.driving_license_url || [],

      otherDocImageFile
        ? uploadOnce(otherDocImageFile, "otherDoc")
    ...
    await api.customers.update(id, {
      ...
      photo_url: photoArr,
      aadhaar_front_url: aadhaarFrontObj,
      aadhaar_back_url: aadhaarBackObj,
      pan_card_url: panCardObj,
      voter_id_url: voterIdCardObj,
      driving_license_url: drivingLicenseCardObj,
      other_doc_url: otherDocImageObj,
      ...
      ...(data.add_loan
        ? {
            manual_loan_details: {
              ...
              emi_start_date:
                data.manual_loan_details?.emi_start_date || undefined,
              emi_end_date:
                data.manual_loan_details?.emi_end_date || undefined,
            },
          }
        : {}),
    });
  ...
  toast.success("Customer updated successfully");
      ...
      await api.customers.create(
        ...
        data.customer_code_type === "manual"
          ? data.customer_code_manual
          : undefined
      );
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <Tabs defaultValue="basic" className="w-full">
            <TabsList className="h-auto flex flex-wrap w-full md:grid md:grid-cols-5 gap-1">
              <TabsTrigger value="basic">Basic Info</TabsTrigger>
              <TabsTrigger value="address">Address</TabsTrigger>
              <TabsTrigger value="kyc">KYC Details</TabsTrigger>
              <TabsTrigger value="bank">Bank Details</TabsTrigger>
              <TabsTrigger value="loan">Product Details</TabsTrigger>
            </TabsList>
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input
                                {...field}
                                placeholder="e.g. Ration Card, Utility Bill"
                              />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
