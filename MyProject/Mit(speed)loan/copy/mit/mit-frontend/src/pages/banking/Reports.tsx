    ...
    return (
      <div className="p-3 sm:p-6 xl:p-8">
        <div className="mb-4 sm:mb-6">
          <h1 className="text-xl sm:text-2xl xl:text-3xl font-bold">Banking Reports</h1>
          <p className="text-xs sm:text-sm text-muted-foreground mt-1">Loading reports...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-3 sm:p-6 xl:p-8">
      ...
      <div className="mb-4 sm:mb-6 print:hidden">
        <Button variant="ghost" size="sm" onClick={() => navigate('/banking')} className="mb-3">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-xl sm:text-2xl xl:text-3xl font-bold">Banking Reports</h1>
            <p className="text-xs sm:text-sm text-muted-foreground mt-1">Analytical insights and summaries</p>
          </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
          ...
          <Card>
            ...
            <CardContent className="px-3 pb-3">
              <div className="text-lg sm:text-2xl font-bold">{formatCurrency(stats.total_balance)}</div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2 px-3 pt-3">
              ...
              <TrendingUp className="h-4 w-4 sm:h-5 sm:w-5 text-success shrink-0" />
            </CardHeader>
            <CardContent className="px-3 pb-3">
      ...
      <Tabs defaultValue="daily" className="space-y-4 sm:space-y-6">
        ...
        <TabsList className="print:hidden w-full sm:w-auto overflow-x-auto flex">
          <TabsTrigger value="daily" className="flex-1 sm:flex-none text-xs sm:text-sm">Daily Summary</TabsTrigger>
          <TabsTrigger value="customers" className="flex-1 sm:flex-none text-xs sm:text-sm">Customer Balances</TabsTrigger>
          <TabsTrigger value="cashflow" className="flex-1 sm:flex-none text-xs sm:text-sm">Cash Flow</TabsTrigger>
        </TabsList>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:items-end">
                <div className="flex-1">
                  ...
                  <Input
                    id="start_date"
                    type="date"
                    className="h-8 text-xs sm:text-sm mt-1"
                ...
                <Button size="sm" className="w-full sm:w-auto" onClick={handleDateRangeChange} disabled={loading}>
                  Update
                </Button>
              </div>
            </CardContent>
                    ...
                    <TableBody>
                      {dailySummary.map((summary) => (
                        <TableRow key={summary.date}>
                          ...
                          <TableCell
                            className={`text-right font-semibold ${summary.net_change >= 0 ? 'text-success' : 'text-destructive'
                              }`}
                          >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <Card>
                      <CardContent className="pt-6">
                        ...
                        <p
                          className={`text-2xl font-bold ${stats.total_deposits_today - stats.total_withdrawals_today >= 0
                              ? 'text-success'
                              : 'text-destructive'
                            }`}
                        >
