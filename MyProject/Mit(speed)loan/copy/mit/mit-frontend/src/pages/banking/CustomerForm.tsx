...
import { ArrowLeft, Upload, X, User, Camera } from 'lucide-react';
...
import { uploadImageForBankingCustomer } from '@/services/UploadPhotoForBankingCustomer';
...
export default function CustomerForm() {
  ...
  const loadCustomer = async () => {
    ...
    try {
      ...
      if (customer) {
        ...
        if (customer.photo_url?.[0]) {
          setPhotoPreview(customer.photo_url[0].fileUrl);
          setPhotoFileId(customer.photo_url[0].fileId);

          setFormData((prev) => ({
            ...prev,
            photo_url: customer.photo_url[0].fileUrl,
          }));
        }
      }
    } catch (error) {
  ...
  const handleCameraCapture = async (imageDataUrl: string) => {
    try {
      ...
      setPhotoFile(file);
      setPhotoFileId(null); // old file replaced
  ...
  const handleRemovePhoto = async () => {
    console.log("photo id : ",photoFileId)
    try {
      if (photoFileId ) {
        // console.log("photo id : ",photoFileId)
        await deleteImageFromBucket(photoFileId);
         setPhotoFileId(null)
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let uploadedFileId = photoFileId;
      ...
      if (photoFile) {
        ...
        try {
          const res = await uploadImageForBankingCustomer(photoFile);
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          const autoAccount = `AC${randomNum}`;
                          setFormData({ ...formData, account_number: autoAccount });
                        }}
                      >
                        Auto Generate
                      </Button>
