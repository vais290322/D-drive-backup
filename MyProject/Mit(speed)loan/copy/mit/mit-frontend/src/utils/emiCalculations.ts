...
export function calculateEMI(
  ...
  if (interestType === 'flat') {
    ...
    if (loanType === 'daily') {
      ...
      totalInterest = (loanAmount * annualRate * tenure) / (365 * 100);
    } else if (loanType === 'weekly') {
      // For weekly loans: tenure is in weeks
      totalInterest = (loanAmount * annualRate * tenure) / (52 * 100);
    } else {
      // For monthly loans: tenure is in months
      totalInterest = (loanAmount * annualRate * tenure) / (12 * 100);
    }
    ...
    const effectivePrincipal = loanAmount + processingFee + insuranceFee;
...
export function calculateTotalInterest(
  ...
  if (interestType === 'flat') {
    ...
    // For reducing balance, calculate EMI logic including fees
    const emiAmount = calculateEMI(principal, annualRate, tenure, interestType, loanType, processingFee, insuranceFee, 0);
    const totalPayable = emiAmount * tenure;
    // Interest = Total Paid - Principal - Fees
    return Math.max(0, totalPayable - principal - processingFee - insuranceFee);
  }
}
