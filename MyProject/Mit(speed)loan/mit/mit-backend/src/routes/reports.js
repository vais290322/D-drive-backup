    ...
    try {
        ...
        customersWithManualLoans.forEach(cust => {
            if (cust.manual_loan_details && cust.manual_loan_details.loan_amount) {
                ...
                if (details.emi_start_date && details.emi_amount) {
                    ...
                    // Cap months passed at total tenure (if known) or just ensure outstanding >= 0
                    // If we have end date, we can know max months
                    let maxMonths = 1000;
                    if (details.emi_end_date) {
                        const end = new Date(details.emi_end_date);
                        // if now > end, loan is closed, outstanding 0
                        if (now > end) {
                            // Zero outstanding for closed loans
                            return;
                        }
                    }

                    const estimatedPaid = details.emi_amount * monthsPassed;
                    const estimatedOutstanding = Math.max(0, amount - estimatedPaid);
                    totalOutstanding += estimatedOutstanding;
                } else {
                    // If no EMI details, assume full amount is outstanding if active
                    // We roughly check if 'active' from the previous loop logic or just add it
                    // Let's add full amount if we can't estimate, unless it looks old?
                    // Safer to add full amount to ensure we don't under-report debt.
