...
router.post('/', auth, async (req, res) => {
  try {
    ...
    // If customer_code_manual is provided, use it as customer_code
    if (data.customer_code_manual) {
      data.customer_code = data.customer_code_manual;
      delete data.customer_code_manual;
    }

    // Remove customer_code_type from data as it's not in the schema
    delete data.customer_code_type;
    ...
    // Send more detailed error for duplicate customer codes
    if (err.code === 11000) {
...
router.get('/:id/calls', auth, async (req, res) => {
  try {
    const logs = await CallLog.find({ customer_id: req.params.id }).sort({ date: -1 });
    res.json(logs);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
...
router.post('/:id/calls', auth, async (req, res) => {
  try {
    const { date, next_followup_date, feedback } = req.body;
    ...
    // Update Customer's last call info for quick access
    await Customer.findByIdAndUpdate(req.params.id, {
      last_call_date: date || new Date(),
      next_followup_date,
      feedback
    });
...
router.get('/search/:value', auth, async (req, res) => {
  try {
    ...
    const customers = await Customer.find({
      $or: [
        { customer_code: { $regex: value, $options: 'i' } },
        { full_name: { $regex: value, $options: 'i' } },
        { mobile_primary: { $regex: value, $options: 'i' } }
      ]
    });
    res.json(customers);
  } catch (err) {
