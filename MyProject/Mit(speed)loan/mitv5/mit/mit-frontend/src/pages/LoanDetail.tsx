...
export default function LoanDetail() {
  ...
  const [qrData, setQrData] = useState<{ qr_id: string; image_url: string; mock?: boolean } | null>(null);
  ...
  const handleShowQR = async () => {
    ...
    try {
      ...
      if (!qrData) {
        const response = await api.payments.getLoanQR(id);
        setQrData(response);
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold">{ledger.loan.loan_id}</h1>
          ...
          <Dialog open={qrDialogOpen} onOpenChange={setQrDialogOpen}>
            ...
            <DialogContent className="sm:max-w-lg">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-center bg-gradient-to-br from-primary/5 to-accent/5 p-8 rounded-lg">
                  {qrData ? (
                    <div className="text-center">
                      <img
                        src={qrData.image_url}
                        alt="Payment QR Code"
                        className="w-72 h-72 mx-auto rounded-lg shadow-lg border-4 border-white bg-white"
                      />
                    </div>
                  ) : (
                ...
                <div className="bg-muted/50 rounded-lg p-4 space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-semibold">{ledger.loan.loan_id}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">Customer:</span>
                    <span className="text-sm font-semibold">{ledger.loan.customer?.full_name}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">EMI Amount:</span>
                    <span className="text-sm font-semibold text-primary">₹{ledger.loan.installment_amount?.toLocaleString('en-IN')}</span>
                  </div>
                </div>
                ...
                <div className="text-center">
                  <p className="text-xs text-muted-foreground">
                    Open any UPI app (Google Pay, PhonePe, Paytm) and scan this QR code to pay
                  </p>
                </div>
              </div>
              ...
              <DialogFooter className="flex-col gap-2 sm:flex-row sm:justify-between">
                ...
                <Button
                  type="button"
                  variant="default"
                  onClick={() => window.open(qrData?.image_url, '_blank')}
                  className="flex-1"
                >
      ...
      <Tabs defaultValue="details" className="w-full">
        <TabsList className="w-full sm:w-auto overflow-x-auto flex justify-start">
          <TabsTrigger value="details" className="flex-shrink-0">Loan Details</TabsTrigger>
          <TabsTrigger value="schedule" className="flex-shrink-0">Repayment Schedule</TabsTrigger>
          <TabsTrigger value="payments" className="flex-shrink-0">Payment History</TabsTrigger>
          <TabsTrigger value="penalties" className="flex-shrink-0">Penalties</TabsTrigger>
          <TabsTrigger value="ledger" className="flex-shrink-0">Ledger Breakup</TabsTrigger>
        </TabsList>
