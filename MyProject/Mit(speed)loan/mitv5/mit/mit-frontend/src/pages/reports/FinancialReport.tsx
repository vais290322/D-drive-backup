...
export default function FinancialReport() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [activeLoans, completedLoans, customersData, paymentsData, penaltiesData] = await Promise.all([
        api.dashboard.getActiveLoans(),
        api.dashboard.getCompletedLoans(),
        api.customers.getAll(),
        api.payments.getAll(),
        api.penalties.getAll()
      ]);

      // Combine active and completed loans to get full history including manual loans
      setLoans([...activeLoans, ...completedLoans]);
  ...
  const getCustomerName = (customerId: string | any) => {
    // Handle both string IDs and populated customer objects
    const customerIdStr = typeof customerId === 'string' ? customerId : customerId?.id;
    const customer = customers.find(c => c.id === customerIdStr);

    // If customer_id is already a populated object with full_name, use it directly
    if (typeof customerId === 'object' && customerId?.full_name) {
      return customerId.full_name;
    }
  ...
  // Calculate total payable (principal + interest + penalties for active loans)
  let totalPayable = 0;
  ...
  const loanDetails = loans.map(loan => {
    ...
    if (loan.status === "active") {
      // Use summary.totalPayable which includes dynamic penalties
              ...
              <TableBody>
                {loanDetails.map(({ loan, summary, customerName }) => (
                  <TableRow key={loan.id}>
                    <TableCell className="font-medium">{loan.loan_id || loan.id?.slice(0, 8)}</TableCell>
                    <TableCell>{customerName}</TableCell>
                    <TableCell>{formatCurrency(loan.principal_amount || 0)}</TableCell>
                    <TableCell>{formatCurrency(summary.totalPayable || 0)}</TableCell>
                    ...
                    <TableCell>
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${loan.status === "completed"
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center py-2 border-b">
              ...
              <span className="text-lg font-bold text-warning">{formatCurrency(totalPayable)}</span>
            </div>
            <div className="flex justify-between items-center py-2 border-b">
