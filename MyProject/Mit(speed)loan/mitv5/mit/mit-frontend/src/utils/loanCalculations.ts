...
export function generateLoanLedger(
  ...
  // Track running balance (Total Debits - Total Credits)
  // Positive = Debt, Negative = Surplus
  ...
  // Initial loan disbursement entry (Debit)
  ...
  // Track unpaid penalties for allocation logic (just to attribute where money goes, not balance)
  ...
  for (const transaction of allTransactions) {
    if (transaction.type === 'penalty') {
      ...
      // Penalty is a Debit (Increases Debt)
      runningBalance += penaltyAmount;
      ...
      // Payment is a Credit (Reduces Debt/Increases Surplus)
      runningBalance -= Math.round(payment.amount_paid);

      // Allocation Logic (unchanged, just for breakdown columns)
      ...
      // Use remarks for Down Payment or custom notes, otherwise standard format
      const description = (payment.remarks && payment.remarks === 'Down Payment')
        ? 'Down Payment'
        : `Payment - ${payment.payment_mode}${payment.transaction_reference ? ` (${payment.transaction_reference})` : ''}`;
...
export function calculateLoanSummary(ledger: LedgerEntry[], loan: Loan): LoanSummary {
  ...
  // If total outstanding is <= 0, it means the user has paid everything (including future penalties/interest that were part of totalPayable)
  // or has a surplus. In this case, individual components should not show outstanding.
  ...
  return {
    ...
    feesPaid,
    penaltiesPaid,
    outstandingPrincipal: isCleared ? 0 : Math.max(0, outstandingPrincipal),
    outstandingInterest: isCleared ? 0 : Math.max(0, outstandingInterest),
    outstandingPenalties: isCleared ? 0 : Math.max(0, outstandingPenalties),
    outstandingFees: isCleared ? 0 : Math.max(0, outstandingFees),
...
export function isLoanDelayed(loan: Loan, summary: LoanSummary): boolean {
  ...
  // Logic: 
  // If today is due date (Day 0), expected = 1? No, usually delayed means overdue.
  // So we count expected *past due* payments.
  // If daysSinceFirstEmi >= 1 (1 day after due date), expected = 1.

  if (daysSinceFirstEmi < 1) return false; // due today or future

  if (loan.loan_type === 'daily') {
    // Daily: Day 1 = 1 overdue. Day 2 = 2 overdue?
    // If due Jan 1. Jan 2 is 1 day late. 1 payment overdue.
    expectedPayments = daysSinceFirstEmi;
  } else if (loan.loan_type === 'weekly') {
    // Weekly: Day 1-6 = 1 overdue? No, usually just 1 until next week.
    // Day 8 = 2 overdue?
    expectedPayments = 1 + Math.floor((daysSinceFirstEmi - 1) / 7);
  } else {
    // Monthly: Day 1-29 = 1 overdue.
    // Day 31 = 2 overdue.
    expectedPayments = 1 + Math.floor((daysSinceFirstEmi - 1) / 30);
  }
  ...
  return summary.totalPaid < (expectedAmountPaid - 5);
}
