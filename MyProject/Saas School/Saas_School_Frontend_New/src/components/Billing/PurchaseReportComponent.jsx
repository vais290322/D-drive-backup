...
const PurchaseReportComponent = () => {
  ...
  const fetchReport = async () => {
    try {
      ...
      const res = await axios.get(
        `${BASE_URL}/api/reports/${schoolId}/alltime/purchases`
      );
  ...
  const filteredData = recentTransactions.filter((transaction) => {
    const matchesSearch =
      searchTerm === "" ||
      transaction.purchaseNo.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const filteredSummary = filterToday
    ? {
        ...
        totalItemsPurchased: filteredData.reduce(
  ...
  const downloadExcel = async () => {
    try {
      ...
      const fileName = filterToday
        ? `purchase_report_today_${today}.xlsx`
      ...
      toast.success(
        filterToday
          ? "Today's purchase data downloaded successfully"
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        >
          <h3 className="text-lg font-semibold">Total Items Purchased</h3>
          <p className="text-2xl">{filteredSummary.totalItemsPurchased || 0}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4 mb-6 items-end">
        <div className="flex-1 min-w-[200px]">
          ...
          <Input
            type="text"
            placeholder="Search by Purchase No, Seller Name, or Phone"
            value={searchTerm}
          ...
          <tbody>
            {currentPageData.map((transaction, index) => (
              ...
              >
                ...
                <td className="px-4 py-3">{transaction.sellerPhone}</td>
