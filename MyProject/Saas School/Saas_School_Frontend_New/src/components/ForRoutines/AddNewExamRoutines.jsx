...
const AddNewExamRoutines = () => {
  ...
  const addRow = () => {
    if (rows.length >= 25) {
      toast.warning("You cannot add more than 25 exams");
      return;
    }
    setRows([
      ...rows,
  ...
  const handleSubmit = async(e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${routineUrlApi?.addExamRoutine?.url}/${schoolId}`, 
        updateData, 
        {
          headers: {
      ...
      if(response) {
        toast.success(response.data.message || "Exam Routine added successfully!");
        setAllData({
          id: Date.now(), 
          className: "",
          examType: "",
    ...
    <Dialog>
      <DialogTrigger asChild>
        <Button className={`addClass flex items-center gap-2 ${
          ...
          <span>Add New Exam Routine</span>
        </Button>
      </DialogTrigger>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className={`mb-6 p-4 rounded-lg ${
              ...
              <Button
                ...
                } ${rows.length >= 25 ? 'opacity-50 cursor-not-allowed' : ''}`}
                onClick={addRow}
                disabled={rows.length >= 25}
              >
                <FaPlus className="text-sm" /> 
                {rows.length >= 25 ? 'Maximum Exams Reached' : 'Add Another Exam'}
              </Button>
            </div>
