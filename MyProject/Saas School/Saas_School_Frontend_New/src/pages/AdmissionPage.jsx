...
const AdmissionPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    alternativePhone: "",
    email: "",
    jointImage: null,  // Change from empty string to null
    // Address information
    pincode: "",
    state: "",
    city: "",
  ...
  const handleSubmit = async () => {
    ...
    for (const key in formData) {
      if (key === "services" || key === "subject") {
        // Only append if arrays have values
        if (Array.isArray(formData[key]) && formData[key].length > 0) {
          // Make sure the array is properly stringified
          try {
            const jsonString = JSON.stringify(formData[key]);
            formData1.append(key, jsonString);
          } catch (error) {
            console.error(`Error stringifying ${key}:`, error);
            toast.error(`Error processing ${key} data`);
          }
        }
    ...
    try {
      setPostLoading(true);
      // console.log("Submitting form data:", Object.fromEntries(formData1));
