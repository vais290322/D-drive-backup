...
const RfidScannerComponent = ({ onScanComplete }) => {
  ...
  const handleRfidScan = async (rfid) => {
    try {
      ...
      setLastScan({
        student: response.data.data.student ? true : false,
    ...
    <div className="flex flex-col items-center justify-center p-8 bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl shadow-lg">
      ...
      {!isManualMode && (
        ...
        >
          <motion.div
            animate={controls}
            className="w-48 h-48 bg-gradient-to-br from-sky-500 via-indigo-500 to-blue-600 rounded-xl flex items-center justify-center shadow-lg mb-6 relative"
          >
        ...
        <div className="mt-8 p-4 bg-white rounded-lg shadow-md w-full max-w-md">
          <h3 className="text-lg font-semibold text-gray-800 mb-2">Last Scan</h3>
          {
            lastScan.student ? (
              ...
              <p className="text-gray-700"><span className="font-medium">Teacher:</span> {lastScan.name}</p>
            )
          }
